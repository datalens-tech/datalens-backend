...
from bi_external_api.ext_examples import (
    ...
    ModificationPlanBuilderWorkbookBased,
    SuperStoreExtDSBuilder,
    WorkbookBuilderSingleDatasetSingleDash,
)
    ...
    def get_user_operation_info_list(self) -> list[UserOperationInfo]:
                ...
                (
                    ChartBuilderSingleDataset(
                        ext.ColumnChart(
                            ...
                            sort=[
                                ext.ChartSort(source=ext.ChartFieldRef(id="region"), direction=ext.SortDirection.ASC)
                            ],
                        )
                    )
        ...
        return [
            UserOperationInfo(
                ...
                MText(ru="Получить конфигурацию воркбука.", en="Get workbook config."),
                example_list=[
                    OperationExample(
                ...
                example_list=[
                    ...
                    OperationExample(
                        ...
                        rq=ext.FakeWorkbookCreateRequest(
                            ...
                            connection_secrets=[
                                ext.ConnectionSecret(
                                    conn_name=DEFAULT_CONN_NAME, secret=ext.PlainSecret("My-Lovely-YT-token")
                                )
                            ],
                        ),
                ...
                example_list=[
                    OperationExample(
                        rq=ext.AdviseDatasetFieldsRequest(
                            partial_dataset=SuperStoreExtDSBuilder("%this_name_will_not_be_taken_in_account%")
                            .set_source_as_sub_select()
                            .build(),
                            connection_ref=ext.EntryIDRef(id="%my_connection_id%"),
                        ),
                        rs=ext.AdviseDatasetFieldsResponse(
                            dataset=SuperStoreExtDSBuilder(DEFAULT_CONN_NAME)

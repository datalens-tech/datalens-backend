...
class WorkbookRPCViewYaTeam(WorkbookRPCView[rpc_ya_team.YaTeamOpRequest, rpc_ya_team.YaTeamOpResponse]):
    ...
    async def resolve_tenant_for_operation_request(self, rq: rpc_ya_team.YaTeamOpRequest) -> TenantDef:
        return TenantCommon()

    def translate_operation_request_public_to_private(self, rq: rpc_ya_team.YaTeamOpRequest) -> rpc.WorkbookOpRequest:
        return self.op_translator.translate_op_rq(rq)

    def translate_operation_response_private_to_public(
        self, rs: rpc.WorkbookOpResponse

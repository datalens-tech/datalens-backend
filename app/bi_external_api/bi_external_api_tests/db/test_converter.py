...
def replace_conn_name(dataset: ext.Dataset, search: str, replace: str) -> ext.Dataset:
    return attr.evolve(
        dataset,
        sources=[
            attr.evolve(src, connection_ref=replace) if src.connection_ref == search else src for src in dataset.sources
        ],
    )
...
async def test_set_default_avatar_config(
    code, input_dataset, expected_dataset, wb_ctx_loader, pseudo_wb_path, pg_connection
):
    ...
    assert actual_dataset == replace_conn_name(
        expected_dataset, search=PG_CONN_NAME_PLACEHOLDER, replace=pg_connection.name
    )
...
async def test_chart_ad_hoc_fields(
    ...
    # Target chart
    ext_chart = chart_converter.fill_defaults(
        ext.Chart(
            visualization=ext.Indicator(field=ext.ChartField(source=ext.ChartFieldRef("ad_hoc_f_id"))),
            ad_hoc_fields=[attr.evolve(ad_hoc_field, field=attr.evolve(ad_hoc_field.field, id="ad_hoc_f_id"))],
    ...
    int_chart = chart_converter.convert_chart_ext_to_int(
        ext_chart, datasets_with_applied_actions=datasets_with_applied_actions
    )

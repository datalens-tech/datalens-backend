...
def read_config():  # type: ignore  # TODO: fix
    ...
    deny_dst_from_env = [
        ...
        for idx, subnet in enumerate(os.environ.get("DL_DENY_DST_SUBNET", "").split(","))
    ]
    ...
    if yenv_type in ("production", "staging"):
        ...
        return dict(
            ...
            deny_dst_macros_list=(("_YANDEXNETS_",),),
        )
        ...
        return dict(
            ...
            allow_dst_host_list=(
                ...
                (
                    ...
                    53,
                ),  # Caching DNS cluster with NAT64
...
def set_iptables(mask, ports, rule_result, comment):  # type: ignore  # TODO: fix
    ...
    if ports:
        ...
        run_cmd(tool, "-A", CHAIN_NAME, "-d", mask, "-j", rule_result, "-m", "comment", "--comment", comment)
    ...
    for tool in TOOLS:
        for hostports in config:
            ...
            if ports:  # Setting up hosts with ports
                ...
                run_cmd(tool, "-A", CHAIN_NAME, "-d", hostname, "-j", rule, "-m", "comment", "--comment", comment)


def allow_rqe_to_respond() -> None:
    for tool in TOOLS:
        run_cmd(tool, "-A", CHAIN_NAME, "-m", "state", "--state", "ESTABLISHED", "-j", "ACCEPT")
...
def main():  # type: ignore  # TODO: fix
    ...
    for tool in TOOLS:
        run_cmd(tool, "-D", "OUTPUT", "-m", "owner", "--uid-owner", USER_NAME, "-j", CHAIN_NAME)
        run_cmd(tool, "-F", CHAIN_NAME)
        run_cmd(tool, "--delete-chain", CHAIN_NAME)
    ...
    for tool in TOOLS:
        run_cmd(tool, "--new-chain", CHAIN_NAME)
        run_cmd(tool, "-A", "OUTPUT", "-m", "owner", "--uid-owner", USER_NAME, "-j", CHAIN_NAME)
    ...
    process_hosts_list((("127.0.0.1", "1:32767"), ("::1", "1:32767")), "DROP")
    process_subnet_list((("169.254.0.0/16", None, "BI-4179"),), "DROP")

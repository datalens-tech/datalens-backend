...
TABLE_SCHEMA = dict(**PLAINLY_PROJECTED_FIELDS_SCHEMA, **CUSTOM_FIELDS_SCHEMA)
...
def validate_args(args: Namespace) -> None:
    ...
    for path in args.src:
        ...
        if tags_actual != tags_expected:
            path_errors[path].append(
                f"The path does not match the arguments: expected {tags_expected}, got {tags_actual}"
            )
...
def get_args() -> Namespace:
    ...
    parser.add_argument(
        "--filter-robot", action="store_true", help="if set, would filter robot-datalens records from the result"
    )
...
def run(job_input: JobInput) -> None:
    ...
    records = log.filter(nf.equals("event_code", b"profile_db_request")).project(
        *PLAINLY_PROJECTED_FIELDS_SCHEMA.keys(),
        event_time=ne.custom(
            lambda _: int(dt.Datetime.from_iso(_).replace(tzinfo=timezone.utc).timestamp()), "isotimestamp"
        ),

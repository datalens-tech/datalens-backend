...
def run(entries_table: str, groups_table: str, dash_admins_dst: str, entries_admins_dst: str) -> None:
    ...
    user_entries, group_entries = entries.split(
        nf.equals('kind', b'user'),
        strategy='stop_if_true'
    )

    # for the group grants, map the groups to the users
    group_entries = group_entries.project('entry_id', 'scope', 'subject_id').join(
        job.table(groups_table), by_left='subject_id', by_right='group_id'
    ).project('entry_id', 'scope', 'puid')

    # join split tables and split them again, this time by entry scope (dash or [dataset, connection])
    dash_admins, entries_admins = user_entries.project(
        'entry_id', 'scope', 'puid',
    ).concat(group_entries).unique(*TABLE_SCHEMA).split(
        nf.equals('scope', b'dash'),
        strategy='stop_if_true'
    )

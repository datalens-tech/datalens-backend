"""
Copy billing info to clickhouse to use it in usage tracking
...
def get_args() -> Namespace:
    parser = argparse.ArgumentParser(description='Copy billing info to clickhouse to use it in usage tracking')
...
def main():
    args = get_args()
    ch_table = 'billing_dl_unit_ext_production'
    script_name = 'billing_dl_unit_to_ch'
    ...
    scheduled_update_query = sql.SQL('''
        ...
        from {table}
        where
            creation_time between %(select_from)s and timestamp 'today' or
            coalesce(activation_time between %(select_from)s and timestamp 'today', false)
        order by folder_id;
    ''').format(table=sql.Identifier(args.pg_table))

    with pg_conn.cursor(name='transfer_billing_cursor') as pg_cur:
        if args.load_freq == 'once':
            pg_cur.execute(full_load_query, {'pg_table': args.pg_table})
        else:
            pg_cur.execute(
                scheduled_update_query,
                {'pg_table': args.pg_table, 'select_from': select_from}
            )
        while records := pg_cur.fetchmany(500):
            ch_cur.executemany(
                'INSERT INTO {table} (folder_id, activation_time, creation_time) VALUES'.format(table=ch_table),
                (tuple(record_field for record_field in record) for record in records)
            )

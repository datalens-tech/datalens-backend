...
def get_args() -> argparse.Namespace:
    ...
    group_dt.add_argument('--partition-to-move', required=True,
                          help='Name of partition of a tmp table to move to the usage tracking table, in YYYYMM format')
    group_dt.add_argument('--partition-to-drop',
                          help='Name of partition to drop from the usage tracking table, in YYYYMM format')
    group_dt.add_argument('--usage-tracking-table', default='usage_tracking', help='Name of the usage tracking table')
    group_dt.add_argument('--tmp-table', default='usage_tracking_tmp',
...
def run_transfer(args: argparse.Namespace, token: str) -> None:
    ...
    resp = requests.post(f'{DATA_TRANSFER_ENDPOINT}/v1/transfer/{args.transfer_id}:activate', headers=headers,
                         json={'transfer_id': args.transfer_id})  # TODO: check if body is really required
...
def update_partitions(args: argparse.Namespace) -> None:
    ...
    with ch_conn.cursor() as ch_cur:
        ...
        if actual_table_size != args.table_size:
            raise ValueError(f'Table sizes mismatch: '
                             f'YT table has {args.table_size} rows but CH table has {actual_table_size}')

        # move partition from tmp table to the full one
        query = (f'ALTER TABLE {args.tmp_table} {on_cluster} MOVE PARTITION {args.partition_to_move} '

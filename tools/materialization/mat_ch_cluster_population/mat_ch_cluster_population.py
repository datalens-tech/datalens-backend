...
def add_dictionaries_database_info_to_secret(
    client: VaultClient, secret_id: str, secret_key: str,
    cluster_id: str, host: str, port: int, username: str, password: str,
    dry_run: bool
) -> str:
...
def create_passwords_table(conn, table_name: str, dry_run: bool) -> None:
    CREATE_PASSWORDS_TABLE_TMP = "CREATE TABLE {table_name} (" \
    ...
    if dry_run:
        ...
        with conn.cursor() as cur:
            psycopg2.extras.execute_values(cur, query, [
                (info.database_name, info.user_name, 'in yav', False)
                for info in infos
            ])
...
@click.pass_context
def save_setup(ctx, input_file,
               host, port, dbname, user, password_secret, password_key, target_session_attrs, sslmode,
               table_name):
    dry_run = ctx.obj.get('dry_run')
    ...
    conn = contextlib.nullcontext() if dry_run else psycopg2.connect(
        host=host,
        port=port,
        dbname=dbname,
        user=user,
...
@click.option('--database-user-name', default=DICT_DB_USER_NAME, help='Dictionaries database user name', show_default=True)
@click.option('--random-len', default=32, help='Random string length', show_default=True)
@click.option('--no-all-users-access', is_flag=True, help='Do not grant dictionaries database access to all created users')
@click.pass_context
def make_dict_db(ctx, host, port, user, password_secret, password_key, database_name, database_user_name, random_len, no_all_users_access):
    dry_run = ctx.obj.get('dry_run')
...
def add_dict_db_to_secret(ctx, cluster, host, port, database_user_name, database_password, secret, secret_key):
    ...
    version_id = add_dictionaries_database_info_to_secret(
        ...
        cluster, host, port, database_user_name, database_password,
        dry_run)

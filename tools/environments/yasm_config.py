...
class YASM_BI(YASMConfigurer):
    ...
    def databases(self):
        ...
        for key in ('postgres', 'clickhouse', 'redis'):
            result.update({
                cid: dict(title=name, cid=cid, db_type=key)
                for name, cid in envcfg.get(key, {}).items()})
        return result
    ...
    def panel_layout(self):
        ...
        for router in envcfg['routers_flat']:
            for kind, chart in (('resources', router_resources_all), ('problems', router_problems_all), ('misc', misc_all)):
                signals = self.make_qloud_router_signals(kinds=(kind,), name=router, const=False)
                signals = self._add_title(signals, router)
                chart['signals'].extend(signals)

            signals = self.process_qloud_router_signals(
                name=router, **net_signals_base)
            signals = self._add_title(signals, router)
            net_all['signals'].extend(signals)
        ...
        for db_cfg in self.databases.values():
            ...
            signals = self.process_db_signals(
                default_itype='mdbdom0',
                **net_signals_base,
                **db_cfg)
            signals = self._add_title(signals, db_cfg['title'])
            net_all['signals'].extend(signals)

        http_5xx_all['signals'] = [
            item for item in problems_all['signals']
            if '5xx_responses_sum' in item['title']]
        return (
            (resources_all, problems_all),
            (problems, net_all),
            (router_resources_all, router_problems_all,),
            (db_resources_all, db_problems_all,),
    ...
    def resolve_components(self, envcfg):
        ...
        envcfg['routers_flat'] = sorted(set(
            name
            for routers in envcfg['prj_routers'].values()
            for name in routers))
        return envcfg
    ...
    def alert_notifications(self, **kwargs):
        ...
        return [
            {"template_name": "on_status_change",
             "template_kwargs": {
                 "method": ["email"],
                 "login": ["datalens-mon"],  # XXX: not exactly a login. https://ml.yandex-team.ru/lists/datalens-mon/
                 "status": ["CRIT"],
             }},
            {"template_name": "on_status_change",
             "template_kwargs": {
    ...
    line_title_to_alert_config = {
        "cpu_free_frac": {
            ...
            "crit": [None, 0.05], "warn": [0.05, 0.15],
            "value_modify": _5minute_avg},
        "mem_free_frac": {
            # # monitorado defaults:
            # "crit": [None, 0.15], "warn": [0.15, 0.25],
            "crit": [None, 0.02], "warn": [0.02, 0.2],  # "ok": [0.2, None],
            "value_modify": _minute_avg},
        # "cpu_free_frac": ...
        "workers_free_total_frac": {
            "crit": [None, 0.15], "warn": [0.15, 0.3],  # "ok": [0.3, None],
            "value_modify": _minute_avg},
        # "workers_free_active_frac": ...
        "proxy_upstream_errors_sum": {
            "warn": [2, 18], "crit": [18, None],  # "ok": [None, 2],
            "value_modify": _minute_avg},
    ...
    def get_alert_config(self, title, signal=None, **kwargs):
        ...
        if title.startswith('united_storage__'):
            ...
            return None
        return (
            self.line_title_to_alert_config.get(title) or
            self.line_title_to_alert_config.get(signal.get('_title_base'))
        )

...
def get_nodes_from_tenant(tenant_id: str) -> list[str]:
    ...
    r = requests.get(f'{us_host}/{US_URL}', headers={'x-us-master-token': token, 'x-dl-tenantid': tenant_id}, verify=False)
    raise_for_status(r)
...
def main():
    ...
    for node in nodes:
        try:
            patched = patch_node(s, node, user_map, args.comment, args.remove_old_permissions, args.dry_run, args.verbose)
            patched_count += int(patched)
            skipped_count += int(not patched)
        except Exception as e:
            logging.error(f'Error while patching the node {node}: {str(e)}')
            error_count += 1
    logging.info(f'Finished patching. {patched_count} nodes patched, {skipped_count} skipped, '

...
class ChangelogGatherer:
    ...
    def get_peerdirs_recursive(self, prj, recurse_filter=None):
        ...
        while queue:
            ...
            item_peers_new = [
                peer
                for peer in item_peers
                if peer not in result_set]
            ...
            queue_new = [
                ...
                and (
                    rec_item.startswith(recurse_filter)
                    if recurse_filter is not None
                    else True)
            ]
            if queue_new:
                LOGGER.debug(
                    'peerdirs recurse queue size %r -> %r: %r',
                    len(queue), len(queue) + len(queue_new), queue_new)
...
def main(v_from, v_to, release_env=None, st_ticket_int=False, st_ticket_ext=False, st_ticket_dc=False, st_ticket_il=False, verbose=0):

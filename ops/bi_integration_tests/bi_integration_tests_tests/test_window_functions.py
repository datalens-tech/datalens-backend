...
from dl_testing.utils import skip_outside_devhost, guids_from_titles
...
async def _create_df_from_datalens_response(
    bi_api_client: BIAPIClient, ds_response: Resp, field_titles: List[str],
    ...
    columns_request = Req(
        "post", f"/api/data/v1/datasets/{dataset_id}/versions/draft/result", data_json=request_data_json,
    )
    ...
    field_names_to_types = {field_name: field_type[1][1]
...
async def test_window_functions_db(
    ...
    main_request_executor = RequestExecutor(
        bi_api_client
    )
    ...
    ds_response = \
        await bi_api_client.execute_request(Req("get", f"/api/v1/datasets/{csv_dataset_id}/versions/draft/"))
    ...
    datalens_df = await _create_df_from_datalens_response(
        bi_api_client=bi_api_client, ds_response=ds_response, field_titles=required_fields_from_datalens,
    )
    ...
    datalens_df = await _create_df_from_datalens_response(
        bi_api_client=bi_api_client, ds_response=ds_response,
    ...
    pandas_total_rsum_filtered_with_bfb = pandas_df['Group Sales'].expanding().sum() \
        .loc[pandas_df.index > start_index_for_filter]

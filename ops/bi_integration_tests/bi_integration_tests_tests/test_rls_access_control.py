...
async def test_rls_access(
    ...
    pg_dataset_str_field_id = next(
        field["guid"] for field in response.json["dataset"]["result_schema"] if field["title"] == "Row ID"
    )
    ...
    data_request = Req(
        "post",
        f"/api/data/v1/datasets/{pg_dataset_id}/versions/draft/result",
    ...
    rls_modification_request = Req(
        "put",
        f"/api/v1/datasets/{pg_dataset_id}/versions/draft/",

...
from bi_integration_tests.common import (
    get_dataset_json_from_validation,
    get_sample_db_dataset_json,
)
...
main_api_scenario = [
    ...
    ScenarioStep(
        "post",
        "/api/data/v1/datasets/data/preview",
    ...
    ScenarioStep(
        ...
        data=lambda context: get_dataset_json_from_validation(
            dataset_name=f"Test PG dataset {shortuuid.random(6)}",
    ...
    ScenarioStep(
        "post",
        "/api/v1/datasets/{pg_dataset_id}/versions/draft/validators/schema",
        data=lambda context: {
            "updates": [{"action": "delete_field", "field": {"guid": context["pg_dataset_to_delete_field_id"]}}],
        },
    ),
    ScenarioStep(
        "put",
        "/api/v1/datasets/{pg_dataset_id}/versions/draft/",
        data=lambda context: get_dataset_json_from_validation(
            validation_response_dataset=context["pg_dataset_validation_1_resp"]["dataset"]
        ),
    ),
    ...
    ScenarioStep(
        "post",
        "/api/v1/datasets/{pg_dataset_id}/copy",
        data=lambda context: {
    ...
    ScenarioStep(
        "post",
        "/api/v1/datasets/validators/field",
        data=lambda context: {
            "multisource": True,
            **{
    ...
    ScenarioStep(
        "post",
        "/api/v1/datasets/{pg_dataset_id}/versions/draft/validators/field",
        data={"field": {"guid": str(uuid.uuid4()), "title": "formula_field", "formula": "count(1)"}},
    ),
    #
    # Dataset data
    #
    ScenarioStep(
        "post",
        "/api/data/v1/datasets/{pg_dataset_id}/versions/draft/result",
        data=lambda context: {
    ...
    ScenarioStep(
        "post",
        "/api/data/v1/datasets/{pg_dataset_id}/versions/draft/values/distinct",
        data=lambda context: {
    ...
    ScenarioStep(
        "post",
        "/api/data/v1/datasets/{pg_dataset_id}/versions/draft/values/range",
        data=lambda context: {
            "field_guid": context["pg_dataset_str_field_id"],
        },
    ),
    ScenarioStep(
        "post",
        "/api/data/v1/datasets/{pg_dataset_id}/versions/draft/preview",
        data=lambda context: {
            "columns": [context["pg_dataset_str_field_id"]],
        },
    ),
    ...
    ScenarioStep(
        "post", "/api/v1/info/datasets_publicity_checker", data=lambda context: {"datasets": [context["pg_dataset_id"]]}
    ),
...
async def test_no_access_for_user_without_datalens_role(
    ...
    default_init_context = dict(base_dir=base_dir, pg_0=integration_tests_postgres_1, workbook_id=workbook_id)
    ...
    integration_tests_reporter.send_steps_exec_results(
        ...
        [exec_rpt for exec_rpt in ok_execution_reports if exec_rpt.response.status != exec_rpt.step.expected_status],
    )
    ...
    try:
        ...
        integration_tests_reporter.send_steps_exec_results("No role results", no_role_execution_reports)
        ...
        integration_tests_reporter.send_steps_exec_results("Final cleanup results", cleanup_reports)

    assert all(
        exec_rpt.response.status == 403 for exec_rpt in no_role_execution_reports
    ), "Not all handles return 403 for user without appropriate permissions: \n" + "\n".join(
        [exec_rpt.to_short_string() for exec_rpt in no_role_execution_reports]
    )

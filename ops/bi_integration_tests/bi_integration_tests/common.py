...
class RequestExecutor(TestSetupExecutor):
    ...
    def create_executor(
        ...
        bi_api_client = BIAPIClient.from_settings(base_url, folder_id, logger, account_creds, public_api_key, tenant=tenant)
        return cls(bi_api_client)

    async def update_dataset_from_validation_response(
        self, validation_response_dataset: dict, dataset_id: str, require_ok: bool = True,
    ) -> Resp:
        return await self.bi_api_client.update_dataset_from_validation_response(validation_response_dataset, dataset_id, require_ok)
    ...
    async def setup_dataset(
        ...
        dataset_response = await self.bi_api_client.create_dataset_from_validation_resp(
            dataset,
            f"test_dataset_{connection_name}",
            base_dir,
            workbook_id
        )
...
def get_dataset_json_from_validation(
    ...
    dataset_json = {
        "dataset": validation_response_dataset
    }
...
async def update_dataset(executor: RequestExecutor, dataset_id: str, update_request: Req) -> Resp:
    ...
    if ds_validation_response.status == 200:
        await executor.update_dataset_from_validation_response(
            ds_validation_response.json["dataset"],
            dataset_id=dataset_id,
            require_ok=update_request.require_ok
        )
...
async def add_formula_fields_to_dataset(
    executor: RequestExecutor, dataset_id: str, title_to_formula: dict, require_ok=True,
) -> Resp:
    field_update_request = Req(
        "post", f"/api/v1/datasets/{dataset_id}/versions/draft/validators/schema",
        data_json={
            "updates": [
...
def create_executor(
        bi_api_client: BIAPIClient,
        dls_client: Optional[DLSClient] = None
) -> TestSetupExecutor:

...
from bi_connector_chyt_internal.api.connection_form.components import (
    CHYTInternalFieldName,
    OAuthTokenCHYTRow,
    clique_alias_row,
    cluster_row,
)
from bi_connector_chyt_internal.api.connection_info import CHYTInternalTokenConnectionInfoProvider
...
class CHYTInternalTokenConnectionFormFactory(ConnectionFormFactory):
    def get_form_config(
        ...
        return ConnectionForm(
            ...
            rows=[
                OAuthTokenCHYTRow(
                    name=CommonFieldName.token, fake_value="******" if self.mode == ConnectionFormMode.edit else None
                ),

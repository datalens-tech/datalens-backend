...
class BaseTestAsyncCHYTAdapter:
    ...
    async def mirror_clique_mock(self, aiohttp_client) -> MirrorCliqueMock:
        ...
        async def query_view(request: web.Request):
            ...
            req_stat[req_id] = MirrorCliqueMock.ReqData(headers=CIMultiDict(request.headers.items()), body=body)
        ...
        return MirrorCliqueMock(client=test_client, requests=req_stat)

    async def _test_mirroring_simple(
        self, adapter_cls, conn_target_dto_w_mirroring, mirror_clique_mock: MirrorCliqueMock
    ):
        rci = attr.evolve(

...
DEFINITIONS_STRING = [
    ...
    base.FuncRegexpExtract(variants=[
        V(D.BIGQUERY, sa.func.REGEXP_EXTRACT)
    ]),
    ...
    base.FuncRegexpExtractNth(variants=[
        V(
            D.BIGQUERY,
            lambda text, pattern, ind: sa.type_coerce(sa.func.REGEXP_EXTRACT_ALL(
                text, pattern
            ), sa.ARRAY(sa.String))[ind-1]
        ),
    ]),

    # regexp_match
    base.FuncRegexpMatch(variants=[
        V(D.BIGQUERY, sa.func.REGEXP_CONTAINS)
    ]),

    # regexp_replace
    base.FuncRegexpReplace(variants=[
        V(D.BIGQUERY, sa.func.REGEXP_REPLACE)
    ]),
    ...
    base.FuncSpaceNonConst(variants=[
        V(D.BIGQUERY, lambda value: sa.func.REPEAT(' ', value))
    ]),
    ...
    base.FuncSplit3(variants=[
        V(D.BIGQUERY, lambda text, delim, ind: (
            sa.type_coerce(sa.func.SPLIT(text, delim), sa.ARRAY(sa.String))[ind-1]
        )),
    ]),

    # startswith
    base.FuncStartswithNonConst(variants=[
        V(D.BIGQUERY, sa.func.STARTS_WITH)
    ]),

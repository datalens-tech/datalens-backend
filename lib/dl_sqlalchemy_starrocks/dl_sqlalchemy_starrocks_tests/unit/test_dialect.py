...
def test_ddl_duplicate_key_quoted(sa_engine):
    """Reserved words in DUPLICATE KEY must be properly quoted."""
    table = sa.Table("t", sa.MetaData(), sa.Column("select", mysql_types.BIGINT))
    ddl = str(sa.schema.CreateTable(table).compile(dialect=sa_engine.dialect))
    assert "DUPLICATE KEY(`select`)" in ddl

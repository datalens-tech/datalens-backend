...
from bi_legacy_test_bundle_tests.api_lib.utils import (
    make_dataset_for_replacing as make_dataset, replace_dataset_connection as replace_connection,
)


def test_ch_to_metrika_api(
        api_v1, clickhouse_db, static_connection_id,
        metrika_api_connection_id, appmetrica_api_connection_id,
):
    ...
    ds = replace_connection(
        ...
        new_connection_id=appmetrica_api_connection_id, fail_ok=True
    ).dataset

...
def test_metrica_connection(client, env_param_getter):
    ...
    for counter in invalid_counters:
        ...
        resp = client.post("/api/v1/connections", data=json.dumps(conn_create_data), content_type="application/json")
    ...
    update_resp = client.put(
        "/api/v1/connections/{}".format(conn_id), data=json.dumps({"token": "asdf"}), content_type="application/json"
    )
    assert update_resp.status_code == 200, update_resp.json
...
def test_metrica_api_dataset(api_v1, data_api_v2, metrika_api_connection_id):
    ...
    result_resp = data_api.get_result(
        ...
        filters=[ds.find_field(title="Дата просмотра").filter("BETWEEN", values=["2019-12-01", "2019-12-07 12:00:00"])],

...
def _get_chyt_table_with_large_row(
    table_source_type
):
    return (
        # name, source_kwargs
        ('table_with_large_row', dict(
            source_type=table_source_type,
            parameters=dict(
...
class BaseTestDatasetChyt:
    ...
    def test_large_row_dataset(self, client, data_api, dataset_large_data):
        ...
        assert response.json['message'] == 'Data source failed to respond correctly' \
                                           ' (too large row).'
...
class TestDatasetChyt(BaseTestDatasetChyt):
    ...
    @pytest.fixture(
        params=TABLE_CONCAT_CASES,
        ids=[item[0] for item in TABLE_CONCAT_CASES])
    ...
    @pytest.fixture(
        params=TABLE_WITH_LARGE_ROW,
        ids=[item[0] for item in TABLE_WITH_LARGE_ROW])
...
class TestDatasetChytUserAuth(BaseTestDatasetChyt):
    ...
    def data_api(
        ...
        return SyncHttpDataApiV1(
            client=WrappedAioSyncApiClient(
                int_wrapped_client=TestClientConverterAiohttpToFlask(
                    loop=loop, aio_client=async_api_local_env_low_level_client,
                ),
            ),
            headers=public_ch_over_yt_user_auth_headers)

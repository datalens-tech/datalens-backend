...
def initdb_ready():
    ...
    return wait_for_initdb(
        initdb_host=get_test_container_hostport(
            "init-db",
            fallback_port=50508,
        ).host,
...
def app(
    ...
    app = create_app_sync(
        ...
        testing_app_settings=ControlApiAppTestingsSettings(fake_tenant=TenantYCFolder(folder_id="folder_1")),
...
def make_sync_services_registry(
    ...
    new_sr = DefaultBiApiServiceRegistry(
        ...
        if file_uploader_worker_settings is not None
        else DummyTaskProcessorFactory,
...
def _make_dataset(
    client, connection_id, request, source_type=None, created_via=None, db="test_data", table="sample_superstore"
):
    ...
    if created_via:
        body.update(
            {
                "dataset": {"created_via": created_via.name},
            }
        )
    response = client.post(
        "/api/v1/datasets/validators/dataset", data=json.dumps(body), content_type="application/json"
    )
    ...
    response = client.post("/api/v1/datasets", data=json.dumps(data), content_type="application/json")
    assert response.status_code == 200, response.json
...
@pytest.fixture(scope="function")
def async_api_local_env_low_level_client(loop, aiohttp_client, async_app_settings_local_env, connectors_settings):
...
@pytest.fixture(scope="function")
def flask_sync_client_with_mutation_cache(
    loop, async_api_local_env_low_level_client_with_mutation_cache
) -> WrappedAioSyncApiClient:
    return WrappedAioSyncApiClient(
        int_wrapped_client=TestClientConverterAiohttpToFlask(
            loop=loop,
...
@pytest.fixture(scope="function", params=[lazy_fixture("data_api_v2"), lazy_fixture("data_api_v2_with_mutation_cache")])
...
def make_data_api_for_v(
    client: SyncHttpClientBase, v: str
) -> Union[SyncHttpDataApiV1, SyncHttpDataApiV1_5, SyncHttpDataApiV2]:
    return {
...
DB_PARAMS = {
    db_key: DbParams(host=host, password=password) for db_key, (host, password) in tests_config_mod.DB_PARAMS.items()
}
...
def db_dispenser() -> CoreDbDispenser:
    ...
    db_dispenser.add_reinit_hook(
        ...
        reinit_hook=lambda: restart_container_by_label(
            label=tests_config_mod.MSSQL_CONTAINER_LABEL, compose_project=tests_config_mod.COMPOSE_PROJECT_NAME
        ),
    )
    return db_dispenser
...
def create_ch_connection(app, client, request, raw_sql_level=None):
    ...
    resp = client.post("/api/v1/connections", data=json.dumps(conn_params), content_type="application/json")

...
def massevolve(base_exprs: Tuple[Exp, ...], *overrides: ExpMod):
    ...
    """
    overrides_map = {
        expr_mod.name: {key: value for key, value in attr.asdict(expr_mod).items() if not isinstance(value, KeepMarker)}
        for expr_mod in overrides
    }
    result = tuple(attr.evolve(base_expr, **overrides_map.pop(base_expr.name, {})) for base_expr in base_exprs)
    if overrides_map:
...
class BaseCHSelectTest:
    ...
    @classmethod
    def make_ch_subselect_result(cls, query, exprs, conn_id, request, client, api_v1, data_api_v1):
        ds_id = cls.make_ch_subselect_dataset(
            query=query, conn_id=conn_id, request=request, client=client, api_v1=api_v1
        )
        ...
        body = dict(
            updates=updates,
            where=[],
            columns=([col["guid"] for col in result_schema] + [fld["id"] for fld in formula_expr_fields] + []),
        )

        response = data_api_v1.get_response_for_dataset_result(dataset_id=ds_id, raw_body=body)
        ...
        types = {title: _unwrap_type(typestruct) for title, typestruct in res_schema}

        data_lists = res_data["Data"]
        data = [dict(zip(titles, row)) for row in data_lists]

        expr_results = {title: ExpResult(type=types[title], data=[row[title] for row in data]) for title in titles}
...
class TestCHCENaiveDatetimesPassthrough(BaseCHSelectTest):
    ...
    exprs = (
        ...
        Exp("dt_over_pg", "[dt_sys] + [some_rank] * 0", "GenericDatetime", ["2010-10-31T00:00:00"]),
        Exp("dt_over_pg_str", "str([dt_over_pg])", "String", ["2010-10-31 00:00:00"]),
        Exp("dt_over_pg_int", "int([dt_over_pg])", "Int64", [dt_to_api_ts("2010-10-31")]),
    )
...
class TestCHCEAwareDatetimesPassthrough(BaseCHSelectTest):
    ...
    exprs = (
        ...
        Exp("some_rank", "rank(min([dt_str]))", "Int64", ["1"]),
        Exp("dt_over_pg", "[dt_msk] + [some_rank] * 0", "GenericDatetime", ["2010-10-30T20:00:00"]),
        Exp("dt_over_pg_str", "str([dt_over_pg])", "String", ["2010-10-30 20:00:00"]),
        Exp("dt_over_pg_int", "int([dt_over_pg])", "Int64", [dt_to_api_ts("2010-10-30 20:00:00")]),
    )
...
class TestNaiveDatetimeOverFormula(BaseCHSelectTest):
    ...
    exprs = (
        # `name, formula, expected_type, expected_data`
        Exp("dt_str", None, "String", ["2010-10-31 00:00:00"]),
        Exp("dtc_some", "datetime([dt_str])", "GenericDatetime", ["2010-10-31T00:00:00"]),
        Exp("dtc_some_str", "str([dtc_some])", "String", ["2010-10-31 00:00:00"]),
        Exp("dtc_some_shifted", "dateadd([dtc_some], 'day', 1)", "GenericDatetime", ["2010-11-01T00:00:00"]),
        Exp("dtc_some_shifted_str", "str([dtc_some_shifted])", "String", ["2010-11-01 00:00:00"]),
        Exp("dtc_some_shifted_ts", "int([dtc_some_shifted])", "Int64", [dt_to_api_ts("2010-11-01")]),
        Exp("dtc_some_intshifted", "[dtc_some] + 1", "GenericDatetime", ["2010-11-01T00:00:00"]),
        Exp("dtc_some_intshifted_ts", "int([dtc_some_intshifted])", "Int64", [dt_to_api_ts("2010-11-01")]),
    )
...
class TestAwareDatetimeOverFormula(BaseCHSelectTest):
    ...
    exprs = (
        # `name, formula, expected_type, expected_data`
        Exp("dt_str", None, "String", ["2010-10-31 00:00:00"]),
        Exp("dtc_msk", "datetime([dt_str], 'Europe/Moscow')", "GenericDatetime", ["2010-10-31T00:00:00"]),
        Exp("dtc_msk_str", "str([dtc_msk])", "String", ["2010-10-31 00:00:00"]),
        ...
        Exp("dtc_msk_shifted", "dateadd([dtc_msk], 'day', 1)", "GenericDatetime", ["2010-11-01T00:00:00"]),
        Exp("dtc_msk_shifted_str", "str([dtc_msk_shifted])", "String", ["2010-11-01 00:00:00"]),
        Exp("dtc_msk_shifted_ts", "int([dtc_msk_shifted])", "Int64", [dt_to_api_ts("2010-10-31 21:00:00")]),
        # 24-hours shifted (datetime + int in CH)
        Exp("dtc_msk_intshifted", "[dtc_msk] + 1", "GenericDatetime", ["2010-10-31T23:00:00"]),
        Exp("dtc_msk_intshifted_ts", "int([dtc_msk_intshifted])", "Int64", [dt_to_api_ts("2010-10-31 20:00:00")]),
    )
...
class TestAwareDatetimeOverFormulaMostlyAwareVersion(TestAwareDatetimeOverFormula):
    exprs = massevolve(
        ...
        ExpMod("dtc_msk_shifted", KEEP, "DatetimeTZAware", ["2010-11-01T00:00:00+03:00"]),
        ...
        ExpMod("dtc_msk_intshifted", KEEP, "DatetimeTZAware", ["2010-10-31T23:00:00+03:00"]),
...
class TestAwareDatetimeMultipleTimezones(BaseCHSelectTest):
    ...
    exprs = (
        ...
        Exp("dtc_msk", "datetime([dtc_str], 'Europe/Moscow')", "GenericDatetime", ["2010-10-31T00:00:00"]),
        # Note: using `dtc_msk` as base.
        Exp("dtc_utc", "datetime([dtc_msk], 'Etc/UTC')", "GenericDatetime", ["2010-10-30T20:00:00"]),  # +00:00
        Exp("dtc_nyc", "datetime([dtc_msk], 'America/New_York')", "GenericDatetime", ["2010-10-30T16:00:00"]),
        Exp("dtc_nyc_ts", "int([dtc_nyc])", "Int64", [dt_to_api_ts("2010-10-30 20:00:00")]),
        Exp("dtc_pch", "datetime([dtc_msk], 'Pacific/Chatham')", "GenericDatetime", ["2010-10-31T09:45:00"]),
        Exp("dtc_pch_ts", "int([dtc_pch])", "Int64", [dt_to_api_ts("2010-10-30 20:00:00")]),
    )
...
class TestAwareDatetimeByFunc(BaseCHSelectTest):
    ...
    exprs = (
        ...
        Exp(
            "dttzc_msk",
            "datetimetz('2010-10-31 00:00:00', 'Europe/Moscow')",
            "DatetimeTZ",
            ["2010-10-31T00:00:00+04:00"],
        ),
        Exp("dttz_msk", "datetimetz([dt_str], 'Europe/Moscow')", "DatetimeTZ", ["2010-10-31T00:00:00+04:00"]),
    )

...
def test_dataset_revision_id(caplog, client, data_api_v1, dataset_id):
    ...
    dataset = client.get("/api/v1/datasets/{}/versions/draft".format(dataset_id)).json["dataset"]
    r = client.put(
    ...
    def _check_revision_id_mismatch_warning(request_revision_id, actual_revision_id, expected):
        ...
        if expected:
            warning = (
                f"Dataset revision id mismatch: got {request_revision_id} from the request, "
...
def test_create_dataset(client, dataset_id):
    dataset_version = client.get("/api/v1/datasets/{}/versions/draft".format(dataset_id)).json

    assert len(dataset_version["dataset"]["sources"][0]["raw_schema"]) == len(
...
def test_update_dataset_version(client, dataset_id):
    ...
    updated_dataset_version = client.get("/api/v1/datasets/{}/versions/draft".format(dataset_id)).json

    assert (
        len(updated_dataset_version["dataset"]["result_schema"]) == len(dataset_version["dataset"]["result_schema"]) + 1
    )
...
def test_get_preview(client, data_api_v1, static_dataset_id):
    ...
    ds_draft = client.get("/api/v1/datasets/{}/versions/draft".format(dataset_id)).json
...
def test_aggregation_sum(api_v1, data_api_v1, static_dataset_id):
    ...
    assert math.fabs(float(preview_data["Data"][0][0]) - float(preview_data["Data"][0][new_i])) < 1e-6
...
def test_simple_formula(api_v1, data_api_v1, static_dataset_id):
    ...
    assert preview_resp.status_code == 200
    preview_data = preview_resp.data
    assert all(math.fabs(float(row[new_i]) * 2 - float(row[0])) < 1e-5 for row in preview_data["Data"])
...
def test_aggregation_on_formula(api_v1, data_api_v1, static_dataset_id):
    ...
    preview_data = preview_resp.data
    assert all(
        math.fabs(float(item[new_i]) * 2 * float(item[0]) - float(item[1])) < 1e-6 for item in preview_data["Data"]
    )
...
def test_dataset_copy_direct(api_v1, data_api_v1, dataset_id):
    ...
    copy_ds = api_v1.copy_dataset(original_ds, new_key=f"ds_copy/{original_ds.id}_copy_{shortuuid.uuid()}").dataset
    ...
    ds_resp = api_v1.apply_updates(
        ...
        updates=[
            ...
            },
            {
                ...
                "source_avatar": {
                    "id": ds.source_avatars["avatar_1"].id,

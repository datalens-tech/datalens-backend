...
def test_update_file_conn(client, default_sync_usm_per_test, file_connection_with_raw_schema_id):
...
def test_consistency_checks(client, default_sync_usm_per_test, file_connection_with_raw_schema_id):
    conn_id = file_connection_with_raw_schema_id
    usm = default_sync_usm_per_test
    conn = usm.get_by_id(conn_id)
    assert len(conn.data.sources) == 4

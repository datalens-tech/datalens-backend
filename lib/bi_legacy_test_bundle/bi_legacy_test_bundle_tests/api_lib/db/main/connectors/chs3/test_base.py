...
def test_add_dataset_source(
    client, default_sync_usm, api_v1, clickhouse_table, test_chs3_connections, file_status, expected_status
):
...
def test_result(client, api_v1, data_api_v2, test_chs3_connections, s3_native_from_ch_table):
    ...
    source_for_ds = {
        k: v for k, v in sources_resp.json["sources"][2].items() if k in ("source_type", "connection_id", "parameters")
    }
...
def test_update_dataset_source(client, default_sync_usm, api_v1, clickhouse_table, file_connection_with_raw_schema_id):
    ...
    sources_resp = client.get(f"/api/v1/connections/{conn_id}/info/sources")
    source_for_ds = {
        k: v for k, v in sources_resp.json["sources"][0].items() if k in ("source_type", "connection_id", "parameters")
    }

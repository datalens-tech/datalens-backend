...
def test_basic(api_v1, ch_data_source_settings, ch_other_data_source_settings):
    ...
    assert {"source_type": SOURCE_TYPE_CH_TABLE.name} in options["sources"]["compatible_types"]
    assert len(options["sources"]["items"]) == 2
    assert options["sources"]["items"][0]["schema_update_enabled"] is True
    compat_conn_types = {item["conn_type"] for item in options["connections"]["compatible_types"]}
...
def test_direct_connection_replacement_types(
    ...
    repl_conn_types = {item["conn_type"] for item in conn_items[0]["replacement_types"]}
    assert {

...
def test_dataset_field_limit_add_source(
    ...
    def add_avatar_and_relation(suffix: str) -> None:
        ...
        ds.avatar_relations[f"relation{suffix}"] = (
            ds.source_avatars["avatar_ok_0"]
            .join(ds.source_avatars[f"avatar{suffix}"])
            .on(ds.col("single_column") == ds.col("single_column"))
        )
...
def test_dataset_field_limit_refresh_source(
    ...
    table = make_table(
        db=clickhouse_db, name=table_name, columns=[C.int_value(name=f"col_{i}") for i in range(limit_soft)]
    )
    ds = Dataset()
    ds.sources["source_1"] = ds.source(connection_id=static_connection_id, **data_source_settings_from_table(table))
    ...
    make_table(db=clickhouse_db, name=table_name, columns=[C.int_value(name=f"col_{i}") for i in range(limit_soft + 1)])
    ...
    make_table(db=clickhouse_db, name=table_name, columns=[C.int_value(name=f"col_{i}") for i in range(limit_soft)])
    ds_resp = api_v1.apply_updates(
        dataset=ds, updates=[ds.sources["source_1"].refresh(), ds.find_field(title=f"col_{limit_soft}").delete()]
    )
    assert ds_resp.status_code == HTTPStatus.OK
    ds = ds_resp.dataset
    ds_resp = api_v1.save_dataset(dataset=ds, fail_ok=True)
    ...
    make_table(db=clickhouse_db, name=table_name, columns=[C.int_value(name=f"col_{i}") for i in range(limit_hard + 1)])
    ds_resp = api_v1.apply_updates(dataset=ds, updates=[ds.sources["source_1"].refresh()], fail_ok=True)
    assert ds_resp.status_code == HTTPStatus.BAD_REQUEST
    assert ds_resp.bi_status_code == "ERR.DS_API." + ".".join(
        dl_query_processing.exc.DatasetTooManyFieldsFatal.err_code

...
from dl_constants.enums import AggregationFunction, BIType, JoinType, TopLevelComponentId, ComponentType
...
from bi_legacy_test_bundle_tests.api_lib.utils import (
    ...
    data_source_settings_from_table, get_dataset_data
)
...
def test_cascading_type_updates(client, static_dataset_id):
    ...
    batch = [
        *[{'action': 'delete_field', 'field': f}
          for f in result_schema if f['title'] != 'Profit'],  # delete all except Profit
        *[{'action': 'update_field', 'field': {'guid': f['guid'], 'cast': 'float', 'title': 'Profit 0'}}
          for f in result_schema if f['title'] == 'Profit'],  # update Profit
    ]
    response = validate_schema(client, dataset_id=dataset_id, dataset_data=dataset_data, update_batch=batch)
...
def test_source_actions(client, api_v1, ch_data_source_settings, ch_other_data_source_settings):
    ...
    response = client.post(
        '/api/v1/datasets/validators/dataset', content_type='application/json',
...
def test_sources_same_title_different_db(client, api_v1, ch_data_source_settings, ch_data_source_settings_for_other_db):
    ...
    assert dataset_data['sources'][0]['parameters']['db_name'] == (
        ch_data_source_settings['parameters']['db_name']
    )
...
def test_source_avatar_actions(api_v1, ch_data_source_settings):
    ...
    ds = ds_resp.dataset
    assert ds_resp.json['dataset']['source_avatars'] == [dict(
...
def test_avatar_relation_actions(client, api_v1, ch_data_source_settings):
    ...
    data = recycle_validation_response(response.json)
    data['updates'] = [{'action': 'update_avatar_relation',
                        'avatar_relation': {'id': relation['id'], 'join_type': 'left'}}]
    response = client.post(
        '/api/v1/datasets/validators/dataset', content_type='application/json',
        data=json.dumps(data),
    )
...
def test_create_edit_and_save_multisource(api_v1, ch_data_source_settings):
    ...
    ds = api_v1.apply_updates(dataset=ds, updates=[
        ds.source_avatars['avatar_2'].update(title='Renamed')
    ]).dataset
    ds = api_v1.save_dataset(dataset=ds).dataset
...
def test_relation_errors(api_v1, ch_data_source_settings):
    ...
    ds.avatar_relations['relation_1'] = ds.source_avatars['avatar_1'].join(
        ...
        ds.col('Order ID') == ds.col('Order Date')
    )
    ds_resp = api_v1.apply_updates(dataset=ds, fail_ok=True)
    assert ds_resp.status_code == HTTPStatus.BAD_REQUEST
    ds = ds_resp.dataset
    ...
    ds.avatar_relations['relation_2'] = ds.source_avatars['avatar_1'].join(
        ...
        ds.col('Order ID') == ds.col('Nonexistent Field')
    )
    ds_resp = api_v1.apply_updates(dataset=ds, fail_ok=True)
    assert ds_resp.status_code == HTTPStatus.BAD_REQUEST
    ds = ds_resp.dataset
    ...
    # valid relation
    ds.avatar_relations['relation_3'] = ds.source_avatars['avatar_1'].join(
        ds.source_avatars['avatar_2']
    ).on(
        ds.col('Order ID') == ds.col('Order ID')
    )
    ...
    with hacked_join_types(join_types={JoinType.inner}):
        ds_resp = api_v1.apply_updates(dataset=ds, updates=[
            ds.avatar_relations['relation_3'].update(join_type='left')
        ], fail_ok=True)
    assert ds_resp.status_code == HTTPStatus.BAD_REQUEST
    ds = ds_resp.dataset
...
def test_resolve_old_fields_for_new_source_in_avatar(api_v1, ch_data_source_settings):
    ...
    new_dsrc_settings = dict(
        ...
        parameters=dict(ch_data_source_settings['parameters'], table_name='new_table')
    )
...
def test_resolve_old_fields_for_updated_old_source(api_v1, ch_data_source_settings):
    ...
    old_dsrc_settings = dict(
        ch_data_source_settings,
        parameters=dict(ch_data_source_settings['parameters'], table_name='my_table')
    )
    ...
    ds = api_v1.apply_updates(dataset=ds, updates=[
        ds.sources['source'].update(parameters=ch_data_source_settings['parameters'])
    ], fail_ok=True).dataset
...
def test_deletion_of_avatars_for_saved_dataset(api_v1, ch_data_source_settings):
    ...
    ds.avatar_relations['relation_2'] = ds.source_avatars['avatar_2'].join(
...
def test_type_autoupdate_for_multiple_fields(api_v1, ch_data_source_settings):
    ...
    ds = api_v1.apply_updates(dataset=ds, updates=[
        ds.result_schema['my_date'].update(cast='datetime')
    ]).dataset
...
def test_avatar_without_fields(api_v1, ch_data_source_settings):
    ...
    ds_resp = api_v1.apply_updates(dataset=ds, updates=[
        field.delete() for field in ds.result_schema
        if field.avatar_id == ds.source_avatars['avatar_2'].id
    ])
    assert ds_resp.status_code == HTTPStatus.OK
    ds = ds_resp.dataset
...
def test_source_errors(api_v1, ch_data_source_settings, default_sync_usm):
    ...
    ds.sources['source_1'] = ds.source(**dict(
        ch_data_source_settings,
    ...
    ds_resp = api_v1.apply_updates(dataset=ds, updates=[
        ds.sources['source_1'].update(
            parameters=dict(ch_data_source_settings['parameters'])
        )
    ], fail_ok=True)
...
def test_source_title_errors(api_v1, ch_data_source_settings, ch_other_data_source_settings, default_sync_usm):
    ...
    ds_resp = api_v1.apply_updates(dataset=ds, updates=[
        ds.sources['source_1'].update(title='Renamed')
    ], fail_ok=True)
    assert ds_resp.status_code == HTTPStatus.OK, ds_resp.response_errors
    ds = ds_resp.dataset
    assert len(ds.component_errors.items) == 0
...
def test_avatar_title_errors(api_v1, ch_data_source_settings, ch_other_data_source_settings, default_sync_usm):
    ...
    ds_resp = api_v1.apply_updates(dataset=ds, updates=[
        ds.source_avatars['avatar_1'].update(title='Renamed')
    ], fail_ok=True)
    assert ds_resp.status_code == HTTPStatus.OK, ds_resp.response_errors
    ds = ds_resp.dataset
    assert len(ds.component_errors.items) == 0
...
def test_field_is_valid_fix_on_refresh_source(api_v1: SyncHttpDatasetApiV1, static_dataset_id):
    ...
    ds_resp = api_v1.apply_updates(
        dataset=ds,
        updates=[ds.sources[0].refresh(force_update_fields=True)],
        fail_ok=True
    )
    assert ds_resp.status_code == HTTPStatus.OK, ds_resp.response_errors
    ds = ds_resp.dataset
...
def test_field_is_invalid_after_refresh_source_with_deleted_column(
    ...
    expected_field_infos = [
        ('formula_2', False), ('formula_1', False),
        ('old_int', False), ('old_datetime', False),
        ('new_int', True), ('new_datetime', True),
    ]
...
def test_field_dependencies(api_v1, static_dataset_id):
    ...
    refs_by_field_id = {
        item['dep_field_id']: set(item['ref_field_ids'])
        for item in ds.result_schema_aux.inter_dependencies['deps']
    }
...
def test_none_as_connection_id(api_v1, connection_id, clickhouse_table):
    ds = Dataset()
    ds.sources['source_1'] = ds.source(
        connection_id=None, **data_source_settings_from_table(clickhouse_table))
    ds_resp = api_v1.apply_updates(dataset=ds, fail_ok=True)
...
def test_field_from_nonexistent_avatar(api_v1, static_dataset_id):
    ...
    ds_resp = api_v1.apply_updates(dataset=ds, updates=[
        ds.find_field(title='My Field').update(title='My Field Renamed')
    ], fail_ok=True)
    assert ds_resp.status_code == HTTPStatus.BAD_REQUEST
    assert ds_resp.bi_status_code == 'ERR.DS_API.VALIDATION.ERROR'  # non-fatal
    ds = ds_resp.dataset
    ...
    ds_resp = api_v1.apply_updates(dataset=ds, updates=[
        ds.find_field(title='My Field Renamed').delete()
    ], fail_ok=True)
    assert ds_resp.status_code == HTTPStatus.OK, ds_resp.response_errors
    ds = ds_resp.dataset

...
def test_sources_with_different_columns(clickhouse_db, connection_id, api_v1):
    db = clickhouse_db
    old_table = make_table(
        db=db, name=_make_name(), columns=[TestColumn.int_value("int_1"), TestColumn.int_value("int_2")]
    )
    new_table = make_table(
        db=db, name=_make_name(), columns=[TestColumn.int_value("int_1"), TestColumn.int_value("int_3")]
    )
    ...
    ds_resp = api_v1.apply_updates(
        dataset=ds, updates=[ds.source_avatars["avatar"].update(source_id=ds.sources["new_source"].id)], fail_ok=True
    )
...
def test_single_src_with_tables_having_different_columns(clickhouse_db, connection_id, api_v1):
    ...
    ds_resp = api_v1.apply_updates(
        dataset=ds,
        updates=[
            ds.sources["source"].update(parameters=dict(ds.sources["source"].parameters, table_name=new_table.name)),
            ds.source_avatars["avatar"].update(
                title=new_table.name,
                source_id=ds.source_avatars[
                    "avatar"
                ].source_id,  # to make it look like we're updating more than just the title
            ),
        ],
        fail_ok=True,
    )

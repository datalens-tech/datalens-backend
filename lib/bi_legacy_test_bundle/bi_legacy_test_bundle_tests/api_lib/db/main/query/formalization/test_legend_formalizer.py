...
def test_tree_legend_formalization(api_v1, clickhouse_db, default_sync_usm, connection_id):
    ...
    filter_items = [item for item in legend.items if isinstance(item.role_spec, FilterRoleSpec)]
    filter_role_specs = [item.role_spec for item in legend.items if isinstance(item.role_spec, FilterRoleSpec)]

...
from dl_api_lib.pivot.primitives import (
    PivotMeasureSorting, PivotMeasureSortingSettings, PivotHeaderValue, PivotHeaderRoleSpec
)
...
def test_pivot_empty_data_with_grand_totals(api_v1, data_api_v2, dataset_id):
    ...
    pivot_abs = check_pivot_response(
        dataset=ds, data_api=data_api_v2,
        columns=['Category', MEASURE_NAME_TITLE], rows=[], measures=['Sales Sum', 'Profit Sum'],
        ...
        ], totals=[()], filters=[ds.find_field('Category').filter(WhereClauseOperation.STARTSWITH, ['lol'])]
    )
...
def test_pivot_measure_sorting_with_total_and_empty_name_column(api_v1, data_api_v2, dataset_id, role):
    ...
    pivot_abs = check_pivot_response(
        dataset=ds, data_api=data_api_v2,
        columns=['Category without Furniture'], rows=['Region'], measures=['Sales Sum'],
...
def test_pivot_with_totals_and_measure_filters(api_v1, data_api_v2, dataset_id):
    ...
    pivot_abs = check_pivot_response(
        ...
        filters=[
            ds.find_field('Sales Sum').filter(WhereClauseOperation.GT, [10.0])
        ],

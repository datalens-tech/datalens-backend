...
def totals_test(api_v1, data_api_legacy, client, static_dataset_id) -> TotalsTestEnv:
    ...
    batch = [field.update(aggregation="avg") for field in ds.result_schema if field.title in measures]
...
def test_get_dataset_version_result_with_totals(totals_test: TotalsTestEnv):
    ...
    count_idx = next(idx for idx, col in enumerate(rd["data"]["Type"][-1][-1]) if col[0] == "count")
    for idx, totals_value in enumerate(rd["totals"]):
        ...
        if type_name == "Date":
            ...
            expected_value = datetime.date.fromordinal(
                int(
                    mean(
...
def test_v2_totals(api_v1, data_api_v2, dataset_id):
    ...
    result_resp = data_api.get_result(
        ...
        fields=[
            ...
            # Total
            ds.placeholder_as_req_legend_item(role=FieldRole.template, template="Total", block_id=1, legend_item_id=2),
            ds.find_field(title="Sales Sum").as_req_legend_item(role=FieldRole.total, block_id=1, legend_item_id=3),
        ],
        fail_ok=True,
    )
    ...
    result_resp = data_api.get_result(
        ...
        fields=[
            ...
            ds.placeholder_as_req_legend_item(
                role=FieldRole.template, template="{{ " + cat_field_id + " }} Subtotal", block_id=1, legend_item_id=3
            ),
            ds.find_field(title="Category").as_req_legend_item(block_id=1, legend_item_id=4),
            ds.find_field(title="Sales Sum").as_req_legend_item(role=FieldRole.total, block_id=1, legend_item_id=5),
        ],
        fail_ok=True,
    )
    assert result_resp.status_code == HTTPStatus.OK, result_resp.json
...
def test_v2_subtotals(api_v1, data_api_v2, dataset_id):
    ...
    result_resp = data_api.get_result(
        ...
        fields=[
            ...
            ds.placeholder_as_req_legend_item(role=FieldRole.template, template="-", block_id=2, legend_item_id=22),
            ...
            # Grand Total
            ds.placeholder_as_req_legend_item(role=FieldRole.template, template="Total", block_id=3, legend_item_id=30),
            ds.placeholder_as_req_legend_item(role=FieldRole.template, template="-", block_id=3, legend_item_id=31),
            ds.placeholder_as_req_legend_item(role=FieldRole.template, template="-", block_id=3, legend_item_id=32),

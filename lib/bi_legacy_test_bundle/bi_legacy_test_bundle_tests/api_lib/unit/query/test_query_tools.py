...
def test_copy_and_remap_query():
    ...
    query = CompiledQuery(
        ...
        order_by=[
            CompiledOrderByFormulaInfo(
                ...
                alias='res_2', avatar_ids={'ava_1'},
                direction=OrderDirection.desc,
            )
        ],
        filters=[], join_on=[], limit=None, offset=None,
        joined_from=joined_from_from_avatar_ids(
            from_ids=['ava_1'], base_avatars=base_avatars,
    ...
    remapped_query, remapped_aliases = copy_and_remap_query(
        query, id='new_q', field_name_map=field_name_map,
    ...
    assert remapped_query == CompiledQuery(
        ...
        select=[
            CompiledFormulaInfo(
                ...
                alias='new_q_0', avatar_ids={'ava_2'},
            )
        ],
        group_by=[],
        order_by=[
            CompiledOrderByFormulaInfo(
                ...
                alias='new_q_1', avatar_ids={'ava_2'},
                direction=OrderDirection.desc,
            )
        ],
        filters=[], join_on=[], limit=None, offset=None,
        joined_from=joined_from_from_avatar_ids(
            from_ids=['ava_2'], base_avatars=base_avatars,

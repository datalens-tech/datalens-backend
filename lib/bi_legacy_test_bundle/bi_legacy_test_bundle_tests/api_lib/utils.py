...
def _make_dataset_for_replacing(
    ...
    ds = Dataset()
    ds.sources["source_1"] = ds.source(connection_id=old_connection_id, **data_source_settings_from_table(old_table_1))
...
def make_dataset_for_replacing(
    ...
    ds_resp = _make_dataset_for_replacing(
        api_v1,
...
def replace_dataset_connection(
    api_v1: SyncHttpDatasetApiV1, ds: Dataset, old_connection_id: str, new_connection_id: str, fail_ok: bool = False
) -> HttpDatasetApiResponse:
        ...
        updates=[
            {
                ...
                "connection": {"id": old_connection_id, "new_id": new_connection_id},
            },
        ],
        fail_ok=fail_ok,
    )
...
def validate_schema(
    client, dataset_id: str, update_batch: Optional[List[dict]] = None, dataset_data: Optional[dict] = None
):
...
def data_source_settings_from_table(table: DbTable):
    # TODO: Remove in favor of the one from `dl_api_lib_testing.helpers.data_source`
...
def recycle_validation_response(body: dict) -> dict:
    ...
    return {key: value for key, value in body.items() if key in ("dataset",)}

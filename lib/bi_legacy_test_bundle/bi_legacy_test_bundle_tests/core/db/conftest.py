...
from dl_core_testing.database import (
    ...
    DbTable,
    make_db_config,
    make_multiple_db_tables,
    make_schema,
    make_table,
    make_view_from_table,
)
...
def custom_clickhouse_table(clickhouse_db, request):
    ...
    table = clickhouse_db.table_from_columns(
        [
            ...
            sa.Column(
                name="my_string_lowcardinality", type_=ch_types.LowCardinality(ch_types.Nullable(ch_types.String()))
            ),
            ...
            sa.Column(
                name="my_datetime", type_=ch_types.DateTime()
...
def saved_dataset_for_view(default_sync_usm, db_view, saved_connection):
    us_manager = default_sync_usm
    dataset = make_dataset(sync_usm=us_manager, db_table=db_view.as_table, connection=saved_connection)
    us_manager.save(dataset)
    yield dataset
    us_manager.delete(dataset)
...
def saved_schematized_dataset_for_view(default_sync_usm, schematized_db_view, saved_schematized_connection):
    ...
    dataset = make_dataset(
        sync_usm=us_manager,
        db_table=schematized_db_view.as_table,
        connection=saved_schematized_connection,
    )
    us_manager.save(dataset)
    yield dataset

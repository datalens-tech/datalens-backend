...
class CHLikeBaseTestSet(BaseConnExecutorSet):
    ...
    def _ch_cd(self, name, sa_type, bi_type, nt_name=None, nullable=True, nt=None, ct=CONNECTION_TYPE_CLICKHOUSE):
        ...
        if nt is None:
            assert nt_name
            nt = ClickHouseNativeType(conn_type=ct, name=nt_name, nullable=nullable, lowcardinality=False)
        return self.CD(cn=name, sa_type=sa_type, ut_bi_type=bi_type, nt_name_str=nt_name, nt=nt)
...
class BaseClickHouseTestSet(CHLikeBaseTestSet):
    ...
    def index_test_case(self, clickhouse_db, request):
        ...
        map_case_name_ddl_idx = dict(
            ...
            one_columns_sorting=(
                "CREATE TABLE `{}` (num_1 UInt64, num_2 UInt64, txt String)" " ENGINE = MergeTree() ORDER BY (num_1)",
                [
                    IndexInfo(
                        columns=("num_1",),
    ...
    def all_supported_types_test_case(self, custom_clickhouse_table):
        ...
        return self.TypeDiscoveryTestCase(
            ...
            expected_schema_info=self.column_data_to_schema_info(columns_data, CONNECTION_TYPE_CLICKHOUSE),
        )

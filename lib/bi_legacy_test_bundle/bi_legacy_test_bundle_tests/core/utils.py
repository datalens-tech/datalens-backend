...
OTHER_DBS = {ConnectionType[name]: url for name, url in config.OTHER_DBS.items()}
...
def assert_no_warnings(caplog):
    for record in caplog.records:
        assert record.levelno < logging.WARNING, f"Found a record with warning (or higher) level in log: {record}"
...
def add_source(
    ...
    *,
    ...
    ds_wrapper.add_data_source(
        source_id=source_id,
        role=role,
        created_from=SOURCE_TYPE_BY_CONN_TYPE[connection.conn_type],
...
def col(ds_wrapper: DatasetTestWrapper, *parts) -> ColumnClause:
    return sa.literal_column(".".join([ds_wrapper.quote(part, role=DataSourceRole.origin) for part in parts]))
...
def patch_dataset_with_two_sources(
    ...
    source_1_id = add_source(
        ...
        fill_raw_schema=fill_raw_schema,
...
def almost_equal(value_1, value_2, **kwargs):
    ...
    if (
        isinstance(value_1, datetime.datetime)
        and isinstance(value_2, datetime.date)
        and kwargs.get("lenient_date")
        and value_1.hour == 0
        and value_1.minute == 0
        and value_1.second == 0
        and value_1.microsecond == 0
        and value_1.tzinfo is None
    ):
        return True
    ...
    if (
        ...
        and kwargs.get("lenient_bool")
        and value_1 in (0, 1)
        and bool(value_1) == value_2
    ):
        return True
    ...
    if isinstance(value_1, (list, tuple)):
        return len(value_1) == len(value_2) and all(
            almost_equal(value_1[i], value_2[i], **kwargs) for i in range(len(value_1))
        )
    if isinstance(value_1, dict):
        return len(value_1) == len(value_2) and all(
            almost_equal(value_1[key], value_2[key], **kwargs) for key in value_1
        )
...
def make_equal_if_almost(value_1, value_2, **kwargs):
    ...
    if isinstance(value_1, (list, tuple)):
        result = [make_equal_if_almost(item_1, item_2, **kwargs) for item_1, item_2 in zip(value_1, value_2)]
        result += value_1[len(result) :]  # if value_1 was longer
        return type(value_1)(result)

    if isinstance(value_1, dict):
        result = {key: make_equal_if_almost(val, value_2.get(key), **kwargs) for key, val in value_1.items()}
        return result

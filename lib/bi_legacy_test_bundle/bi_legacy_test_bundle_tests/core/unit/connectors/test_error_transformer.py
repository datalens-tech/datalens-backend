...
def test_transform_with_custom_rules():
    transformer = error_transformer.make_default_transformer_with_custom_rules(error_transformer.ErrorTransformerRule(
        when=wrapper_exc_is_and_matches_re(Exception, "Hello"),
    ...
    transformed_default = transformer.make_bi_error(
        Exception()
    )

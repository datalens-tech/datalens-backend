...
async def test_commit_rci_middleware(caplog, aiohttp_client):
    ...
    app = web.Application(middlewares=[
        ...
        commit_rci_middleware(
            rci_extra_plain_headers=('X-Add-Me', 'x-add-me-2'),
            rci_extra_secret_headers=('X-Add-Me-Secret',)
        ),
    ])
    ...
    class TestView(DLRequestView):
        async def get(self):
            ...
            return web.json_response({
                'secret_headers': [
                    [k, v] for k, v in rci.secret_headers.items()
                ],
                'plain_headers': [
                    [k, v] for k, v in rci.plain_headers.items()
                ],
    ...
    actual_headers = await get_headers(CIMultiDict((
        ('X-Add-Me', 'plain_val'),
        ('X-Ignore-Me', 'some'),

...
class BaseCHYTTestSet(CHLikeBaseTestSet):
    ...
    def index_test_case(self, request) -> "TypeDiscoveryTestCase":
        test_data = dict(
            ...
            two_col=(
                "//home/yandexbi/datalens-back/bi_test_data/bi_2132_two_col_sorting",
                (
                    "col_sort_2",
                    "col_sort_1",
                ),
    ...
    def is_table_exists_test_case(self, request) -> BaseConnExecutorSet.TableExistsTestCase:
        ...
        if param == "not_exists":
            ...
            yield self.TableExistsTestCase(TableIdent(table_name=TEST_TABLE, schema_name=None, db_name=None), True)
        else:
...
class BaseTestCHYTAsyncAdapterConnExecutor(BaseCHYTTestSet):
    ...
    async def test_mirroring_ok(self, executor_builder, exec_mode, default_conn_options, caplog):
        ...
        mirrored_q_result_log_rec = await self.wait_for_log_record(
            caplog=caplog, prefix="Mirrored query result", timeout=mirror_req_timeout + 0.5
        )
        assert mirrored_q_result_log_rec.message == f"Mirrored query result: response from '{mirror_clique}' status=200"
        ...
        assert mirrored_q_result_log_rec.message == "Mirrored query result: timeout"
...
class TestCHYTUserAuthAsyncAdapterConnExecutor(CHYTUserAuthAuthMixin, BaseTestCHYTAsyncAdapterConnExecutor):
    ...
    async def test_unauthorized(self, executor_builder, conn_dto):
        # Making bad request
        broken_conn_dto = attr.evolve(
            conn_dto, header_authorization="thereismysecrettoken", header_cookie="Session_id=secretcookie"
        )

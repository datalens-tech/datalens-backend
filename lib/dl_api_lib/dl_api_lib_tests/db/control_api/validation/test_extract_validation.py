...
class TestExtractValidation(DefaultApiTestBase):
    ...
    def test_save_dataset_with_valid_extract_settings(
        ...
        extract_update = {
            ...
            "extract": {
                "mode": "manual",
                "filters": [{"id": "filter_1", "field_guid": field_guids[0], "filters": []}],
                "sorting": [{"id": "sort_1", "field_guid": field_guids[0], "direction": "asc"}],
            },
        }
        ...
        assert saved_dataset.extract == ExtractProperties(
            ...
            status=ExtractStatus.disabled,
            filters=[
                FilterField(id="filter_1", field_guid=field_guids[0], filters=[]),
            ],
            sorting=[
                OrderField(id="sort_1", field_guid=field_guids[0], direction=OrderDirection.asc),
            ],
            errors=[],
            last_completed=0,
        )
    ...
    def test_save_dataset_with_invalid_sorting_fields_fails(
        ...
        extract_update = {
            ...
            "extract": {
                "mode": "manual",
                "filters": [],
                "sorting": [{"id": "invalid_sort", "field_guid": "non-existent-guid", "direction": "asc"}],
            },
        }
    ...
    def test_save_dataset_with_invalid_filter_fields_fails(
        ...
        extract_update = {
            ...
            "extract": {
                "mode": "manual",
                "filters": [{"id": "invalid_filter", "field_guid": "non-existent-guid", "filters": []}],
                "sorting": [{"id": "sort_1", "field_guid": field_guids[0], "direction": "asc"}],
            },
        }
    ...
    def test_update_extract_with_existing_fields_succeeds(
        ...
        updated_extract = {
            ...
            "extract": {
                "mode": "schedule",
                "filters": [{"id": "filter_1", "field_guid": field_guids[0], "filters": []}],
                "sorting": [{"id": "sort_1", "field_guid": field_guids[0], "direction": "desc"}],
            },
        }
        ...
        assert ds_resp.dataset.extract == ExtractProperties(
            ...
            sorting=[
                OrderField(id="sort_1", field_guid=field_guids[0], direction=OrderDirection.desc),
            ],
            errors=[],
            last_completed=0,
        )
    ...
    def test_update_extract_with_invalid_sorting_fields_fails(
        ...
        extract_update = {
            ...
            "extract": {
                "mode": "manual",
                "filters": [],
                "sorting": [{"id": "valid_sort", "field_guid": field_guids[0], "direction": "asc"}],
            },
        }
    ...
    def test_update_extract_with_invalid_filter_fields_fails(
        ...
        extract_update = {
            ...
            "extract": {
                "mode": "manual",
                "filters": [{"id": "valid_filter", "field_guid": field_guids[0], "filters": []}],
                "sorting": [{"id": "sort_1", "field_guid": field_guids[0], "direction": "asc"}],
            },
        }
    ...
    def test_extract_enabled_requires_non_empty_sorting(
        ...
        errors = [error for error in dataset.component_errors.items if error.id == "__extract_properties__"]
        assert len(errors) == 1
        assert errors[0].errors[0].code == "ERR.DS_API.VALIDATION.ERROR.EXTRACT.SORTING_EMPTY"

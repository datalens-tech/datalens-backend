...
DEFINITIONS_DATETIME = [
    ...
    base.FuncDateadd3DateConstNum(
        variants=[
            V(
                D.STARROCKS,
                lambda date, what, num: sa.cast(date + datetime_interval(un_literal(what), un_literal(num)), sa.Date()),
            ),
        ]
    ),
    ...
    # datetrunc — use native StarRocks DATE_TRUNC
    ...
    base.FuncDayofweek2(
        variants=[
            V(
                D.STARROCKS,
                lambda date, firstday: base.dow_firstday_shift((sa.func.DAYOFWEEK(date) + 5) % 7 + 1, firstday),
            ),
        ]
    ),
    # genericnow

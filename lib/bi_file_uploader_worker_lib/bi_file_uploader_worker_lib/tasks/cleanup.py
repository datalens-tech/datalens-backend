...
from bi_file_uploader_lib.redis_model.models import DataSourcePreview, PreviewSet
from bi_file_uploader_lib.redis_model.models import RenameTenantStatusModel

from bi_file_uploader_worker_lib.tasks.save import _make_source_s3_filename
    ...
    async def run(self) -> TaskResult:
        try:
            ...
            async with RedisLock(redis, name=lock_key, timeout=120, blocking_timeout=120):
                ...
                # lc == lifecycle
                try:
                    lc_config = await s3_client.get_bucket_lifecycle_configuration(Bucket=s3_service.persistent_bucket_name)
                except ClientError as ex:
                    if ex.response['Error']['Code'] == 'NoSuchLifecycleConfiguration':
                        lc_config = {'Rules': []}
                    else:
...
@attr.s
class CleanupTenantFilePreviewsTask(BaseExecutorTask[task_interface.CleanupTenantFilePreviewsTask, FileUploaderTaskContext]):
                            ...
                            async with usm.locked_entry_cm(
                                ...
                                for source in conn.data.sources:
                                    ...
                                    if old_s3_filename is None:
                                        LOGGER.info(f'Cannot rename file for source_id {source.id} - s3_filename not set')
                                        continue

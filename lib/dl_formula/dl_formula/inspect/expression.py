...
import dl_formula.definitions.type_strategy
import dl_formula.inspect.env
...
def is_constant_expression(
    ...
    if result is None:
        ...
        if isinstance(node, nodes.BaseLiteral) or isinstance(node, nodes.Null):
            ...
            # check if all arguments of a function or an operator are constant;
            # avoid aggregates and WFs, for example count() or count(1)
            result = (
                not dl_formula.inspect.node.is_aggregate_function(node)
                and not dl_formula.inspect.node.is_window_function(node)
                and all(is_constant_expression(arg, env) for arg in node.args)
            )
        else:
            result = False
...
def infer_data_type(
    ...
    def _infer_data_type() -> DataType:
        if isinstance(node, nodes.Field):
            ...
            result = dl_formula.inspect.literal.get_data_type(node)
            ...
            result = dl_formula.definitions.type_strategy.FromArgs().get_from_args(arg_types=child_types)
            ...
            result = dl_formula.inspect.function.get_return_type(
...
def is_double_aggregated_expression(node: nodes.FormulaItem) -> bool:
    level = get_wrapping_level(
        node,
        qualify_node_cb=lambda _node: dl_formula.inspect.node.is_aggregate_function(_node),
        stop_at_level=2,
    )
    return level == 2
...
def contains_extended_aggregations(node: nodes.FormulaItem, include_double_agg: bool = False) -> bool:
    if dl_formula.inspect.node.is_extended_aggregation(node):
        return True
    for child in autonomous_children(node):
...
def iter_aggregation_functions(node: nodes.FormulaItem) -> Generator[nodes.FuncCall, None, None]:
    ...
    for _, child in node.enumerate(max_depth=1):
        ...
        if dl_formula.inspect.node.is_aggregate_function(child):

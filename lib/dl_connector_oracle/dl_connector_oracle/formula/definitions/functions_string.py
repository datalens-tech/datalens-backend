...
DEFINITIONS_STRING = [
    ...
    # notcontains
    base.FuncNotContainsConst.for_dialect(D.ORACLE),
    base.FuncNotContainsNonConst.for_dialect(D.ORACLE),
    base.FuncNotContainsNonString.for_dialect(D.ORACLE),
    # endswith
    ...
    base.FuncEndswithNonConst(
        variants=[
            V(
                D.ORACLE,
                lambda x, y: (sa.func.SUBSTR(x, sa.func.LENGTH(x) - sa.func.LENGTH(y) + 1, sa.func.LENGTH(y)) == y),
            ),
        ]
    ),

...
from typing import TYPE_CHECKING, Any, Callable, ClassVar, Optional, Tuple, Type, TypeVar
...
class YQLAdapterBase(BaseClassicAdapter[_DBA_YQL_BASE_DTO_TV]):
    ...
    _type_code_to_sa = {
        ...
        'Int8': sa.INTEGER, 'Int16': sa.INTEGER, 'Int32': sa.INTEGER, 'Int64': sa.INTEGER,
        'Uint8': sa.INTEGER, 'Uint16': sa.INTEGER, 'Uint32': sa.INTEGER, 'Uint64': sa.INTEGER,
        'Float': sa.FLOAT, 'Double': sa.FLOAT,
        'String': sa.TEXT, 'Utf8': sa.TEXT,
        'Json': sa.TEXT, 'Yson': sa.TEXT,
        'Uuid': sa.TEXT,
        'Date': sa.DATE, 'Datetime': sa.DATETIME,
        'Timestamp': sa.DATETIME, 'Interval': sa.INTEGER,
        "Bool": sa.BOOLEAN,
    }
    _type_code_to_sa = {
        **_type_code_to_sa,
    ...
    def _get_row_converters(self, cursor_info: ExecutionStepCursorInfo) -> Tuple[Optional[Callable[[Any], Any]], ...]:
        ...
        return tuple(
            self._convert_bytes if type_name_norm == 'string'
            else self._convert_ts if type_name_norm == 'timestamp'
            else None

...
DEFINITIONS_BINARY = [
    ...
    base.BinaryPlusDatetimeNumber(variants=[
        V(D.YQL, lambda date, days: (
            date + sa.func.DateTime.IntervalFromMicroseconds(base.as_bigint(days * DAY_USEC))
        )),
    ]),
    base.BinaryPlusGenericDatetimeNumber(variants=[
        V(D.YQL, lambda dt, days: (
            dt + sa.func.DateTime.IntervalFromMicroseconds(base.as_bigint(days * DAY_USEC))
        )),
    ]),

    # -
    base.BinaryMinusInts(variants=[
        V(D.YQL, lambda num1, num2: (
            sa.cast(num1, sa.INTEGER) - sa.cast(num2, sa.INTEGER)
        )),
    ]),
    ...
    base.BinaryMinusDateFloat(variants=[
        V(D.YQL, lambda date, days: (
            date - sa.func.DateTime.IntervalFromDays(
                sa.cast(sa.func.Math.Ceil(days), sa.INTEGER)
            )
        )),
    ]),
    ...
    # _dneq
    base.BinaryEqualDenullified(variants=[

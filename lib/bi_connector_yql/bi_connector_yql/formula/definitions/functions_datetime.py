...
def _datetrunc2_yql_impl(date_ctx, unit_ctx):  # type: ignore
    ...
    if func_name is not None:
        func = getattr(sa.func.DateTime, func_name)
        return sa.func.DateTime.MakeDatetime(sa.func.DateTime.StartOf(
            date_expr,
            func(amount),
        ))
...
DEFINITIONS_DATETIME = [
    ...
    base.FuncDatetrunc2Date(variants=[
        V(
            D.YQL, lambda date, unit:
            sa.func.DateTime.MakeDate(
                getattr(
                    sa.func.DateTime,
                    YQL_DATE_DATETRUNC_FUNCS[base.norm_datetrunc_unit(unit)]
                )(date)
            )
    ...
    base.FuncDayofweek2(variants=[
        V(D.YQL, lambda date_expr, firstday_expr: base.dow_firstday_shift(

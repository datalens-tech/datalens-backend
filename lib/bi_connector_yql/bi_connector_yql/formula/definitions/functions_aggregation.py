...
DEFINITIONS_AGG = [
    ...
    base.AggQuantile(variants=[
        V(D.YQL, lambda expr, quant: sa.func.PERCENTILE(
            expr,
    ...
    base.AggTopConcat1(variants=[
        ...
        V(D.YQL, lambda expr, amount, sep=', ': sa.func.String.JoinFromList(
            sa.func.ListMap(
                sa.func.TOPFREQ(expr, amount),
                ydb_sa.types.Lambda(lambda x: sa.cast(x, sa.Text))
            ),
            ', ',
        )),
    ]),

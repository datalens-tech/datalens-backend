...
DEFINITIONS_TYPE = [
    ...
    base.FuncDate1FromNumber(variants=[
        V(D.YQL, lambda expr: sa.cast(sa.cast(sa.cast(expr, sa.BIGINT), sa.DATETIME), sa.DATE)),  # number -> dt -> date
    ]),
    ...
    base.FuncStrFromUnsupported(variants=[
        ...
        V(D.YQL, lambda value: sa.cast(
            sa.func.ToBytes(sa.func.Yson.SerializePretty(sa.func.Yson.From(value))),
            sa.TEXT)),
    ]),

...
class TestOperatorYQL(YQLTestBase, DefaultOperatorFormulaConnectorTestSuite):
    ...
    def test_in_date(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        assert dbe.eval('[date_value] in (#2014-10-05#)', where='IF([date_value] = #2014-10-05#, TRUE, FALSE)',
                        from_=data_table)
        assert dbe.eval('[date_value] not in (#2014-10-05#)', where='IF([date_value] = #2014-10-06#, TRUE, FALSE)',
                        from_=data_table)

...
class RotateCryptoKeyInConnection(USEntryCrawler):
    ...
    async def process_entry_get_save_flag(
        ...
        fields_with_non_actual_crypto_keys = [
            field_name
            for field_name, key_info in fields_key_info.items()
        ...
        missing_fields = [field_name for field_name, key_info in fields_key_info.items() if key_info is None]

        if missing_fields:
            LOGGER.warning(
                "US entry has missing sensitive fields: %s %s", entry.uuid, missing_fields, extra=logging_extra
            )

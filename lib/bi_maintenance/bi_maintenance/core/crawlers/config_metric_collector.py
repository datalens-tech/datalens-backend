...
from typing import Any, AsyncIterable, Generic, Sequence, TypeVar, Union, Optional
...
class RawIntegerCollector(RawValueCollector[int]):
    ...
    def _get_quantiles(
            self, parts: int, return_values: Sequence[int],
    ...
    def get_aggregate(
            self, quantile_settings: tuple[int, Sequence[int]] = DEFAULT_QUANTILE_SETTINGS, **kwargs: Any,
    ) -> dict[str, Any]:
        return dict(
...
class StatisticsCollector:
    ...
    def get_jsonable_dict(
            self, quantile_settings: tuple[int, Sequence[int]], chars: bool = True,
        ...
        if chars:
            result.update({
                key.name: coll.get_aggregate()
                for key, coll in self._char_collectors.items()
            })
...
class DatasetMetricCollector(USEntryCrawler):
    ...
    async def process_entry_get_save_flag(self, entry: USEntry, logging_extra: dict[str, Any], usm: Optional[AsyncUSManager] = None) -> tuple[bool, str]:
        ...
        self._stats_collector.int_collector(M.ds_cnt_field_formula).add_value(len(
            [field for field in data['result_schema'] if field['calc_mode'] == 'formula']
        ))
        self._stats_collector.int_collector(M.ds_cnt_field_direct).add_value(len(
            [field for field in data['result_schema'] if field['calc_mode'] == 'direct']
        ))
        self._stats_collector.int_collector(M.ds_cnt_field_parameter).add_value(len(
            [field for field in data['result_schema'] if field['calc_mode'] == 'parameter']
        ))
        ...
        for dsrc_coll in data.get('source_collections', []):
            ...
            if origin is not None:
                ...
                if origin.get('schema_name'):
                    self._stats_collector.int_collector(M.ds_len_source_schema_name).add_value(len(origin['schema_name']))
                ...
                if origin.get('raw_schema'):
                    ...
                    for column in origin['raw_schema']:
                        self._stats_collector.int_collector(M.ds_len_source_column_name).add_value(
                            len(column['name']))

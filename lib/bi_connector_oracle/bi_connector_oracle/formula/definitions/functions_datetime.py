...
class FuncDatetrunc2Oracle(base.FuncDatetrunc2):
    ...
    variants = [
        V(D.ORACLE, lambda date, unit: (
            sa.func.TRUNC(
                date,
                literal(FuncDatetrunc2Oracle._oracle_fmt_map[base.norm_datetrunc_unit(unit)], d=D.ORACLE)
            )
            if base.norm_datetrunc_unit(unit) in FuncDatetrunc2Oracle._oracle_fmt_map else
            date
        )),
    ]
    argument_types = [
...
DEFINITIONS_DATETIME = [
    ...
    base.FuncDateadd3DateConstNum(variants=[
        V(D.ORACLE, lambda date, what, num: (
            sa.cast(
                date + datetime_interval(un_literal(what), un_literal(num), literal_mult=True),
                sa.Date
            )
        )),
    ]),
    base.FuncDateadd3DatetimeConstNum(variants=[
        V(D.ORACLE, lambda dt, what, num: (
            sa.cast(
                dt + datetime_interval(what.value, num.value, literal_mult=True),
                sa.Date
            )
        )),
    ]),
    ...
    base.FuncDayofweek2(variants=[
        V(D.ORACLE, lambda date, firstday: sa.cast(
            date - EPOCH_START_D - (EPOCH_START_DOW + base.norm_fd(firstday) - 1), sa.Integer) % 7 + 1),
    ]),
    ...
    base.FuncToday(variants=[
        V(D.ORACLE, lambda: sa.type_coerce(sa.func.TRUNC(
            sa.cast(raw_sql('SYSTIMESTAMP'), sa.Date)
        ), sa.Date)),
    ]),

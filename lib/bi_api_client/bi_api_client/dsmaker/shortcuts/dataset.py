...
from bi_api_client.dsmaker.primitives import Dataset, ParameterValue, ParameterValueConstraint
...
def _add_anything_to_dataset(
    *,
    ...
    dataset: Optional[Dataset] = None, dataset_id: Optional[str] = None,
    ...
    def _add_parameters(ds: Dataset) -> Dataset:
        for field_name, parameter in parameters.items():
            ds.result_schema[field_name] = ds.field(
                cast=parameter[0].type,
                default_value=parameter[0],
                value_constraint=parameter[1]
            )
        return ds
...
def create_basic_dataset(
        *,
        api_v1: SyncHttpDatasetApiV1,
        connection_id: str, data_source_settings: Dict[str, Any],

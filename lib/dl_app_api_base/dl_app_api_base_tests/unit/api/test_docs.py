...
class TestHttpServerAppDocs(dl_app_api_ext_testing.DocsExtTestSuite):
    @pytest.mark.asyncio
    async def test_docs_spec_content(
        self,
        ...
        assert (
            spec == expected_spec
        ), "Spec mismatch. Set REWRITE_DOC_SPECS environment variable to rewrite expected spec."

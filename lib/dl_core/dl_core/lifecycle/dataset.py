    ...
    def post_copy_hook(self) -> None:
        ...
        for dsrc_coll_spec in own_collection_specs:
            ds_editor.remove_data_source(
                source_id=dsrc_coll_spec.id, role=DataSourceRole.materialization, delete_mat_table=False
            )
            ds_editor.remove_data_source(
                source_id=dsrc_coll_spec.id, role=DataSourceRole.sample, delete_mat_table=False
            )

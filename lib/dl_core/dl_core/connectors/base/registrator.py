...
class CoreConnectorRegistrator:
    ...
    def register_source_definition(cls, source_def: type[CoreSourceDefinition]) -> None:
        ...
        if source_def.us_storage_schema_cls is not None:
            ...
            LOGGER.warning(
                "Source definition %s for type %s has no us_storage_schema_cls",
                source_def.__name__,
                source_def.source_type.value,
            )
    ...
    def register_connection_definition(
        ...
        if conn_def.us_storage_schema_cls is not None:
            register_connection_schema(conn_cls=conn_def.connection_cls, schema_cls=conn_def.us_storage_schema_cls)
        else:
            LOGGER.warning(
                "Connection definition %s for type %s has no us_storage_schema_cls",
                conn_def.__name__,
                conn_def.conn_type.value,
            )
        ...
        if conn_def.sync_conn_executor_cls is not None:
            ...
            LOGGER.warning(
                "Connection definition %s for type %s has no sync_conn_executor_cls",
                conn_def.__name__,
                conn_def.conn_type.value,
            )
        ...
        if conn_def.async_conn_executor_cls is not None:
            ...
            LOGGER.warning(
                "Connection definition %s for type %s has no async_conn_executor_cls",
                conn_def.__name__,
                conn_def.conn_type.value,
            )
        ...
        if conn_def.settings_definition is not None:
            ...
            LOGGER.warning(
                "Connection definition %s for type %s has no settings_definition",
                conn_def.__name__,
                conn_def.conn_type.value,
            )

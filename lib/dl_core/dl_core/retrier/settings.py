...
class RetryPolicySettings(dl_settings.BaseSettings):
    ...
    retryable_codes: set[int] = Field(default_factory=lambda: set([500, 501, 502, 503, 504, 521]))
...
class RetryPolicyFactorySettings(dl_settings.BaseSettings):
    RETRY_POLICIES: dict[str, RetryPolicySettings] = Field(default_factory=dict)
    DEFAULT_POLICY: RetryPolicySettings = Field(default_factory=RetryPolicySettings)

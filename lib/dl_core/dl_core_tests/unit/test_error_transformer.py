...
def test_transform_with_custom_rules():
    transformer = error_transformer.make_default_transformer_with_custom_rules(
        error_transformer.ErrorTransformerRule(
            when=wrapper_exc_is_and_matches_re(Exception, "Hello"), then_raise=exc.InvalidQueryError
        )
    )
    ...
    assert isinstance(transformed_invalid_query, exc.InvalidQueryError)
    ...
    assert isinstance(transformed_no_such_table, exc.SourceDoesNotExistError)

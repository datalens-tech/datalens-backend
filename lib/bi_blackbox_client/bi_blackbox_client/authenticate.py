...
from bi_blackbox_client.tvm_client import (
    TVM_INFO,
    get_tvm_headers,
)
...
def _blackbox_req_prepare(  # type: ignore  # TODO: fix
    ...
    if session_id_cookie or sessionid2_cookie:
        params.update((("method", "sessionid"),))
        body_params.update((("sessionid", session_id_cookie),))
        if sessionid2_cookie:
            body_params.update((("sslsessionid", sessionid2_cookie),))

        if statbox_id is not None:
            params.update((("statbox_id", statbox_id),))

        if user_ticket:
            params.update((("get_user_ticket", "yes"),))
    elif authorization_header:
        params.update((("method", "oauth"), ("scopes", ",".join(scopes))))
        headers.update((("Authorization", authorization_header),))
        # Note: empty body, POST request, still works.
    else:
        return None, None, None

    if tvm_info is None and (os.environ.get("BI_FORCE_BLACKBOX_TVM") or force_tvm):
        tvm_info = TVM_INFO

...
class BaseCHBillingAnalyticsTestClass(BaseConnectionTestClass[BillingAnalyticsCHConnection]):
    ...
    def saved_connection(
        ...
        monkeypatch.setattr(BillingAnalyticsCHConnectionLifecycleManager, "post_init_async_hook", _post_init_hook)
        conn = make_saved_ch_billing_analytics_connection(sync_usm=sync_us_manager, **connection_creation_params)

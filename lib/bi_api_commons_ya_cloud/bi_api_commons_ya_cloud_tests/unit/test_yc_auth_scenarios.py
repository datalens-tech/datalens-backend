...
from typing import Dict, Any, ClassVar, Callable, Generator, Union
...
class Scenario_YCAuth_Base:  # noqa
    ...
    def yc_token_header_mode(self, request) -> YcTokenHeaderMode:
        ...
        assert isinstance(actual_param, YcTokenHeaderMode), \
    ...
    def test_no_headers(self, client, should_be_ignored_header):
        ...
        """
        headers = None if should_be_ignored_header is None else get_headers({
            should_be_ignored_header: "dummy-iam-token"
        })
    ...
    @pytest.mark.parametrize("yc_token_header_mode", [
    ...
    def test_correct_error_on_invalid_authorization_header(self, client):
        ...
        """
        resp = client.get("/auth_ctx", headers=get_headers({DLHeadersCommon.AUTHORIZATION_TOKEN: 'some-garbage-14351234'}))
...
class Scenario_YCAuth_ModeYC(Scenario_YCAuth_Base):
    ...
    def unauthorized_user(self, iam) -> IAMMockUser:
        user = IAMMockUser(
            id="the-no-perms-user-id",
            iam_tokens=["the_no_perms_user_iam_token"]
        )
        iam.data_holder.add_users(user)
        return user
    ...
    @pytest.mark.parametrize("tenant_header_mode", (
        "folder_id",
        "tenant_id",
    ))

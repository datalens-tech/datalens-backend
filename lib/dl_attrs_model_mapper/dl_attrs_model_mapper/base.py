...
class ModelDescriptor(BaseClassDescriptor):
    # TODO FIX: Add on_setattr=attr.setters.frozen after MyPy upgrade
    #  At this moment:
    #  attrs_model_mapper/base.py:16: error: Module has no attribute "setters"
    """
    api_types: optional list of enum.Enum where model should be used, if no tag is provided model is used
        for all environments
    api_types_exclude: optional list of enum.Enum where model should NOT be used, if no tag is provided model is used
        for all environments
...
class AttribDescriptor:
    ...
    _description: str | MText | None = attr.ib(default=None)

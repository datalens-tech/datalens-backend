...
class RequestEventKeyTemplate(SettingsBase):
    key: str = s_attrib("KEY")
    headers: tuple[str, ...] = s_attrib("HEADERS", missing_factory=tuple)
...
class RateLimiterPattern(SettingsBase):
    url_regex: str = s_attrib("URL_REGEX")
    methods: tuple[str, ...] = s_attrib("METHODS", missing_factory=tuple)
    event_key_template: RequestEventKeyTemplate = s_attrib("EVENT_KEY_TEMPLATE")
    limit: int = s_attrib("LIMIT")
    window_ms: int = s_attrib("WINDOW_MS")
...
class RateLimiterConfig(SettingsBase):
    PATTERNS: tuple[RateLimiterPattern, ...] = s_attrib(
        "PATTERNS",

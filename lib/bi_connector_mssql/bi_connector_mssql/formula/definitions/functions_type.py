...
class MSSQLFuncBoolFromNumber(base.FuncBoolFromNumber):
    variants = [
        V(D.MSSQLSRV, lambda value: sa.case(
            whens=[(value.is_(None), sa.null()), (value != sa.literal(0), 1)],
            else_=0
        )),
    ]
...
DEFINITIONS_TYPE = [
    ...
    base.FuncFloatFromDate(variants=[
        V(D.MSSQLSRV, lambda value: sa.cast(
            sa.func.DATEDIFF(sa.text('SECOND'), EPOCH_START_S, value), sa.FLOAT)),
    ]),
    ...
    base.FuncIntFromDate(variants=[
        V(D.MSSQLSRV, lambda value: sa.cast(
            sa.func.DATEDIFF(sa.text('SECOND'), EPOCH_START_S, value),
            sa.BIGINT())),
    ]),

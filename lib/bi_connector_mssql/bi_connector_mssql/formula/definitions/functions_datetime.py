...
def make_mssql_datetrunc_week(date: ClauseElement) -> ClauseElement:
    ...
    return sa.cast(sa.cast(set_day_to_monday, sa.types.DATE), sa.dialects.mssql.DATETIME)
...
DEFINITIONS_DATETIME = [
    ...
    base.FuncDateadd3DatetimeConstNum(
        variants=[
            V(
                D.MSSQLSRV,
                lambda dt, what, num: (sa.cast(sa.func.DATEADD(raw_sql(what.value), num.value, dt), sa.DateTime)),
            ),
        ]
    ),
    # datepart
    ...
    base.FuncDatetrunc2Date(
        variants=[
            V(
                D.MSSQLSRV,
                lambda date, unit: (
                    make_mssql_datetrunc(date, unit, sa.func.DATEFROMPARTS(sa.func.YEAR(date), 1, 1))
    ...
    base.FuncDatetrunc2Datetime(
        variants=[
            V(
                D.MSSQLSRV,
                lambda date, unit: make_mssql_datetrunc(

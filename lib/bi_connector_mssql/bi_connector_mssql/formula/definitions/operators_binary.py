...
DEFINITIONS_BINARY = [
    ...
    base.BinaryPlusDateFloat(variants=[
        V(D.MSSQLSRV, lambda date, days: sa.type_coerce(
            sa.func.DATEADD(sa.text('day'), base.as_bigint(days), date), sa.Date
        )),
    ]),
    ...
    base.BinaryMinusDates(variants=[
        V(D.MSSQLSRV, lambda left, right: sa.cast(
            sa.func.FLOOR(sa.cast(sa.cast(left, sa.DateTime()), sa_mssqlsrv.FLOAT) -
                          sa.cast(sa.cast(right, sa.DateTime()), sa_mssqlsrv.FLOAT)),
            sa.BIGINT()
        )),
    ]),
    base.BinaryMinusDatetimes(variants=[
        V(D.MSSQLSRV, lambda left, right: (

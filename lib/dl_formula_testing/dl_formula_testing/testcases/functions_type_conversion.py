...
class DefaultStrTypeFunctionFormulaConnectorTestSuite(FormulaConnectorTestBase):
    ...
    def test_str_from_datetime_custom_tz(self, dbe: DbEvaluator) -> None:
        ...
        assert (
            to_str(dbe.eval('STR(GENERICDATETIME("2019-01-02 03:04:05", "Pacific/Chatham"))')) == "2019-01-02 03:04:05"
        )
    ...
    def test_str_from_const_null(self, dbe: DbEvaluator) -> None:
    ...
    def test_str_from_data_null(self, dbe: DbEvaluator, null_data_table: sa.Table) -> None:
        if self.empty_str_is_null:
            pytest.skip()
...
class DefaultBoolTypeFunctionFormulaConnectorTestSuite(FormulaConnectorTestBase):
    ...
    def test_bool_from_data_null(self, dbe: DbEvaluator, null_data_table: sa.Table) -> None:
...
class DefaultGeopolygonTypeFunctionFormulaConnectorTestSuite(FormulaConnectorTestBase):
    def test_geopolygon(self, dbe: DbEvaluator) -> None:
        assert (
            to_str(dbe.eval('GEOPOLYGON("[[[55,37],[56,37],[56,38],[55,38]],[[50,37],[51,37],[51,38],[50,38]]]")'))

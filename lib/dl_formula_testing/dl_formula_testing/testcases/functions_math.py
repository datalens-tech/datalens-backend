...
class DefaultMathFunctionFormulaConnectorTestSuite(FormulaConnectorTestBase):
    ...
    def test_rounding_functions(self, dbe: DbEvaluator) -> None:
        ...
        # Rounding result mustn't be eq to -0
        assert str(dbe.eval("CEILING(-0.1)")) in ("0", "0.0")
        assert str(dbe.eval("ROUND(-0.1)")) in ("0", "0.0")
        assert str(dbe.eval("ROUND(-0.1, 0)")) in ("0", "0.0")
    ...
    def test_least(self, dbe: DbEvaluator) -> None:
        ...
        assert dt_strip(dbe.eval("LEAST(#2019-01-02 04:03:02#, #2019-01-17 03:02:01#)")) == datetime.datetime(
            2019, 1, 2, 4, 3, 2
        )
    ...
    def test_greatest(self, dbe: DbEvaluator) -> None:
        ...
        assert dt_strip(dbe.eval("GREATEST(#2019-01-02 04:03:02#, #2019-01-17 03:02:01#)")) == datetime.datetime(
            2019, 1, 17, 3, 2, 1
        )

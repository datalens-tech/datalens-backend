...
class DefaultReportingProfiler(ReportingProfiler):
    ...
    def get_data_proc_records_for_query(self, query_id: str) -> Tuple[DataProcessingReportingRecord, ...]:
        ...
        return tuple(r for r in records if isinstance(r, DataProcessingReportingRecord) and r.processing_id in proc_ids)
    ...
    def flush_query_report(self, query_id: str) -> None:
        ...
        start_record = self._find_record_of_type(query_records, rtype=QueryExecutionStartReportingRecord)
        assert start_record is not None, "need a matching `QueryExecutionStartReportingRecord`"
        end_selector_record = self._find_record_of_type(query_records, rtype=QueryExecutionEndReportingRecord)
        end_data_proc_record = self._find_record_of_type(query_records, rtype=DataProcessingEndReportingRecord)
        cache_selector_record = self._find_record_of_type(query_records, rtype=QueryExecutionCacheInfoReportingRecord)
    ...
    def flush_all_query_reports(self) -> None:
        all_query_ids = sorted(
            {
                r.query_id
                for r in self.reporting_registry.get_reporting_records()

...
import bi_formula.definitions.base as op_base
...
class OperationRegistry:
    ...
    def get_definition(
        ...
        if dialect is not None and for_any_dialect or dialect is None and not for_any_dialect:
            raise ValueError('Either dialect should be provided or for_any_dialect be set to True.'
                             ' Cannot provide both.')
    ...
    def items(self) -> Generator[tuple[FuncKey, 'op_base.NodeTranslation'], None, None]:
        ...
        sorted_keys = sorted(
            self.ops.keys(),
            key=lambda key: (key[0], key[1] if key[1] is not None else float('inf'), *key[2:])
        )

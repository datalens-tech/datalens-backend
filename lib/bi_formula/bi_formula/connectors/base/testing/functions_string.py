...
from bi_formula.testing.util import to_str, to_unicode
...
class DefaultStringFunctionFormulaConnectorTestSuite(FormulaConnectorTestBase):
    ...
    def test_regexp_extract(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        assert to_str(dbe.eval('REGEXP_EXTRACT([str_null_value], "or..")',
                               from_=data_table)) in (None, '')
    ...
    def test_regexp_extract_nth(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        assert to_str(dbe.eval('REGEXP_EXTRACT_NTH([str_null_value], "or..", 2)',
                               from_=data_table)) in (None, '')
    ...
    def test_space(self, dbe: DbEvaluator) -> None:
        assert to_str(dbe.eval('SPACE(4)')) == '    '  # SPACE(<int literal>) is evaluated in Python rather than in the DB,
    ...
    def test_contains_extended(
            self, dbe: DbEvaluator, data_table: sa.Table, forced_literal_use: Any,
            value_fl: str, pattern_fl: str, expected: bool,
    ) -> None:

...
class DefaultLiteralFormulaConnectorTestSuite(FormulaConnectorTestBase):
    ...
    def test_datetime_extended(self, dbe: DbEvaluator) -> None:
        ...
        assert as_dt(dbe.db.scalar(
            literal(datetime.datetime(2020, 4, 8, 12, 34, 56), d=dbe.dialect)
        )) == datetime.datetime(
            2020, 4, 8, 12, 34, 56, tzinfo=self.default_tz
        )

        # With microseconds
        assert as_dt(dbe.db.scalar(
            literal(datetime.datetime(
                2020, 4, 8, 12, 34, 56, microsecond=7890
            ), d=dbe.dialect)
        )) == datetime.datetime(
            2020, 4, 8, 12, 34, 56,
        ...
        if self.supports_utc:
            assert as_dt(dbe.db.scalar(
                literal(datetime.datetime(
                    2020, 4, 8, 12, 34, 56, tzinfo=datetime.timezone.utc
                ), d=dbe.dialect)
            )) == datetime.datetime(
                2020, 4, 8, 12, 34, 56, tzinfo=datetime.timezone.utc
            )
        ...
        if self.supports_utc:
            assert as_dt(dbe.db.scalar(
                literal(datetime.datetime(
                    2020, 4, 8, 12, 34, 56, microsecond=7890, tzinfo=datetime.timezone.utc
                ), d=dbe.dialect)
            )) == datetime.datetime(
                2020, 4, 8, 12, 34, 56,
                microsecond=7890 if self.supports_microseconds else 0,
        ...
        if self.supports_custom_tz:
            assert as_dt(dbe.db.scalar(
                literal(pytz.timezone('Europe/Moscow').localize(datetime.datetime(
                    2020, 4, 8, 15, 34, 56, tzinfo=None
                )), d=dbe.dialect)

...
from bi_formula.inspect.function import (
    can_be_window, can_be_nonwindow,
    supports_grouping, supports_ordering,
    supports_bfb, supports_ignore_dimensions,
    supports_lod,
)
...
class FormulaParser(abc.ABC):
    ...
    def get_usage_stats(self) -> ParserStats:
        ...
        return ParserStats(
            ...
            global_cache_maxsize={
                key: stats.maxsize
                for key, stats in global_stats.items()
            },
            global_cache_currsize={
                key: stats.currsize
                for key, stats in global_stats.items()
            },
            ...
            length_counts={
                length: len(times) for length, times in self.by_length.items()
            },
            length_avg_times={
                length: statistics.mean(times) for length, times in self.by_length.items()
            },
            length_median_times={
                length: statistics.median(times) for length, times in self.by_length.items()
            },
        )

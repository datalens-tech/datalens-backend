...
def test_parse(tool):
    ...
    stdout, stderr = tool.run([
        'parse', '--pretty', '--with-meta', 'very_very_long_func_name(nested_func())'
    ])
...
"""
    ...
    stdout, stderr = tool.run([
        'parse', '--suppress-errors',
        ], stdin=('SUM(1)\n'
                  'AVG(2)\n'
                  'INVALID_FORMULA([n1'),
    )
    assert stdout.strip() == ("Formula(expr=FuncCall(name='sum', args=(LiteralInteger(value=1),)))\n"
...
def test_translate(tool):
    ...
    stdout, stderr = tool.run([
        'translate',
        '--dialect', 'clickhouse', '--suppress-errors',
        ], stdin=('SUM([n1])\n'
                  'AVG([n1])\n'
                  'UNKNOWN_FUNC([n1])'),
    )
...
def test_split(tool):
    stdout, stderr = tool.run([
        'split', '--diff', '45 BETWEEN [n2] = 123 AND 456'
    ])
    assert stdout == (
        ' 0: ▫▫▫▫▫▫▫▫▫▫▫▫▫▫▫▫▫▫▫▫▫▫▫▫▫▫▫▫▫\n'
        ' 1: ▫▫ BETWEEN ▫▫▫▫▫▫▫▫▫▫ AND ▫▫▫\n'
...
def test_graph(tool):
    stdout, stderr = tool.run([
        'graph', ('IF ([n0]) IN (5, 6) THEN MY_FUNC(NULL, NOT [n2], "qwerty") '
...
def test_slice(tool):
    stdout, stderr = tool.run([
        'slice', '--diff', '--levels', 'aggregate,window,toplevel',

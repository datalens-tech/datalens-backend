...
from bi_formula.core.nodes import (
    BeforeFilterBy, Binary, CaseBlock, ExpressionList,
    Field, FuncCall, IfBlock, IfPart, IgnoreDimensions,
    LiteralBoolean, LiteralDate, LiteralGenericDatetime,
    LiteralFloat, LiteralInteger, LiteralString,
    Null, OrderAscending, OrderDescending, Ordering,
    Ternary, Unary, WhenPart, WindowFuncCall,
    WindowGroupingAmong, WindowGroupingTotal, WindowGroupingWithin,
    FixedLodSpecifier, IncludeLodSpecifier, ExcludeLodSpecifier,
)
from bi_formula.shortcuts import n


def test_keyword_case_insensitivity(parser):
    upper_formula = ('IF [n0]*5 THEN MY_FUNC(NULL, NOT [n2], "qwerty") ELSEIF ISNULL([n3]) THEN "result" '
                     'ELSE CASE [n4] WHEN 5 THEN "five" WHEN 6 THEN "seven" ELSE "forty two" END END')
    lower_formula = ('if [n0]*5 then MY_FUNC(null, not [n2], "qwerty") elseif ISNULL([n3]) then "result" '
                     'else case [n4] when 5 then "five" when 6 then "seven" else "forty two" end end')
...
def test_position_getter(parser):
    formula = parser.parse(
        'IF [n0] * 5 '
        'THEN MY_FUNC(NULL, NOT [n2], "qwerty") '
        'ELSE [SOMETHING] AND [Q1] IN (12, 34) '
        'END'
    )
...
def test_parse_parentheses(parser):
    ...
    ) == n.formula(n.p(LiteralString.make('expr with parentheses')))
...
def test_parse_operations(parser):
    ...
    assert parse('[left]^-[right]') == n.formula(Binary.make('^', Field.make('left'), Unary.make('neg', Field.make('right'))))
    assert parse('[left] ^ NOT [right]') == n.formula(Binary.make('^', Field.make('left'), Unary.make('not', Field.make('right'))))
    assert parse('[left] IS NULL ^ [right]') == n.formula(Binary.make('^', Unary.make('isnull', Field.make('left')), Field.make('right')))
    ...
    assert parse('-[expr]^2') == n.formula(Unary.make('neg', Binary.make('^', Field.make('expr'), LiteralInteger.make(2))))
    ...
    # priority: is vs +
    assert parse('[n1] + [n2] is null') == n.formula(Unary.make('isnull', Binary.make('+', Field.make('n1'), Field.make('n2'))))

    # like
    assert parse('[left] like [right]') == n.formula(Binary.make('like', Field.make('left'), Field.make('right')))
    assert parse('[left] not like [right]') == n.formula(Binary.make('notlike', Field.make('left'), Field.make('right')))
    # priority: like vs is
    assert parse('[n1] like [n2] is null') == n.formula(Binary.make('like', Field.make('n1'), Unary.make('isnull', Field.make('n2'))))
    ...
    assert parse('[my_field]NOT BETWEEN(123)AND(456)') == n.formula(
        Ternary.make(
            'notbetween',
            Field.make('my_field'),
            n.p(LiteralInteger.make(123)),
            n.p(LiteralInteger.make(456))
        )
    )
    ...
    assert parse('[first] BETWEEN NOT [second] AND NOT [third]') == n.formula(
        Ternary.make('between', Field.make('first'), Unary.make('not', Field.make('second')), Unary.make('not', Field.make('third')))
    )
    ...
    assert parse('[my_field] IN (123, 456)') == n.formula(
        Binary.make('in', Field.make('my_field'), ExpressionList.make(LiteralInteger.make(123), LiteralInteger.make(456)))
    )
    assert parse('[my_field]NOT IN(123, 456)') == n.formula(
        Binary.make('notin', Field.make('my_field'), ExpressionList.make(LiteralInteger.make(123), LiteralInteger.make(456)))
    )
    ...
    # priority: AND vs NOT
    assert parse('NOT [n1] AND [n2]') == n.formula(Binary.make('and', Unary.make('not', Field.make('n1')), Field.make('n2')))
...
def test_parse_if_block(parser):
    assert parser.parse(
        'IF "cond_1" THEN "res_1" ELSEIF "cond_2" THEN "res_2" ELSE "res_3" END'
    ) == n.formula(
        IfBlock.make(if_list=[
    ...
    assert parser.parse(
        'IF "cond_1" THEN "res_1" ELSE IF "cond_2" THEN "res_2" ELSE "res_3" END END'
    ) == n.formula(
        IfBlock.make(if_list=[
            IfPart.make(LiteralString.make('cond_1'), LiteralString.make('res_1')),
        ], else_expr=IfBlock.make(if_list=[
            IfPart.make(LiteralString.make('cond_2'), LiteralString.make('res_2'))
        ], else_expr=LiteralString.make('res_3')))
    )
...
def test_parse_win_func_call(parser):
    ...
    assert parser.parse('mavg([my_field] AMONG [dim1], [dim2])') == n.formula(
        WindowFuncCall.make(
            name='mavg', args=[Field.make('my_field')],
            grouping=WindowGroupingAmong.make(dim_list=[
                Field.make('dim1'),
                Field.make('dim2')
            ])
        )
    ...
    assert parser.parse('mavg([my_field] WITHIN [dim1], [dim2])') == n.formula(
        WindowFuncCall.make(
            name='mavg', args=[Field.make('my_field')],
            grouping=WindowGroupingWithin.make(dim_list=[
                Field.make('dim1'),
                Field.make('dim2'),
            ]),
        )
...
def test_agg_with_lod(parser):
    assert parser.parse('sum([my_field] FIXED [dim1], [dim2])') == n.formula(
        FuncCall.make(
            name='sum', args=[Field.make('my_field')],
            lod=FixedLodSpecifier.make(dim_list=[
                Field.make('dim1'),
                Field.make('dim2')
            ])
        )
    ...
    assert parser.parse('sum([my_field] INCLUDE [dim1], [dim2])') == n.formula(
        FuncCall.make(
            name='sum', args=[Field.make('my_field')],
            lod=IncludeLodSpecifier.make(dim_list=[
                Field.make('dim1'),
                Field.make('dim2'),
            ]),
        )
    ...
    assert parser.parse('sum([my_field] EXCLUDE [dim1], [dim2])') == n.formula(
        FuncCall.make(
            name='sum', args=[Field.make('my_field')],
            lod=ExcludeLodSpecifier.make(dim_list=[
                Field.make('dim1'),
                Field.make('dim2'),
            ]),
        )
...
def test_parse_win_func_call_with_order_by(parser):
    assert parser.parse('mavg([my_field] WITHIN [dim1], [dim2] ORDER BY [dim3])') == n.formula(
        WindowFuncCall.make(
            ...
            ordering=Ordering.make(expr_list=[
                Field.make('dim3'),
            ]),
        )
    )
...
def test_parse_func_call_with_ignore_dimensions(parser):
    assert parser.parse(
        'ago([my_field] IGNORE DIMENSIONS [dim1], [dim2])'
    ) == n.formula(
        FuncCall.make(
            name='ago', args=[Field.make('my_field')],
...
def test_inconceivably_long_if_block(parser):
    ...
    if_list = [IfPart.make(
        cond=FuncCall.make('contains', [LiteralString.make('s'), Field.make('v')]),
        expr=LiteralString.make('a')
    )]
    for if_num in range(num):
        formula += f'elseif contains("s_{if_num}", [v]) then "a_{if_num}"\n'
        if_list.append(IfPart.make(

...
def test_no_errors():
    ...
    # no window function
    validate_window_functions(node=nodes.Formula(
        nodes.FuncCall.make(name='+', args=[
    ...
    validate_window_functions(node=nodes.Formula(
        nodes.WindowFuncCall.make(name='rank', args=[
            nodes.FuncCall.make(name='sum', args=[
                nodes.LiteralString.make('qwerty')
            ]),
    ...
    validate_window_functions(node=nodes.Formula(
        nodes.FuncCall.make(name='+', args=[
            ...
            nodes.WindowFuncCall.make(name='rank_dense', args=[
                nodes.FuncCall.make(name='sum', args=[
                    nodes.LiteralString.make('qwerty')
                ]),
            ], grouping=nodes.WindowGroupingTotal()),
...
def test_nested_window_functions():
    ...
    with pytest.raises(exc.ValidationError) as exc_info:
        validate_window_functions(nodes.Formula(
            nodes.FuncCall.make(name='+', args=[

...
class FuncGreatestCH(base.FuncGreatestBase):
    variants = [
        V(D.CLICKHOUSE, lambda x, y: getattr(sa.func, 'if')(x >= y, x, y))
    ]
    argument_types = [
...
DEFINITIONS_MATH = [
    ...
    # compare
    base.FuncCompare(variants=[
        V(D.CLICKHOUSE, lambda left, right, epsilon: sa.func.multiIf(
            sa.func.lessOrEquals(sa.func.abs(left - right), epsilon), 0,
            sa.func.less(left, right), -1,
            1,
        )),
    ]),

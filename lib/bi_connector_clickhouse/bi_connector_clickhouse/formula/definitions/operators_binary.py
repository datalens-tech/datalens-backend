...
from bi_formula.definitions.base import TranslationVariant, TranslationVariantWrapped
...
from bi_formula.definitions.literals import literal, Literal
...
class CHBinaryEqNEqOrderedBaseArgTransformer(
    ...
    def transform_args(
            self, env: TranslationEnvironment,
            args: list[TranslationCtx], arg_types: list[DataType]
    ) -> list[TranslationCtx]:
        ...
        if env.dialect & D.CLICKHOUSE:  # FIXME: Connectorize
            ...
            # TODO: DATETIMETZ
            if (
                arg_types[0] in {DataType.DATE, DataType.CONST_DATE}
                and arg_types[1] in {DataType.DATETIME, DataType.CONST_DATETIME}
            ):
                args = [self._to_datetime(args[0]), args[1]]
            elif (
                arg_types[0] in {DataType.DATE, DataType.CONST_DATE}
                and arg_types[1] in {DataType.GENERICDATETIME, DataType.CONST_GENERICDATETIME}
            ):
                args = [self._to_genericdatetime(args[0]), args[1]]
            elif (
                arg_types[0] in {DataType.DATETIME, DataType.CONST_DATETIME}
                and arg_types[1] in {DataType.DATE, DataType.CONST_DATE}
            ):
                args = [args[0], self._to_datetime(args[1])]
            elif (
                arg_types[0] in {DataType.GENERICDATETIME, DataType.CONST_GENERICDATETIME}
                and arg_types[1] in {DataType.DATE, DataType.CONST_DATE}
            ):
...
def _get_null_substitute_value(data_type: DataType, dialect: DialectCombo) -> tuple[Literal, bool]:
    ...
    null_value: Literal
    if data_type in {
            DataType.STRING, DataType.CONST_STRING,
            DataType.GEOPOINT, DataType.CONST_GEOPOINT,
            DataType.GEOPOLYGON, DataType.CONST_GEOPOLYGON,
            DataType.MARKUP, DataType.CONST_MARKUP,
    }:
        null_value = lit('-')
    elif data_type in {
            DataType.DATE, DataType.CONST_DATE,
            DataType.DATETIME, DataType.CONST_DATETIME,
            DataType.GENERICDATETIME, DataType.CONST_GENERICDATETIME,
    }:
        null_value = lit('-')
        ...
        stringify_value = True  # Convert original value to string
            ...
            DataType.INTEGER, DataType.CONST_INTEGER,
            DataType.FLOAT, DataType.CONST_FLOAT,
            DataType.BOOLEAN, DataType.CONST_BOOLEAN,
    }:
        null_value = lit(0)
...
def _denullified_eq(
        left_ctx: TranslationCtx, right_ctx: TranslationCtx, translation_env: TranslationEnvironment,
) -> ClauseElement:
...
DEFINITIONS_BINARY = [
    ...
    base.BinaryMultStringNonConst(variants=[
        V(D.CLICKHOUSE, lambda text, size: sa.func.arrayStringConcat(
    ...
    base.BinaryMinusDateFloat(variants=[
        V(D.CLICKHOUSE, lambda date, days: sa.func.date_add(raw_sql('day'), base.as_bigint(-sa.func.ceil(days)), date)),
    ]),
    ...
    base.BinaryMinusDates(variants=[

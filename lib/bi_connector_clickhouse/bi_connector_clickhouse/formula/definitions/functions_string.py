...
DEFINITIONS_STRING = [
    ...
    base.FuncFind3(variants=[
        V(
            ...
            lambda x, y, z: n.if_(  # type: ignore  # TODO: fix
                ...
                sa.func.positionUTF8(sa.func.substringUTF8(x, z, sa.func.lengthUTF8(x)), y) + z - 1
            ).else_(0)
        ),
    ]),

    # icontains
    base.FuncIContainsConst(variants=[
        V(D.CLICKHOUSE, lambda x, y: sa.func.positionCaseInsensitiveUTF8(x, y) != 0),
    ]),
    base.FuncIContainsNonConst(variants=[
        V(D.CLICKHOUSE, lambda x, y: sa.func.positionCaseInsensitiveUTF8(x, y) != 0),
    ]),
    ...
    # ltrim
    base.FuncLtrim(variants=[
        V(D.CLICKHOUSE, sa.func.trimLeft),
    ]),

    # regexp_extract
    base.FuncRegexpExtract(variants=[
        V(D.CLICKHOUSE, lambda text, pattern: sa.func.arrayElement(sa.func.extractAll(
            sa.func.assumeNotNull(text), pattern
        ), 1)),
    ]),

    # regexp_extract_nth
    base.FuncRegexpExtractNth(variants=[
        V(D.CLICKHOUSE, lambda text, pattern, ind: sa.func.arrayElement(sa.func.extractAll(
            sa.func.assumeNotNull(text), pattern
        ), ind)),
    ]),
    ...
    base.FuncRight(variants=[
        V(D.CLICKHOUSE,
          lambda x, y: sa.func.substringUTF8(x, sa.func.lengthUTF8(x) - y + 1, y)),
    ]),

    # rtrim
    base.FuncRtrim(variants=[
    ...
    base.FuncSpaceNonConst(variants=[
        V(D.CLICKHOUSE, lambda size: sa.func.arrayStringConcat(
            sa.func.arrayResize(sa.func.emptyArrayString(), size, ' '))),
    ]),
    ...
    base.FuncSplit3(variants=[
        V(
            D.CLICKHOUSE,
            lambda text, delim, ind: sa.func.arrayElement(sa.func.splitByString(
                delim, sa.func.assumeNotNull(text)), ind
            )
        ),
    ]),
    ...
    base.FuncSubstr2(variants=[
        V(D.CLICKHOUSE, lambda text, start: sa.func.substringUTF8(
            text, start, sa.func.lengthUTF8(text))),
    ]),
    base.FuncSubstr3(variants=[
        V(D.CLICKHOUSE,
          lambda text, start, length: sa.func.substringUTF8(text, start, length)),
    ]),

    # trim
    base.FuncTrim(variants=[

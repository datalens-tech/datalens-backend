...
from bi_testing.s3_utils import s3_file_exists
...
async def saved_gsheets_v2_connection(loop, bi_context, default_async_usm_per_test, s3_persistent_bucket, s3_client):
    ...
    data = GSheetsFileS3Connection.DataModel(sources=[
    ...
    for src in itertools.chain(conn.data.sources, updated_conn.data.sources):  # cleanup original and updated files if any
...
async def test_update_connection_data_with_save(
    ...
    src_5 = updated_conn.get_file_source_by_id(conn.data.sources[4].id)  # this is a valid source, but it failed during the previous update
...
async def test_update_in_progress_sources(
    ...
    resp = await fu_client.make_request(ReqBuilder.update_conn_data(
        conn,
        save=False,
        require_ok=False,
    ))
    ...
    assert resp.json['details'] == {
        'incomplete_sources': [
            {'source_id': conn.data.sources[0].id, 'title': conn.data.sources[0].title}
        ]
    }

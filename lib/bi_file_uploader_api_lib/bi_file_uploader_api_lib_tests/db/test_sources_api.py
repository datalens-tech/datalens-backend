...
async def test_source_info(fu_client, uploaded_file_id: str):
    ...
    resp1 = await fu_client.make_request(ReqBuilder.source_info(
        file_id=file_id,
        source_id=source_id,
        data_json={
            'column_types': [
...
async def test_apply_settings(fu_client, redis_model_manager, uploaded_file_id: str):
    ...
    resp_info = await fu_client.make_request(ReqBuilder.source_info(
        file_id=file_id,
        source_id=source_id,
    ))
    previous_title = resp_info.json['source']['title']

    resp = await fu_client.make_request(ReqBuilder.apply_source_settings(
        file_id=file_id,
        source_id=source_id,
        data_json={
            'data_settings': {
...
async def test_source_preview(fu_client, master_token_header, uploaded_file_id: str, redis_model_manager):
    ...
    resp = await fu_client.make_request(ReqBuilder.preview(
        file_id, source_id, master_token_header,
        data_json={
...
async def test_excel_source(
    ...
    assert resp.json['preview'][0] == [
        '1', 'MX-2014-143658', '2014-10-02T00:00:00', '2014-10-06T00:00:00', 'Standard Class', 'SC-20575',
        'Sonia Cooley', 'Consumer', 'Mexico City', 'Distrito Federal', 'Mexico', None, 'LATAM', 'North',
        'OFF-LA-10002782', 'Office Supplies', 'Labels', 'Hon File Folder Labels, Adjustable', '13.08', '3', '0.0',
...
async def test_source_raw_schema(fu_client, master_token_header, uploaded_file_id: str):
    ...
    resp = await fu_client.make_request(ReqBuilder.internal_params(
        file_id, source_id, master_token_header,
        data_json={
            'raw_schema': [
                {"user_type": "string", "name": "f3"},

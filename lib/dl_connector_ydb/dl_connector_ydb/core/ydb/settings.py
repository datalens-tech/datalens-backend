...
class YDBConnectorSettings(ConnectorSettingsBase):
    HAS_MANY_AUTH_TYPES: Optional[bool] = s_attrib("HAS_MANY_AUTH_TYPES", missing=False)  # type: ignore  # 2024-01-30 # TODO: Incompatible types in assignment (expression has type "Attribute[Any]", variable has type "bool | None")  [assignment]
...
class ConnectorsDataYDBBase(ConnectorsDataBase):
    HAS_MANY_AUTH_TYPES: ClassVar[Optional[bool]] = False
...
def ydb_settings_fallback(full_cfg: ConnectorsConfigType) -> dict[str, ConnectorSettingsBase]:
    ...
    if cfg is None:
        return {}
    return dict(YDB=YDBConnectorSettings(HAS_MANY_AUTH_TYPES=cfg.HAS_MANY_AUTH_TYPES))  # type: ignore  # 2024-01-30 # TODO: Unexpected keyword argument "HAS_MANY_AUTH_TYPES" for "YDBConnectorSettings"  [call-arg]

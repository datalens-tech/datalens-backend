...
CATEGORY_AGGREGATION = FunctionDocCategory(
    ...
    description=_(
        ...
        "\n"
        "Aggregate functions can be used with constants, such as `COUNT(1)` or `SUM(1)`. "
        "If the chart does not use other measures and dimensions, the result of such an "
        "expression will always be `1`. This is because the function does not include any "
        "fields, so {{ datalens-short-name }} accesses no source tables in the query.\n"
        "\n"
        "{text:agg_syntax}\n"

...
def _gen_m_expr_distinct_via_group_by(eng):
    expr = sqlalchemy.select(
        columns=[sqlalchemy.column('ym:pv:browserName').label('browser')])
    expr = expr.select_from(sqlalchemy.Table(METRIKA_SAMPLE_COUNTER_ID, sqlalchemy.MetaData(bind=eng)))
    expr = expr.group_by(sqlalchemy.column('browser'))
    return expr
...
def _gen_m_expr_date_min(eng):
    expr = sqlalchemy.select(
        columns=[sqlalchemy.column('ym:pv:date').label('date')])
    expr = expr.select_from(sqlalchemy.Table(METRIKA_SAMPLE_COUNTER_ID, sqlalchemy.MetaData(bind=eng)))
    expr = expr.group_by(sqlalchemy.column('date'))
    expr = expr.limit(1)
    ...
    expr = sqlalchemy.select(
        columns=[
            ...
            sqlalchemy.type_coerce(
                sqlalchemy.type_coerce(
                    sqlalchemy.column('ym:pv:users'),
                    sqlalchemy.Integer),
                sqlalchemy.Integer).label('users_int'),
        ])
    expr = expr.select_from(sqlalchemy.Table(METRIKA_SAMPLE_COUNTER_ID, sqlalchemy.MetaData(bind=eng)))
    return expr
...
def _gen_m_expr_date_between(eng):
    expr = sqlalchemy.select(
        columns=[sqlalchemy.column('ym:pv:pageviews'),
                 sqlalchemy.column('ym:pv:users')])
    expr = expr.select_from(sqlalchemy.Table(METRIKA_SAMPLE_COUNTER_ID, sqlalchemy.MetaData(bind=eng)))
...
def _gen_m_expr_datetime_between_datetime(eng):
    expr = sqlalchemy.select(
        columns=[sqlalchemy.column('ym:pv:pageviews'),
                 sqlalchemy.column('ym:pv:users')])
    expr = expr.select_from(sqlalchemy.Table(METRIKA_SAMPLE_COUNTER_ID, sqlalchemy.MetaData(bind=eng)))
    expr = expr.where(sqlalchemy.column('ym:pv:dateTime').between(
...
def _gen_am_expr1(eng):
    expr = sqlalchemy.select(
        columns=[sqlalchemy.column('ym:u:sessions'),
                 sqlalchemy.column('ym:u:activeUsers')])
...
def m_expr_distinct(metrika_sample_counter_id, metrika_db_engine):
    ...
    expr = expr.select_from(
        sqlalchemy.Table(metrika_sample_counter_id, sqlalchemy.MetaData(bind=metrika_db_engine))
    )
    return expr

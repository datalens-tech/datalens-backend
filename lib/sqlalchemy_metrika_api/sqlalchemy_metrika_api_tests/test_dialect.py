...
def test_future_date(metrika_expr_select_date_users, metrika_db_engine):
    ...
    expr = metrika_expr_select_date_users.where(sa.column('ym:pv:date').between(
...
def test_calculations(metrika_sample_counter_id, metrika_db_engine):
    expr = sa.select(
        columns=[
            ...
            (
                sa.column('ym:pv:pageviews') * sa.column('ym:pv:users') - sa.column('ym:pv:pageviewsPerDay') + 14
            ).label('some_strange_expr'),
        ],
    )
    expr = expr.select_from(
        sa.Table(metrika_sample_counter_id, sa.MetaData(bind=metrika_db_engine))
    )
    ...
    assert all([
        ...
        round(row['some_strange_expr'], 2) for row in data
    ])

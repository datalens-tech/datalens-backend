...
class MetrikaApiClient(object):
    ...
    def _request(self, method: str, uri: str, timeout: int = -1, _raw_resp: bool = False, **kwargs):
        ...
        LOGGER.info(
            ...
            method, full_url, kwargs.get('params', {}), kwargs.get('json', {})
        )
        ...
        try:
            ...
            LOGGER.info(
                ...
                response.status_code, method, uri, response.headers.get('Content-Length'),
            )
            if response.status_code >= 400:
                LOGGER.error(
                    'Metrika API error on %s %s (%s): %s',
                    method, uri, kwargs.get('json', {}), response.text,
                )
    ...
    def _parse_data_resp(self, resp, result_columns=None, req_metrics=None):
        ...
        """
        LOGGER.info(
            'Received data response: total_rows: %s, sample_share: %s',
            resp['total_rows'], resp['sample_share'])
        ...
        try:
            ...
            if not result_columns:
                result_columns = [
                    dict(name=col_name, label=None) for col_name in (*q_dims, *q_metrics)
                ]

            for slice in resp['data']:
                # TODO: date dimensions values better retrieve from 'id' key instead of 'name'?
                row_map = {
                    q_dims[i]: dim_item[dims_src_keys[i]] for i, dim_item in enumerate(slice['dimensions'])
                }
                row_map.update(
                    {q_metrics[i]: metr_val for i, metr_val in enumerate(slice['metrics'])}
                )
                rows.append(tuple(
                    col['cast_processor'](row_map[col['name']]) if 'cast_processor' in col
                    else row_map[col['name']]
    ...
    def get_available_counters(self, **kwargs) -> List[dict]:
        ...
        return [
            dict(id=c_info['id'], name=c_info['name'])
            for c_info in resp[obj_name]
        ]
    ...
    def get_counter_info(self, counter_id):
        ...
        uri = '/management/v1/{obj_name}/{counter_id}'.format(
            obj_name=obj_name, counter_id=counter_id)
        resp = self.get(uri)

...
from bi_sqlalchemy_clickhouse.base import BIClickHouseDialect as UPSTREAM

import bi_sqlalchemy_chyt.types as chyt_types
...
class CHYTTablesConcat(CHYTTableExpression):
    def __init__(self, *tables, **kwargs):
        text = 'concatYtTables({})'.format(', '.join(
...
class CHYTTablesRange(CHYTTableExpression):
    def __init__(self, directory, start=None, end=None, **kwargs):
        ...
        text = 'concatYtTablesRange({})'.format(', '.join(
            self._quote_value(arg)
            for arg in args))
...
def register_dialect():
    sa.dialects.registry.register("bi_chyt", "bi_sqlalchemy_chyt.base", "BICHYTDialect")

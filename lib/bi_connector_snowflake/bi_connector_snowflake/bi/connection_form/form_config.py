...
from bi_api_connector.form_config.models.common import CommonFieldName, FormFieldName, remap_skip_if_null, MarkdownStr
from bi_api_connector.form_config.models.rows.base import DisplayConditionsMixin, TDisplayConditions, FormRow
...
class SnowFlakeConnectionFormFactory(ConnectionFormFactory):
    def get_form_config(
        ...
        help_account_name = help_text_sf_docs.format(LINK=f'{sf_docs_url}/user-guide/client-redirect.html#snowsight-the-snowflake-web-interface')
        help_client = help_text_sf_docs.format(LINK=f'{sf_docs_url}/sql-reference/functions/system_show_oauth_client_secrets.html#system-show-oauth-client-secrets')
        ...
        help_warehouse = help_text_sf_docs.format(LINK=f'{sf_docs_url}/sql-reference/sql/create-warehouse.html#create-warehouse')
        ...
        auth_section: list[FormRow] = [
            ...
            OAuthSnowFlakeRow(
                button_text=None if self.mode == ConnectionFormMode.create else self._localizer.translate(Translatable('button_get-new-token')),
                display_conditions=on_auth_opened,
                fake_value='******' if self.mode == ConnectionFormMode.edit else None,
            ),

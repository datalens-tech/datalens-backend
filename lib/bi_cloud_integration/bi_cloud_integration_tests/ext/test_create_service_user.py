...
def test_list_svc_acct_role_ids(iam_rm_client, svc_acct_per_test, cloud_integration_stand_folder_id):
    ...
    roles = iam_rm_client.list_svc_acct_role_ids_on_folder_sync(
        svc_acct_id=svc_acct.id, folder_id=folder_id, _page_size=1
    )
...
def test_grant_role_on_sa(
    ...
    as_client = DLASClient(
        service_config=DLYCServiceConfig(endpoint=ext_sys_helpers_per_test.ext_sys_requisites.YC_AS_ENDPOINT)
    )
...
def test_list_robot_keys(
    ...
    for i in range(3):
        created_key_data_list.append(ctrl.create_svc_acct_key_sync(svc_acct.id, f"test key {i}").svc_acct_key_data)

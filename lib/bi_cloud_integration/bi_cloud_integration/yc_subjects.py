...
import attr
from yandex.cloud.priv.iam.v1.console import (
    membership_service_pb2,
    membership_service_pb2_grpc,
)
from yandex.cloud.priv.oauth import claims_pb2
from yandex.cloud.priv.oauth.v1 import (
    claim_service_pb2,
    claim_service_pb2_grpc,
)
...
class DLYCMSClient(DLYCSingleServiceClient):
    ...
    def make_members_filter(cls, search_text: str, default_type_filter: bool = True) -> str:
        ...
        if default_type_filter:
            query = cls.make_filter_and(query, cls.DEFAULT_SUBJECT_TYPE_FILTER)
        return query

    @classmethod
    def make_email_queries_generator(cls, emails: Iterable[str]) -> Iterable[str]:
        return join_in_chunks(
            (f"(claims.email ICONTAINS {cls.quote_filter_string(email)})" for email in sorted(emails)),
            sep=cls.FILTER_OR,
    ...
    async def list_members(
        ...
        for page in range(max_pages):
            ...
            subject_datas = [subject_claims_to_info(subj.subject_claims) for subj in resp.members]

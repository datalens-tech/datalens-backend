...
import json
import attr
from typing import List, AsyncGenerator, Dict, Optional, Iterable
import pkgutil
...
def dls_subjects_mock(app, dls_subjects_data, us_host):
    def subjects_callback(request):
        ...
        response = {'results': {
            subject_name: dls_subjects_data[subject_name]
...
def check_text_config_to_rls_entries(case, subject_resolver):
    ...
    for exp_entry in expected_rls_entries:
        gotten_entries = [
            ...
            if entry.pattern_type == exp_entry.pattern_type and
            entry.allowed_value == exp_entry.allowed_value and
            entry.subject.subject_id == exp_entry.subject.subject_id and
            entry.subject.subject_type.name == exp_entry.subject.subject_type.name and
            entry.subject.subject_name == exp_entry.subject.subject_name
        ]
...
def test_create_and_update_rls(app, client, dataset_id, case, dls_subjects_mock):
    ...
    assert response.status_code == 200
    assert config_to_comparable(response.json['dataset']['rls'][field_guid]) \
           == config_to_comparable(case['config_to_compare'])
...
async def test_filter_expr(dataset_id, default_async_usm_per_test: AsyncUSManager):
    ...
    rls_entries = (
        [RLSEntry(field_guid=field_a, subject=subject, allowed_value=value)
         for value in field_a_allowed_values] +
        [RLSEntry(field_guid=field_b, subject=subject, allowed_value=value)
         for value in field_b_allowed_values] +
        [RLSEntry(
            field_guid=field_b,
            subject=RLSSubject(subject_id='', subject_name='userid', subject_type=RLSSubjectType.userid),
            pattern_type=RLSPatternType.userid, allowed_value=None)]
    )

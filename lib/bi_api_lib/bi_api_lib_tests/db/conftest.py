...
from bi_api_lib_tests.conftest import (  # noqa
    DB_PARAMS, _db_for, db_for_key, ch_connection_params, create_ch_connection, loaded_libraries
)
from bi_api_lib_tests.utils import get_random_str, make_connection_get_id
...
def pg_fresh_connection_id(app, client, request, pg_fresh_connection_params):
    return make_connection_get_id(
        connection_params=pg_fresh_connection_params,
        client=client, request=request)
...
def promql_subselectable_connection_id(app, client, request, promql_connection_params):
    return make_connection_get_id(
        connection_params=promql_connection_params,
        client=client, request=request)

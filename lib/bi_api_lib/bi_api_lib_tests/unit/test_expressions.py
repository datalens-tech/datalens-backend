...
class CompilerTestBase:
    ...
    def _make_fields(self):
        return [
            self._make_field(col.name, type=FieldType.DIMENSION)
        ...
        compiled_multi_query = CompiledMultiLevelQuery(
            levels=[
                CompiledLevel(
                    queries=[
                        CompiledQuery(
                            id='1', joined_from=joined_from,
                            select=[comp_formula],
                            group_by=[], order_by=[], filters=[], join_on=[],
    ...
    def make_compiled_exprs(self):
        return {
            title: self.compile(expr)
            for title, expr in self.make_expressions().items()}
...
class TestGenerateExpressions(CompilerTestBase):
    ...
    def test_generate_expressions(self, prepared):
        ...
        assert exprs['f2'] in (
            f'"{self.avatar_sid}".f1 + 1', f'("{self.avatar_sid}".f1 + 1)'
        )  # legacy version adds parentheses
        assert exprs['f3'] in (
            f'avg("{self.avatar_sid}".f1 + 1)', f'avg(("{self.avatar_sid}".f1 + 1))'
        )
        assert exprs['f4'] in (
            f'sum("{self.avatar_sid}".f1)', f'(sum("{self.avatar_sid}".f1))'
        )
...
class TestGenerateFilterExpressionIntCol(CompilerTestBase):
    ...
    def test_generate_filter_expression(self, prepared):
        ...
        # unary
        assert self.compile_filter_expression(
            f1.guid, WhereClauseOperation.ISNULL, [],
        ) == f'isNull("{self.avatar_sid}".f1)'
        assert self.compile_filter_expression(
            f1.guid, WhereClauseOperation.ISNOTNULL, [],
        ) == f'NOT isNull("{self.avatar_sid}".f1)'

        # binary
        assert self.compile_filter_expression(
            f1.guid, WhereClauseOperation.EQ, [12],
        ) == f'"{self.avatar_sid}".f1 = 12'
        assert self.compile_filter_expression(
            f1.guid, WhereClauseOperation.NE, [12],
        ) == f'"{self.avatar_sid}".f1 != 12'
        assert self.compile_filter_expression(
            f1.guid, WhereClauseOperation.LT, [12],
        ) == f'"{self.avatar_sid}".f1 < 12'
        assert self.compile_filter_expression(
            f1.guid, WhereClauseOperation.LTE, [12],
        ) == f'"{self.avatar_sid}".f1 <= 12'
        assert self.compile_filter_expression(
            f1.guid, WhereClauseOperation.GT, [12],
        ) == f'"{self.avatar_sid}".f1 > 12'

        # binary with list
        assert self.compile_filter_expression(
            f1.guid, WhereClauseOperation.IN, [12, 34, 56],
        ) == f'"{self.avatar_sid}".f1 IN (12, 34, 56)'
        assert self.compile_filter_expression(
            f1.guid, WhereClauseOperation.NIN, [12, 34, 56],
        ) == f'("{self.avatar_sid}".f1 NOT IN (12, 34, 56)) OR "{self.avatar_sid}".f1 IS NULL'
        assert self.compile_filter_expression(
            f1.guid, WhereClauseOperation.IN, [12, 34, 56, None],
        ) == f'"{self.avatar_sid}".f1 IS NULL OR "{self.avatar_sid}".f1 IN (12, 34, 56)'
        assert self.compile_filter_expression(
            f1.guid, WhereClauseOperation.NIN, [12, 34, 56, None],
        ) == f'("{self.avatar_sid}".f1 NOT IN (12, 34, 56)) AND "{self.avatar_sid}".f1 IS NOT NULL'
        assert self.compile_filter_expression(
            f1.guid, WhereClauseOperation.IN, [None],
        ) == f'"{self.avatar_sid}".f1 IS NULL'
        assert self.compile_filter_expression(
            f1.guid, WhereClauseOperation.NIN, [None],
        ) == f'"{self.avatar_sid}".f1 IS NOT NULL'

        # ternary
        assert self.compile_filter_expression(
            f1.guid, WhereClauseOperation.BETWEEN, [12, 34],
...
class TestGenerateFilterExpressionStrCol(CompilerTestBase):
    ...
    def test_generate_filter_expression(self, prepared):
        ...
        assert self.compile_filter_expression(
            f3.guid, WhereClauseOperation.STARTSWITH, ['qwe'],
        ) == f"startsWith(\"{self.avatar_sid}\".f3, 'qwe')"
        assert self.compile_filter_expression(
            f3.guid, WhereClauseOperation.ISTARTSWITH, ['qwe'],
        ) == f"startsWith(lowerUTF8(\"{self.avatar_sid}\".f3), 'qwe')"
        assert self.compile_filter_expression(
            f3.guid, WhereClauseOperation.ENDSWITH, ['qwe'],
        ) == f"endsWith(\"{self.avatar_sid}\".f3, 'qwe')"
        assert self.compile_filter_expression(
            f3.guid, WhereClauseOperation.IENDSWITH, ['qwe'],
        ) == f"endsWith(lowerUTF8(\"{self.avatar_sid}\".f3), 'qwe')"
        assert self.compile_filter_expression(
            f3.guid, WhereClauseOperation.CONTAINS, ['qwe'],
        ) == f"\"{self.avatar_sid}\".f3 LIKE '%%qwe%%'"
        assert self.compile_filter_expression(
            f3.guid, WhereClauseOperation.ICONTAINS, ['qwe'],
        ) == f"positionCaseInsensitiveUTF8(\"{self.avatar_sid}\".f3, 'qwe') != 0"
        assert self.compile_filter_expression(
            f3.guid, WhereClauseOperation.NOTCONTAINS, ['qwe'],
        ) == f"\"{self.avatar_sid}\".f3 NOT LIKE '%%qwe%%'"
        assert self.compile_filter_expression(
            f3.guid, WhereClauseOperation.NOTICONTAINS, ['qwe'],
        ) == f"positionCaseInsensitiveUTF8(\"{self.avatar_sid}\".f3, 'qwe') = 0"
        ...
        assert self.compile_filter_expression(
            f4.guid, WhereClauseOperation.STARTSWITH, ['123'],
        ) == f"startsWith(toString(\"{self.avatar_sid}\".f4), '123')"
        assert self.compile_filter_expression(
            f4.guid, WhereClauseOperation.ISTARTSWITH, ['123'],
        ) == f"startsWith(lowerUTF8(toString(\"{self.avatar_sid}\".f4)), '123')"
        assert self.compile_filter_expression(
            f4.guid, WhereClauseOperation.ENDSWITH, ['123'],
        ) == f"endsWith(toString(\"{self.avatar_sid}\".f4), '123')"
        assert self.compile_filter_expression(
            f4.guid, WhereClauseOperation.IENDSWITH, ['123'],
        ) == f"endsWith(lowerUTF8(toString(\"{self.avatar_sid}\".f4)), '123')"
        assert self.compile_filter_expression(
            f4.guid, WhereClauseOperation.CONTAINS, ['123'],
        ) == f"toString(\"{self.avatar_sid}\".f4) LIKE '%%123%%'"
        assert self.compile_filter_expression(
            f4.guid, WhereClauseOperation.ICONTAINS, ['123'],
        ) == f"positionCaseInsensitiveUTF8(toString(\"{self.avatar_sid}\".f4), '123') != 0"
        assert self.compile_filter_expression(
            f4.guid, WhereClauseOperation.NOTCONTAINS, ['123'],
        ) == f"toString(\"{self.avatar_sid}\".f4) NOT LIKE '%%123%%'"
        assert self.compile_filter_expression(
            f4.guid, WhereClauseOperation.NOTICONTAINS, ['123'],
        ) == f"positionCaseInsensitiveUTF8(toString(\"{self.avatar_sid}\".f4), '123') = 0"
...
class TestGenerateFilterExpressionDateCol(CompilerTestBase):
    ...
    def test_generate_filter_expression(self, prepared):
        ...
        expr = self.compile_filter_expression(
            f4.guid, WhereClauseOperation.BETWEEN,
            ['2017-01-01T00:00:00', '2017-01-01T23:59:59'])
...
class TestGenerateFilterExpressionDatetimeCol(CompilerTestBase):
    ...
    def test_generate_filter_expression(self, prepared):
        ...
        expr = self.compile_filter_expression(
            f4.guid, WhereClauseOperation.BETWEEN,
            ['2017-01-01T00:00:00Z', '2017-01-01T23:59:59Z'])
        expected = f'''"{self.avatar_sid}".f4 BETWEEN toDateTime('2017-01-01T00:00:00') AND ''' \
...
class TwoSourcesCompilerTestBase(CompilerTestBase):
    ...
    def _make_columns(self):
        return (
            list(super()._make_columns()) +
            list(self._make_second_source_columns()))

    def _make_avatar_source_map(self):
        return {
            **super()._make_avatar_source_map(),
            self.second_avatar_id: self.second_source_id}
...
class TestGenerateRelationExpression(TwoSourcesCompilerTestBase):
    ...
    def _make_fields(self):
        return [
            ...
            self._make_field(
                'f1_1', source='f1',
                avatar_id=self.second_avatar_id, type=FieldType.DIMENSION),
            self._make_field(
                'f3',
                avatar_id=self.second_avatar_id, type=FieldType.DIMENSION),
        ]
...
class TestDoubleAggregation(CompilerTestBase):
    ...
    def test_double_aggregation(self, prepared):
        ...
        exprs = {
            title: self.compile(expr_info) if expr_info is not None else None
            for title, expr_info in expr_infos.items()}

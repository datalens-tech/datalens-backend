...
class MarkupProcessingBase(Generic[_NODE_TV]):
    ...
    def _dump_i(self, node: NodeActual) -> str:
        ...
        return (
            self.lpar + funcname + self.sep +
            self.sep.join(
                # recurse
                self._dump_i(arg) for arg in funcargs) +
            self.rpar)
            ...
            while True:
                ...
                if self._dbg:
                    self._dbg_print(
                        ...
                        data[:pos], pos, data[pos:next_quot], next_quot, data[next_quot:])
    ...
    def _verbalize_i(self, node: NodeActual) -> Any:
        ...
        if funcname == self.node_concat:
            return dict(
                type='concat',
                children=[self._verbalize_i(arg) for arg in funcargs])
        if funcname == self.node_url:
            if len(funcargs) != 2:
                return self._argcount_mismatch(
                    node=node, funcname=funcname, funcargs=funcargs,
                    expected_args=2)
            url, content = funcargs
            return dict(
                type='url',
                url=url,
                content=self._verbalize_i(content))

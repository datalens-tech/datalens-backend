...
import re
from typing import (
    AbstractSet, Any, Collection, Dict, FrozenSet, Iterator, List,
    NamedTuple, Optional, Set, Tuple, TYPE_CHECKING, cast,
)
...
from bi_query_processing.compilation.primitives import (
    CompiledJoinOnFormulaInfo, JoinedFromObject,
    FromColumn, FromObject, SubqueryFromObject,
)
from bi_query_processing.legacy_pipeline.separation.primitives import (
    CompiledMultiLevelQuery, MultiQueryIndex,
)
from bi_query_processing.legacy_pipeline.subqueries.sanitizer import MultiQuerySanitizer
from bi_query_processing.legacy_pipeline.subqueries.grouping_normalizer import GroupByNormalizer
from bi_query_processing.legacy_pipeline.subqueries.query_tools import (
    remap_formula_obj_fields, copy_and_remap_query,
    CompiledMultiLevelQueryIncrementalPatch, apply_multi_query_incremental_patches,
    CompiledMultiLevelQueryReplacementPatch, apply_multi_query_replacement_patches,
    add_dummy_select_column, get_query_level_used_fields,
)

if TYPE_CHECKING:
    from bi_query_processing.compilation.primitives import (
        CompiledFormulaInfo, CompiledOrderByFormulaInfo, CompiledQuery,
    )
...
class QueryForker:
    ...
    def _validate_dimensions_and_extract_field_names(
        ...
        for dim in dimensions:
            if isinstance(dim, formula_nodes.Field):
                ...
                raise TypeError(
                    f'Expected to see Field node as dimension, '
                    f'but got {type(dim).__name__}'
                )
    ...
    def _collect_ancestors_by_level(
        ...
        for sub_level_idx in reversed(range(subquery_idx.level_idx)):
            all_parent_ids = set(
                from_id
                for child in upper_level_child_queries
                for from_id in child.joined_from.iter_ids()
            )

            # Generate list of queries to be used as children for the next level
            parents = [
                query for query in compiled_multi_query.levels[sub_level_idx].queries
                if query.id in all_parent_ids
            ]
        ...
        for sub_level_idx in range(level_idx):
            ...
            for parent in parents_by_level[sub_level_idx]:
                ...
                cloned_parent, remapped_parent_aliases = copy_and_remap_query(
                    parent, id=new_parent_id, field_name_map=old_all_remapped_parent_aliases,
    ...
    def _fork_subquery(
        ...
        stripped_query_select: List[CompiledFormulaInfo] = [
            expr for expr in original_subquery.select
            if expr.alias in required_select_aliases
        ]
        ...
        stripped_query_filters = [
            expr for expr in original_subquery.filters
            if expr.original_field_id not in bfb_field_ids
        ]
    ...
    def _make_fork_join_on_expression(
        ...
        if isinstance(joining_node, formula_fork_nodes.QueryForkJoiningWithList):
            ...
            for condition in joining_node.condition_list:
                ...
                fork_expr = remap_formula_obj_fields(
                    node=fork_expr, field_name_map=remapped_forked_aliases)
    ...
    def _are_subquery_dimensions_compatible(
        ...
        dimension_sets = [
            subquery_info_key.dim_extracts
            for subquery_info_key in subquery_infos
        ]
    ...
    def _get_child_dimension_sets(
        ...
        for field_node in used_field_nodes:
            ...
            for child_qfork_idx, child_qfork in field_child_expr.formula_obj.enumerate():
    ...
    def _collect_query_fork_infos(
            self, expr_lists: Tuple[List[CompiledFormulaInfo], ...],
            alias_to_subquery_id_map: Dict[str, AvatarId],
            compiled_multi_query: CompiledMultiLevelQuery,
        ...
        def _get_lod_idx_for_query_fork(qfork_node: formula_fork_nodes.QueryFork) -> int:
            ...
            child_dimension_sets = self._get_child_dimension_sets(
                node=qfork_node, alias_to_subquery_id_map=alias_to_subquery_id_map,
                ...
                for node_idx, node in expr.formula_obj.enumerate():
                    if isinstance(node, formula_fork_nodes.QueryFork):
                        ...
                        required_select_aliases = {
                            used_fld.name
                            for expr in significant_expressions
                            for used_fld in used_fields(expr)
                        }
                        required_subquery_ids = {
                            alias_to_subquery_id_map[alias] for alias in required_select_aliases
                        }
                        ...
                        if isinstance(lod, formula_nodes.FixedLodSpecifier):
                            ...
                            dim_list = tuple(
                                formula_nodes.Field.make(str_not_none(fla.alias))
                                for fla in original_subquery.group_by
                            )
                        else:
    ...
    def _scan_subquery_and_make_forks(
            ...
            CompiledQuery,
            List[CompiledMultiLevelQueryIncrementalPatch],
            List[CompiledMultiLevelQueryReplacementPatch]
    ]:
        ...
        subquery_infos_by_joining = self._collect_query_fork_infos(
            expr_lists=expr_lists, alias_to_subquery_id_map=alias_to_subquery_id_map,
            compiled_multi_query=compiled_multi_query,
        )
        ...
        for subquery_info_key, subquery_info in subquery_infos_by_joining.items():
                ...
                if (
                    ...
                    if dummy_col_alias not in select_aliases:
                        ...
                        new_repl_patch = CompiledMultiLevelQueryReplacementPatch(
                            subquery_idx=subquery_idx, new_subquery=original_subquery,
                        )
                        ...
                        compiled_multi_query = apply_multi_query_replacement_patches(
                            compiled_multi_query, patches=[new_repl_patch],
                        )
                        replacement_patches.append(new_repl_patch)

                    joining_node = formula_fork_nodes.QueryForkJoiningWithList.make(
                        condition_list=[formula_fork_nodes.SelfEqualityJoinCondition.make(
        ...
        updated_compiled_flat_query = compiled_flat_query.clone(
            select=select, group_by=group_by, order_by=order_by, filters=filters, join_on=join_on,
    ...
    def scan_and_fork_multi_query(
        ...
        for level_idx in reversed(range(len(compiled_multi_query.levels))):
            ...
            compiled_level = sanitizer.sanitize_compiled_level(
                compiled_level=compiled_level, used_aliases=upper_level_used_aliases,

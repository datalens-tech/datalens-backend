...
from bi_query_processing.compilation.primitives import (
    CompiledFormulaInfo, CompiledOrderByFormulaInfo, CompiledJoinOnFormulaInfo, CompiledQuery,
    FromColumn, JoinedFromObject, SubqueryFromObject,
)
from bi_query_processing.legacy_pipeline.slicing.primitives import (
    SlicedQuery, SlicedFormula, SlicedOrderByFormula, SlicedJoinOnFormula,
)
from bi_query_processing.legacy_pipeline.separation.primitives import CompiledLevel, CompiledMultiLevelQuery
...
class QuerySeparator:
    ...
    def _make_query_id(
            self, top_id: AvatarId, level_idx: int, query_idx: int,
            is_top_level: bool, iteration_id: str = '',
    ) -> AvatarId:
    ...
    def _restore_original_order_for_top_level_expressions(
        ...
        for sliced_formula in sliced_expression_list:
            ...
            assert len(top_level_parts_in_sliced_formula) == 1,\
    ...
    def separate_query(self, sliced_query: SlicedQuery, iteration_id: str) -> CompiledMultiLevelQuery:
        ...
        for level_idx, level_type in enumerate(sliced_query.level_plan.level_types):
            ...
            query_id = self._make_query_id(
                top_id=sliced_query.id, level_idx=level_idx, query_idx=query_idx,
                is_top_level=is_top_level, iteration_id=iteration_id
            )

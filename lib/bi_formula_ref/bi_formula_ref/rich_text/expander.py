...
from bi_formula_ref.texts import (
    DialectStyle, HUMAN_DIALECTS,
)
from bi_formula_ref.rich_text.macro import (
    BaseMacro, RefMacro, LinkMacro, TableMacro, TextMacro,
    DialectsMacro, TypeMacro, ArgMacro, ArgNMacro, ExtMacroMacro, CategoryMacro,
    SINGLE_ARG_MACROS, DOUBLE_ARG_MACROS, LIST_MACROS,
)
from bi_formula_ref.rich_text.elements import (
    MacroReplacementKey, BaseTextElement, RichText,
    ConditionalBlock, NoteBlock, AudienceBlock,
    LinkTextElement, TableTextElement, ListTextElement,
    TermTextElement, CodeSpanTextElement, ExtMacroTextElement,
)
...
_MACRO_CHOICES = '|'.join((
    'ref', 'link', 'text', 'table', 'dialects', 'arg', 'argn', 'type', 'macro', 'category'
))
# 3 kinds of macros:
#     {type: single_value}
...
_BLOCK_MACRO_CHOICES = '|'.join((
    'if', 'note', 'audience', 'end',
))
END_TAG = 'end'
BLOCK_TAG_RE = re.compile(
    r'\{\s*(?P<tag_name>'+_BLOCK_MACRO_CHOICES+r')(\s+(?P<block_param>[^}]+))?\s*\}'
)
...
class MacroExpander:
    ...
    def _make_block_macro(
            self, tag_name: str, block_param: Optional[str], block_text: str,
    ...
    def _get_block_replacements(self, text: str) -> Dict[MacroReplacementKey, BaseTextElement]:
        ...
        while block_match is not None and block_match.group('tag_name') != END_TAG:
            ...
            nested_macro_replacements = self._get_macro_replacements(
                nested_text, nested_block_replacements
            )

            block_replacement = self._make_block_macro(
                tag_name=tag_name, block_param=block_param, block_text=nested_text,
                nested_replacements={
    ...
    def expand_macro_text(self, macro: TextMacro) -> RichText:
        return self.expand_text(
            self._get_raw_text_body_by_alias(macro.arg)
        )
    ...
    def expand_macro_dialects(self, macro: DialectsMacro) -> ListTextElement:
        ...
        for dialect_name in macro.values:
            ...
            human_dialect = HUMAN_DIALECTS[dialect].for_style(
                DialectStyle.split_version
            )
    ...
    def expand_macro_type(self, macro: TypeMacro) -> ListTextElement:
        items: List[TermTextElement] = []
        type_list = [
            DataType[type_name.strip().upper()]
            for type_name in macro.values
        ]
        for h_type_name in get_human_data_type_list(types=type_list):
            items.append(TermTextElement(

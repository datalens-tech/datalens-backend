...
class ReadableDataStorage(ReadableDataStorageBase):
    ...
    def load(self) -> None:
        ...
        self._data = {
            key: self._load_data_table(table_data)
            for key, table_data in data_as_list
        }
...
class WritableDataStorage(WritableDataStorageBase):
    ...
    def _dump_data_table(self, data_table: DataTable) -> dict:
        return {
            'columns': [
                {'name': col.name, 'type': col.data_type.name}
    ...
    def save(self) -> None:
        data_as_list = sorted([
            [key, self._dump_data_table(data_table)]
            for key, data_table in self._data.items()
        ])

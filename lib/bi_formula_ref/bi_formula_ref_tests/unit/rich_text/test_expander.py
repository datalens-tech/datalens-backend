...
from bi_formula_ref.registry.aliased_res import (
    AliasedLinkResource, AliasedTextResource, AliasedTableResource, AliasedResourceRegistry
)
from bi_formula_ref.rich_text.elements import (
    MacroReplacementKey as MRK, RichText, TermTextElement, ListTextElement,
    ExtMacroTextElement, LinkTextElement, TableTextElement,
    CodeSpanTextElement, ConditionalBlock, NoteBlock, AudienceBlock,
)
...
def test_expand_macro_dialects():
    ...
    exp_res = RichText(text=text, replacements={
...
def test_expand_macro_type():
    ...
    exp_res = RichText(text=text, replacements={
        MRK(7, 31): ListTextElement(
            items=[
                TermTextElement(term='Integer', wrap=False),
                TermTextElement(term='String', wrap=False)],
            sep=' | ',
            wrap=True,
        ),
    })
...
def test_expand_macro_table():
    resources = AliasedResourceRegistry(resources={
        'my_table': AliasedTableResource(
            table_body=[['Some {text: nested_text} Text', 'Something']]
        ),
        'nested_text': AliasedTextResource(body='Nested'),
    })
    expander = MacroExpander(resources=resources)
        ...
        MRK(7, 24): TableTextElement(
            table_body=[
                [
                    RichText(
                        'Some {text: nested_text} Text',
                        replacements={MRK(5, 24): RichText('Nested')}
                    ),
                    RichText('Something'),
                ],
            ]
...
def test_expand_macro_ref():
    def func_link_provider(func_name: str, category_name: str) -> Tuple[str, str]:
        return (
            func_name, f'https://{func_name.lower()}{"-win" if category_name == "window" else ""}.funcs.com'
        )

    def cat_link_provider(category_name: str, anchor_name: Optional[str]) -> Tuple[str, str]:
        return (
            category_name, f'https://{category_name.lower()}.cats.com'
        )

...
from bi_core.data_processing.stream_base import DataStream, DataStreamAsync, DataSourceVS
from bi_core.data_processing.prepared_components.default_manager import DefaultPreparedComponentManager
from bi_core.data_processing.processing.operation import CalcOp, DownloadOp, JoinOp
...
class DataFetcher:
    ...
    def __attrs_post_init__(self) -> None:
        if self._selector is None:
            ...
            self._selector = sel_factory.get_dataset_selector(
                dataset=self._dataset, selector_type=self._selector_type,
    ...
    def _get_avatar_virtual_data_stream(
        ...
        prep_component_manager = DefaultPreparedComponentManager(
            dataset=self._dataset, role=role, us_entry_buffer=self._us_entry_buffer,
        )
    ...
    def get_data_stream(
        ...
        async_data_stream = await_sync(self.get_data_stream_async(
            role=role,
            bi_query=bi_query,
            row_count_hard_limit=row_count_hard_limit,
            root_avatar_id=root_avatar_id,

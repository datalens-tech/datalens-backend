...
from bi_core.base_models import EntryLocation, PathEntryLocation, ConnectionDataModelBase
...
def make_connection(
    ...
    return make_connection_base(
        us_manager=us_manager, conn_type=conn_type, conn_name=conn_name,
        data_dict=data_dict,
    )
...
def make_saved_connection(  # type: ignore  # TODO: fix
    ...
    conn = make_connection(
        us_manager=sync_usm, db=db,
        conn_type=conn_type, conn_name=conn_name,
        data_dict=data_dict
    )
    sync_usm.save(conn)
    return conn

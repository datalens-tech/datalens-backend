...
class TestCHFrozenConnection(CHFrozenConnectionTestBase, DefaultConnectorConnectionTestSuite):
    ...
    def test_crud(
        ...
        update_resp = control_api_sync_client.put(
            f"/api/v1/connections/{conn_id}", data=json.dumps(conn_data), content_type="application/json"
        )
        ...
        # DELETE
        assert (
            delete_resp := control_api_sync_client.delete(f"/api/v1/connections/{conn_id}")

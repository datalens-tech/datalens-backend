...
class TestComponentErrors(DefaultCoreTestClass):
    def test_component_errors(self, sync_us_manager, saved_dataset):
        ...
        field_id = '111111'
        erreg.add_error(id=field_id, type=ComponentType.field, message='This field needs ploughing', code=['ERR', '3'],
                        details=dict(row=10, column=8))
        erreg.add_error(id=field_id, type=ComponentType.field, message='Just an error', code=['ERR', '3', '1'],
                        details=dict(row=100, column=80))

        obligatory_filter_id = str(uuid.uuid4())
        erreg.add_error(
            id=obligatory_filter_id, type=ComponentType.obligatory_filter,
            message='obligatory_filter test err', code=['ERR', 'OBLIG_FILT', 'TEST']
        )

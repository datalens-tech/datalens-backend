...
import difflib
from typing import Sequence, Dict, List, Any, Iterable, Optional
...
class DictFieldsDiff:
    ...
    def short_str(self):  # type: ignore  # TODO: fix
        parts = [
            f"{f_name}={f_val}"
            for f_name, f_val in attr.asdict(self).items()
            if f_val
        ]
        return f"({';'.join(parts)})"
...
class EntryFieldsDiff:
    ...
    def short_str(self):  # type: ignore  # TODO: fix
        parts = [
            f"{f_name}={f_val.short_str()}"
            for f_name, f_val in attr.asdict(self, recurse=False).items()
            if f_val
        ]
...
def get_dict_top_level_diff(a: Dict[str, Any], b: Dict[str, Any]) -> DictFieldsDiff:
    ...
    return DictFieldsDiff(
        ...
        modified=tuple(sorted(
            field for field in a_fields & b_fields
            if a[field] != b[field]
        ))
    )
...
def make_diff(  # type: ignore  # TODO: fix
        value_a, value_b,
        unified_n: int = 1, dumper=lambda value: dump_gron(value, sort_keys=True),
        colorize: bool = True):
    value_a_lines = list(dumper(value_a))
    value_b_lines = list(dumper(value_b))
    diff_lines = difflib.unified_diff(
        value_a_lines, value_b_lines,
        n=unified_n,
    )
    result = '\n'.join(
...
def get_diff_text(  # type: ignore  # TODO: fix
    ...
    result = make_diff(
        ...
        unified_n=unified_n, dumper=dumper,
        colorize=colorize)
    return result

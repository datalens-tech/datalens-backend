...
class DatetimeSerializer(TypeSerializer):
    ...
    def from_jsonable(value: TJSONLike) -> datetime.datetime:  # type: ignore  # TODO: fix
        ...
        tzinfo = (
            ...
            if offset_sec is not None else None)
...
COMMON_SERIALIZERS = [
    DateSerializer, DatetimeSerializer,
    TimeSerializer, TimedeltaSerializer,
    DecimalSerializer, UUIDSerializer,
]
...
class RedisDatalensDataJSONEncoder(json.JSONEncoder):
    ...
    def default(self, obj):  # type: ignore  # TODO: fix
        ...
        if preprocessor is not None:
            return dict(
                __dl_type__=preprocessor.typename,
                value=preprocessor.to_jsonable(obj)
            )

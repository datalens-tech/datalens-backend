...
from typing import Any, ClassVar, Collection, Dict, List, NamedTuple, Optional, Set, Tuple, Union
...
class SqlSourceBuilder:
    ...
    def make_join_clause(
            self, left: SqlSourceType, right: SqlSourceType,
            join_type: JoinType, on_clause: JoinExpressionType,
    ) -> SqlSourceType:
    ...
    def collect_avatar_join_info_list(
        ...
        def _recursive_collect(avatar_id: Optional[AvatarId], ambiguous_relations: bool) -> None:
            avatar_user_relations = [
                rel for rel in relations_by_left[avatar_id]
                if (
                    (rel.left_id is not None or ambiguous_relations)
                    and rel.right_id in required_avatar_ids
                )
            ]
            for prep_relation in avatar_user_relations:
                ...
                avatar_join_info_list.append(AvatarJoinInfo(
                    avatar_id=child_avatar_id,
                    join_type=prep_relation.join_type,
                    on_clause=relation_expr
                ))
        ...
        for avatar_join_info in avatar_join_info_list:
            ...
            if not prep_src_info.supports_join_type(join_type):
                raise exc.DatasetConfigurationError(
                    f'Data source does not support {join_type.name.upper()} JOIN'
                )
            right = prep_src_info.non_null_sql_source
            source = self.make_join_clause(
                left=source, right=right,
                join_type=join_type, on_clause=avatar_join_info.on_clause,
            )
    ...
    def build_source(
        ...
        data_source_list = tuple(chain.from_iterable(
            (prep_src_info.data_source_list or ()) for prep_src_info in prepared_sources
        ))
        pass_db_query_to_user = all(
            prep_src_info.pass_db_query_to_user for prep_src_info in prepared_sources
        )

...
from bi_formula.definitions.common_datetime import (
    datetime_interval, ensure_naive_first_arg,
)
...
DEFINITIONS_DATETIME = [
    ...
    base.FuncDateadd3DateConstNum(variants=[
        V(D.POSTGRESQL, lambda date, what, num: (
            sa.cast(
                date + datetime_interval(un_literal(what), un_literal(num), literal_type=True, literal_mult=True),
                sa.Date
            )
        )),
    ]),
    base.FuncDateadd3DatetimeConstNum(variants=[
        V(D.POSTGRESQL, lambda dt, what, num: (
    ...
    base.FuncDatetrunc2Date(variants=[
        V(D.POSTGRESQL, lambda date, unit: (
            sa.cast(sa.func.date_trunc(base.norm_datetrunc_unit(unit), date), sa.Date)
            if base.norm_datetrunc_unit(unit) in {'year', 'quarter', 'month', 'week', 'day'} else
            date
        )),
    ]),
    base.FuncDatetrunc2Datetime(variants=[
        VW(D.POSTGRESQL, lambda date, unit: (
            sa.func.date_trunc(base.norm_datetrunc_unit(unit.expression), date.expression)
        )),
    ]),
    ...
    base.FuncDayofweek2(variants=[
        VW(D.POSTGRESQL, ensure_naive_first_arg(lambda date_ctx, firstday_ctx: base.dow_firstday_shift(

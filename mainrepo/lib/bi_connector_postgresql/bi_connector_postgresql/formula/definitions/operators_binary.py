...
DEFINITIONS_BINARY = [
    ...
    base.BinaryPlusDateInt(variants=[
        V(D.POSTGRESQL, lambda date, days: sa.cast(
    ...
    base.BinaryMinusDatetimes(variants=[
        V(D.POSTGRESQL, lambda left, right: (
            (sa.func.extract('epoch', left) - sa.func.extract('epoch', right)) / DAY_SEC)),
    ]),

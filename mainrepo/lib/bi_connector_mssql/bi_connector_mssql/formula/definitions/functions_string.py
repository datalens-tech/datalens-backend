...
DEFINITIONS_STRING = [
    ...
    base.FuncEndswithNonConst(variants=[
        V(D.MSSQLSRV, lambda x, y: (
            sa.func.SUBSTRING(x, sa.func.LEN(x) - sa.func.LEN(y) + 1, sa.func.LEN(x)) == y)),
    ]),
    ...
    base.FuncFind3(variants=[
        V(D.MSSQLSRV, lambda x, y, z: sa.func.IIF(
            sa.func.PATINDEX(
                make_like_pattern_mssql_clause(y, left_any=True, right_any=True),

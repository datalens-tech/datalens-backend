...
from bi_constants.enums import OrderDirection, FieldRole, RangeType

from bi_api_client.dsmaker.primitives import (
    UpdateAction, Dataset, ResultField,
    OrderedField, RequestLegendItem,
    WhereClause,
    PivotPagination, RequestPivotItem, PivotTotals,
    BlockSpec,
    ParameterFieldValue,
)
from bi_api_client.dsmaker.api.base import HttpApiResponse
from bi_api_client.dsmaker.api.http_sync_base import SyncHttpApiV1Base, ClientResponse
from bi_api_client.dsmaker.api.http_async_base import AsyncHttpApiV1Base
from bi_api_client.dsmaker.api.serialization_base import BaseApiV1SerializationAdapter
from bi_api_client.dsmaker.api.schemas.dataset import WhereClauseSchema
from bi_api_client.dsmaker.api.schemas.data import (
    QueryFieldsItemSchema, PivotPaginationSchema,
    QueryBlockSchema, FilterClauseSchema, PivotItemSchema, PivotTotalsSchema,
    ResultResponseSchema, PivotResponseSchema,
)
...
class SyncHttpDataApiV1(SyncHttpDataApiBase):
    ...
    def get_response_for_dataset_result(self, dataset_id: str, raw_body: dict,
                                        headers: Optional[dict] = None) -> ClientResponse:
...
class DataApiV2SerializationAdapter(BaseApiV1SerializationAdapter):
    ...
    def make_req_data_get_result(
        ...
        data = self.make_req_data_common(
            dataset=dataset, limit=limit, offset=offset, fields=fields,
            order_by=order_by, filters=filters, parameters=parameters, blocks=blocks, updates=updates,
            ignore_nonexistent_filters=ignore_nonexistent_filters,
    ...
    def make_req_data_get_value_range(
        ...
        data = self.make_req_data_common(
            ...
            filters=filters, parameters=parameters, blocks=blocks, updates=updates,
            ignore_nonexistent_filters=ignore_nonexistent_filters,
        )
    ...
    def make_req_data_get_pivot(
        ...
        data = self.make_req_data_common(
            dataset=dataset, limit=limit, offset=offset, fields=fields,
            order_by=order_by, filters=filters, blocks=blocks,
            autofill_legend=autofill_legend, updates=updates,
            ignore_nonexistent_filters=ignore_nonexistent_filters,
        )
        data['pivot'] = {}
        if pivot_structure is not None:
            data['pivot']['structure'] = [
                PivotItemSchema().dump(item) for item in pivot_structure
            ]
...
class SyncHttpDataApiV2(SyncHttpDataApiBase):
    ...
    def get_response_for_dataset_preview(
            self, dataset_id: Optional[str], raw_body: dict,
            headers: Optional[dict] = None,
    ) -> ClientResponse:
        if dataset_id is None:
            url = f'/api/data/{self.api_v}/datasets/data/preview'
    ...
    def get_response_for_dataset_result(
            self, dataset_id: str, raw_body: dict,
            headers: Optional[dict] = None,
    ) -> ClientResponse:
    ...
    def get_preview(
        ...
        data = self.serial_adapter.make_req_data_get_preview(
            dataset=dataset, limit=limit, updates=updates
        )
        response = self.get_response_for_dataset_preview(
            dataset_id=(dataset.id if dataset.created_ else None),
            raw_body=data,

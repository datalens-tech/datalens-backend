...
from bi_api_client.dsmaker.api.data_api import SyncHttpDataApiV1, SyncHttpDataApiV2, HttpDataApiResponse
...
def get_regular_result_data(
        ds: Dataset, data_api: Union[SyncHttpDataApiV1, SyncHttpDataApiV2], field_names: list[str],
) -> dict[str, list[str]]:
    field_names = sorted(set(field_names))  # Deduplicate fields
    result_resp = data_api.get_result(
        dataset=ds,
        fields=[
            ds.find_field(title=field_name)
    ...
    return {
        field_name: [row[field_idx] for row in data_rows]

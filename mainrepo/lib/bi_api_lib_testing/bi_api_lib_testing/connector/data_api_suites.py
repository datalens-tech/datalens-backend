...
from bi_api_lib_testing.data_api_base import DataApiTestParams, StandardizedDataApiTestBase
...
class DefaultConnectorDataResultTestSuite(StandardizedDataApiTestBase):
    ...
    def test_array_contains_filter(
        ...
        columns = [
            ...
            C('array_str_value', BIType.array_str,
              vg=lambda rn, **kwargs: [str(i) if i != 5 else None for i in reversed(range(rn))]),
            C('array_float_value', BIType.array_float,
    ...
    @pytest.mark.parametrize('field_title,filter_field_title,is_numeric', (
...
class DefaultConnectorDataGroupByFormulaTestSuite(StandardizedDataApiTestBase):
    def test_complex_result(
        ...
        grouped_resp = self.get_result_ordered(ds, data_api,
                                               field_names=(data_api_test_params.two_dims[0], 'CityNameLength'),
                                               order_by=('CityNameLength',))

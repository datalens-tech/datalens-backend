...
class DefaultOperatorFormulaConnectorTestSuite(FormulaConnectorTestBase):
    ...
    def test_grouping_with_operators(self, dbe: DbEvaluator) -> None:
        ...
        assert dbe.eval('[f1] / [f2]', other_fields={
            'f1': 'ROUND(1000)',
            'f2': '(ROUND(100) / 10) * 10',
        }) == 10
    ...
    def test_addition_array_array(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        assert dbe.eval('[arr_int_value] + [arr_int_value]', from_=data_table) == \
               dbe.eval('ARRAY(0, 23, 456, NULL, 0, 23, 456, NULL)')
        assert dbe.eval('ARRAY(1, 2, 3) + ARRAY(6, NULL, 0)', from_=data_table) == \
               dbe.eval('ARRAY(1, 2, 3, 6, NULL, 0)')

        assert dbe.eval('[arr_str_value] + [arr_str_value]', from_=data_table) == \
               dbe.eval('ARRAY("", "", "cde", NULL, "", "", "cde", NULL)')
        assert dbe.eval('ARRAY("a", "", "3") + ARRAY("c", NULL, "e")', from_=data_table) == \
               dbe.eval('ARRAY("a", "", "3", "c", NULL, "e")')

        if self.make_float_array_cast is not None:
            assert dbe.eval(
                '[arr_float_value] + [arr_float_value]', from_=data_table
            ) == dbe.eval(
                f'DB_CAST(ARRAY(0, 45, 0.123, NULL, 0, 45, 0.123, NULL), "{self.make_float_array_cast}")'
            )
            assert dbe.eval(
                f'DB_CAST(ARRAY(1.1, 2.2, 3.3) + ARRAY(6.6, NULL, 0), "{self.make_float_array_cast}")',
                from_=data_table
            ) == dbe.eval('ARRAY(1.1, 2.2, 3.3, 6.6, NULL, 0)')
        else:
            assert dbe.eval(
                '[arr_float_value] + [arr_float_value]', from_=data_table
            ) == dbe.eval('ARRAY(0, 45, 0.123, NULL, 0, 45, 0.123, NULL)')
            assert dbe.eval(
                'ARRAY(1.1, 2.2, 3.3) + ARRAY(6.6, NULL, 0)', from_=data_table
            ) == dbe.eval('ARRAY(1.1, 2.2, 3.3, 6.6, NULL, 0)')

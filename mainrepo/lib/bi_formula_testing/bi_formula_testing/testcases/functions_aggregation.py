...
import datetime
from typing import ClassVar, Collection, TypeVar, Union
...
class DefaultMainAggFunctionFormulaConnectorTestSuite(FormulaConnectorTestBase):
    ...
    def test_conditional_aggregation_functions(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        assert dbe.eval(
            'AVG_IF([int_value] + 2 * 2, [int_value] > 5)', from_=data_table
        ) == (lambda x: sum(x) / len(x))(
            [v + 2 * 2 for v in values if v > 5]
        )

        # Zero matching elements
        assert dbe.eval('AVG_IF([int_value], [int_value] > {max_val} and [int_value] < {min_val})'.format(
            max_val=max(values),
            min_val=min(values)
        ), from_=data_table) is None

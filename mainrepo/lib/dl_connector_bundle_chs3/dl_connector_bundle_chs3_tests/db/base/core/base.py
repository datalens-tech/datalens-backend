...
class BaseCHS3TestClass(BaseConnectionTestClass[FILE_CONN_TV], metaclass=abc.ABCMeta):
    ...
    async def s3_client(self, s3_settings: S3Settings) -> aiobotocore.client.AioBaseClient:
        async with create_s3_client(s3_settings) as client:
            yield client

    @pytest.fixture(scope="function")
    async def s3_bucket(self, s3_client: aiobotocore.client.AioBaseClient) -> str:

...
from dl_connector_bundle_chs3_tests.db.gsheets_v2.api.base import GSheetsFileS3ApiConnectionTestBase
from dl_core.us_manager.us_manager_sync import SyncUSManager
from dl_testing.utils import get_log_record


class TestGSheetsFileS3Connection(GSheetsFileS3ApiConnectionTestBase, CHS3ConnectionTestSuite[GSheetsFileS3Connection]):
    ...
    def test_authorization_field(
        ...
        conn = sync_us_manager.get_by_id(conn_id, GSheetsFileS3Connection)
        base_update_data = {
        ...
        assert "refresh_token" not in conn_resp.json
        ...
        assert resp.status_code == HTTPStatus.OK, resp.json
        conn: GSheetsFileS3Connection = sync_us_manager.get_by_id(conn_id, GSheetsFileS3Connection)
        assert conn.authorized is False
        conn_resp = client.get(f"/api/v1/connections/{conn_id}")
    ...
    def test_force_update_with_file_id(
        ...
        conn_id = saved_connection_id
        usm = sync_us_manager
        conn = usm.get_by_id(conn_id, GSheetsFileS3Connection)

...
FUNCTION_IFNULL = FunctionDocRegistryItem(
    ...
    category=CATEGORY_LOGICAL,
    description=_(
        "Returns {arg:0} if it's not `NULL`. Otherwise returns {arg:1}."
    ),
    examples=[
        DataExample(
            example_config=ExampleConfig(
...
FUNCTION_ISNAN = FunctionDocRegistryItem(
    ...
    category=CATEGORY_LOGICAL,
    description=_(
        "Returns `TRUE` if {arg:0} is `NaN`, otherwise returns `FALSE`.\n"
    ),
)
...
FUNCTION_IFNAN = FunctionDocRegistryItem(
    ...
    category=CATEGORY_LOGICAL,
    description=_(
        "Returns {arg:0} if it's not `NaN`. Otherwise returns {arg:1}."
    ),
    examples=[
...
FUNCTION_ZN = FunctionDocRegistryItem(
    ...
    category=CATEGORY_LOGICAL,
    description=_(
        "Returns {arg:0} if it's not `NULL`. Otherwise returns 0."
    ),
    examples=[
        DataExample(
            example_config=ExampleConfig(

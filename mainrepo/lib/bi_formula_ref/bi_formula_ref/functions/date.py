...
_SOURCE_DATE_1 = ExampleSource(
    columns=[('Date', DataType.DATE)],
    data=[[_d(2014, 10, 6)], [_d(2014, 10, 7)], [_d(2017, 3, 8)], [_d(2024, 2, 12)]]
)
_SOURCE_DATETIME_1 = ExampleSource(
    columns=[('DateTime', DataType.GENERICDATETIME)],
    data=[
        [_dt(2014, 10, 6, 7, 45, 12)], [_dt(2014, 10, 7, 11, 10, 15)],
        [_dt(2017, 3, 8, 23, 59, 59)], [_dt(2024, 2, 12, 7, 40, 33)],
    ]
)
...
FUNCTION_DATEADD = FunctionDocRegistryItem(
    ...
    notes=[
        Note(
            _(
                "For all sources except {dialects:CLICKHOUSE}, {arg:2} takes "
    ...
    examples=[
        DataExample(
            example_config=ExampleConfig(
                name=_('Example with date'), source=_SOURCE_DATE_1,
                formula_fields=[
                    ('Date', '[Date]'), ('Year', 'DATEPART([Date], "year")'),
                    ('Month', 'DATEPART([Date], "month")'), ('Day', 'DATEPART([Date], "day")'),
                    ('DayOfWeek', 'DATEPART([Date], "dayofweek")'), ('DOW', 'DATEPART([Date], "dow")'),
                ],
        ...
        DataExample(
            example_config=ExampleConfig(
                ...
                formula_fields=[
                    ('DateTime', '[DateTime]'), ('Year', 'DATEPART([DateTime], "year")'),
                    ('Month', 'DATEPART([DateTime], "month")'), ('Day', 'DATEPART([DateTime], "day")'),
                    ('Hour', 'DATEPART([DateTime], "hour")'), ('Minute', 'DATEPART([DateTime], "minute")'),
...
FUNCTION_DAY = FunctionDocRegistryItem(
    ...
    category=CATEGORY_DATE,
    description=_(
        "Returns the number of the day in the month of the specified date {arg:0}."
    ),
    examples=[
...
FUNCTION_MONTH = FunctionDocRegistryItem(
    ...
    category=CATEGORY_DATE,
    description=_(
        "Returns the number of the month in the year of the specified date {arg:0}."
    ),
    examples=[
...
FUNCTION_QUARTER = FunctionDocRegistryItem(
    ...
    category=CATEGORY_DATE,
    description=_(
        "Returns the number of the quarter (from `1` to `4`) of the year of the specified date {arg:0}."
    ),
    examples=[
...
FUNCTION_YEAR = FunctionDocRegistryItem(
    ...
    category=CATEGORY_DATE,
    description=_(
        "Returns the year number in the specified date {arg:0}."
    ),
    examples=[
...
FUNCTION_NOW = FunctionDocRegistryItem(
    ...
    category=CATEGORY_DATE,
    description=_(
        "Returns the current date and time, depending on the data source and "
        "connection type."
    ),
    notes=[
        Note(

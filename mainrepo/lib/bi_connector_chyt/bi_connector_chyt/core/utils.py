...
from bi_connector_chyt.core import exc as chyt_exc
...
class CHYTUtils(ClickHouseBaseUtils):
    ...
    chyt_expr_exc = {
        ...
        r'Invalid sorted JOIN: joined columns should form prefix of joined table key columns':
            chyt_exc.CHYTInvalidSortedJoinNotKeyPrefixColumn,
        r'Invalid sorted JOIN: only single table may currently be joined':
            chyt_exc.CHYTInvalidSortedJoinMoreThanOneTable,
        ...
        r'Invalid sorted JOIN: CHYT does not support compound expressions in ON/USING clause':
            chyt_exc.CHYTInvalidSortedJoinCompoundExpressionsNotSupported,
        ...
        r'Invalid sorted JOIN: joining concatenation of multiple tables is not supported':
            chyt_exc.CHYTInvalidSortedJoinConcatNotSupported,
        r'Invalid sorted JOIN: joined columns (?P<col1>.*) and (?P<col2>.*)'
            r' do not occupy same positions in key columns of joined tables':
            chyt_exc.CHYTInvalidSortedJoinNotSameKeyPosition,
        ...
        r'Reading multiple dynamic tables or dynamic table together with static table is not supported':
            chyt_exc.CHYTMultipleDynamicTablesNotSupported,

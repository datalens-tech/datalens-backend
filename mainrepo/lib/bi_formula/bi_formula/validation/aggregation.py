...
class AggregationChecker(Checker):
    ...
    def _check_is_dimension_bound(self, node: nodes.FormulaItem, dimension_set: NodeSet) -> bool:
        ...
        dim_bound = bi_formula.inspect.expression.is_bound_only_to(node, allow_nodes=dimension_set)
        return dim_bound
    ...
    def perform_node_check(
        ...
        if not self._allow_nested_agg and bi_formula.inspect.node.is_aggregate_function(node):

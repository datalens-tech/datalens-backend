...
import decimal
from typing import Callable, ClassVar, List, Optional, Sequence, TypeVar, Union
...
class DefaultWindowFunctionFormulaConnectorTestSuite(FormulaConnectorTestBase):
    ...
    def test_rank_dense(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        assert dbe.eval('RANK_DENSE([id], "asc" WITHIN [int_value])', from_=data_table, order_by=['[id]'], many=True) == (
            [1] * 10 + [2] * 10
        )
    ...
    def test_rank_unique(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        assert dbe.eval('RANK_UNIQUE([id], "asc" WITHIN [int_value])', from_=data_table, order_by=['[id]'], many=True) == (
            [1] * 10 + [2] * 10
        )
    ...
    def test_rank_percentile(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        assert approx(dbe.eval(
            'RANK_PERCENTILE([int_value], "asc")', from_=data_table, order_by=['[id]'], many=True
        )) == (
    ...
    def test_count(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        # 0 args
        assert dbe.eval('COUNT(TOTAL)', from_=data_table, order_by=['[id]'], many=True) == (
            [len(values)] * 20
        )
    ...
    def test_avg(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        assert values == list(range(0, 100, 10)) + list(range(0, 100, 10))
        assert approx(dbe.eval(
            'AVG([int_value] TOTAL)', from_=data_table, order_by=['[id]'], many=True
        )) == (
            [sum(values) / 20] * 20
        )
        assert approx(dbe.eval(
            'AVG([id] WITHIN [int_value])', from_=data_table, order_by=['[id]'], many=True
        )) == (
    ...
    def test_sum_if(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        assert values == list(range(0, 100, 10)) + list(range(0, 100, 10))
        assert dbe.eval(
            'SUM_IF([int_value], [id] < 11 TOTAL)', from_=data_table, order_by=['[id]'], many=True
        ) == (
    ...
    def test_count_if(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        assert values == list(range(0, 100, 10)) + list(range(0, 100, 10))
        assert dbe.eval(
            'COUNT_IF([int_value], [id] < 11 TOTAL)', from_=data_table, order_by=['[id]'], many=True
        ) == (
            [10] * 20
        )
        assert dbe.eval(
    ...
    def test_avg_if(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        assert dbe.eval(
            ...
            [val / 1.0 for val in range(1, 11)] + [val for val in range(1, 11)]
        )

    def _check_rfunc(
            self, dbe: DbEvaluator, data_table: sa.Table,
            func_name: str, py_agg_func: Callable[[List[NUMERIC]], NUMERIC]
    ) -> None:
        ids = list(range(1, 21))
        values = data_table.int_values
        assert values == list(range(0, 100, 10)) + list(range(0, 100, 10))
        assert approx(dbe.eval(
            f'{func_name}([int_value] TOTAL)', from_=data_table, order_by=['[id]'], many=True
        )) == (
            [py_agg_func(values[:i+1]) for i in range(20)]
        )
        assert approx(dbe.eval(
            f'{func_name}([id] WITHIN [int_value])', from_=data_table, order_by=['[id]'], many=True
        )) == (
            [py_agg_func([ids[i]]) for i in range(10)] + [py_agg_func([ids[i], ids[i+10]]) for i in range(10)]
        )
        assert approx(dbe.eval(
            f'{func_name}([int_value], "desc" TOTAL)', from_=data_table, order_by=['[id]'], many=True
        )) == (
            [py_agg_func(values[i:]) for i in range(20)]
        )
        assert approx(dbe.eval(
            f'{func_name}([int_value], "asc" TOTAL ORDER BY [id] DESC)', from_=data_table, order_by=['[id]'], many=True
        )) == (
            [py_agg_func(values[i:]) for i in range(20)]
        )
        assert approx(dbe.eval(
            f'{func_name}([id], "desc" WITHIN [int_value])', from_=data_table, order_by=['[id]'], many=True
        )) == (
    ...
    def _check_mfunc(
        ...
        assert values == list(range(0, 100, 10)) + list(range(0, 100, 10))
        assert approx(dbe.eval(
            f'{func_name}([int_value], 1)', from_=data_table, order_by=['[id]'], many=True
        )) == (
            [py_agg_func(values[max(0, i - 1):i + 1]) for i in range(20)]
        )
        assert approx(dbe.eval(
            f'{func_name}([int_value], 1 ORDER BY [id] DESC)', from_=data_table, order_by=['[id]'], many=True
        )) == (
        ...
        assert approx(dbe.eval(
            f'{func_name}([int_value], 5, 5)', from_=data_table, order_by=['[id]'], many=True
        )) == (
    ...
    def test_lag(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        def lag(
                values: Sequence[VALUE_TV], idx: int, offset: int = 1,
        ...
        assert values == list(range(0, 100, 10)) + list(range(0, 100, 10))
        assert dbe.eval(
            'LAG([int_value])', from_=data_table, order_by=['[id]'], many=True
        ) == (
            [lag(values, i, offset=1) for i in range(20)]
        )
        assert dbe.eval(
            'LAG([int_value] ORDER BY [id] DESC)', from_=data_table, order_by=['[id]'], many=True
        ) == (
            [lag(values, i, offset=-1) for i in range(20)]
        )
        assert dbe.eval(
            'LAG([int_value], -2)', from_=data_table, order_by=['[id]'], many=True
        ) == (
        ...
        # With default
        assert dbe.eval(
            'LAG([int_value], -3, 8)', from_=data_table, order_by=['[id]'], many=True
        ) == (
    ...
    def test_first(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        assert dbe.eval(
            'FIRST([int_value] TOTAL ORDER BY [id])', from_=data_table, many=True
        ) == [values[0]] * len(values)
        assert dbe.eval(
            'FIRST([int_value] TOTAL ORDER BY [id] DESC)', from_=data_table, many=True
        ) == [values[-1]] * len(values)
    ...
    def test_last(self, dbe: DbEvaluator, data_table: sa.Table) -> None:
        ...
        assert dbe.eval(
            'LAST([int_value] TOTAL ORDER BY [id])', from_=data_table, many=True
        ) == [values[-1]] * len(values)
        assert dbe.eval(
            'LAST([int_value] TOTAL ORDER BY [id] DESC)', from_=data_table, many=True
        ) == [values[0]] * len(values)

...
from bi_formula.definitions.base import (
    Function, TranslationVariant, TranslationVariantWrapped,
)
from bi_formula.definitions.flags import ContextFlag
from bi_formula.definitions.type_strategy import Fixed, FromArgs, ParamsFromArgs
...
class AggCountd(AggregationFunction):
    ...
    variants = [
        V(D.DUMMY | D.SQLITE,
          lambda x: sa.func.count(sa.distinct(x))),
    ]
    return_type = Fixed(DataType.INTEGER)
...
class AggCountdIf(AggregationFunction):
    ...
    arg_names = ['expression', 'condition']
    argument_types = [
        ArgTypeSequence([expr_type, DataType.BOOLEAN]) for expr_type in DataType
    ]
    argument_flags = ArgFlagSequence([None, ContextFlag.REQ_CONDITION])
    return_type = Fixed(DataType.INTEGER)
...
AGG_CONCAT_INPUT_TYPES = (
    set(DataType) -
    set((DataType.MARKUP, DataType.CONST_MARKUP, DataType.UNSUPPORTED))
)

...
from bi_formula.slicing.schema import SliceSchema, LevelBoundary
from bi_formula.inspect.expression import enumerate_autonomous_children
...
class LevelSlice:
    ...
    def is_lazy(self) -> bool:
        ...
        for alias, node in self.aliased_nodes.items():
...
class FormulaSlicer:
        ...
        while from_level < to_level:
            if self._boundary_for_level(from_level).should_maintain_level(
                ...
                parent_node, child_node = self._slice_node(
                    ...
                    parent_node=parent_node, child_node=child_node, child_node_idx=child_node_idx,
                    level=from_level,
                )
            from_level += 1
    ...
    def _slice_node_recursively(
        ...
        for child_node_idx, child_node, _ in enumerate_autonomous_children(cur_node, parent_stack=parent_stack):
            child_level, sliced_child_node = self._slice_node_recursively(
                sliced_formula_info=sliced_formula_info, cur_node=child_node,
        ...
        while self._boundary_for_level(cur_level).should_raise_level(
                node=original_cur_node, inspect_env=self.env.inspect_env,
        ...
        for child_node_idx, sliced_child_node, child_level in children_and_levels:
            sliced_cur_node = self._pull_to_level_and_replace(
                sliced_formula_info=sliced_formula_info,
                parent_node=sliced_cur_node, child_node=sliced_child_node, child_node_idx=child_node_idx,
                parent_stack=parent_stack_for_children,
                from_level=child_level, to_level=cur_level,
            )
    ...
    def slice_formula(
        ...
        sliced_formula_info = SlicedFormulaInfo(
            slices=[
                LevelSlice(name=boundary.name) for boundary in self.slice_schema.levels
            ],
        )
        ...
        level_after_slicing, sliced_expr = self._slice_node_recursively(
            sliced_formula_info=sliced_formula_info,
            cur_node=formula_expr, parent_stack=formula_expr_parent_stack,
        )
        ...
        sliced_formula = self._pull_to_level_and_replace(
            sliced_formula_info=sliced_formula_info,
            parent_node=sliced_formula, child_node=sliced_expr, child_node_idx=formula_expr_idx,
            from_level=level_after_slicing, parent_stack=formula_expr_parent_stack,

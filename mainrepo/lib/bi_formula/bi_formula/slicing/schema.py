...
class AggregateFunctionLevelBoundary(LevelBoundary):
    ...
    def check_node(
        ...
        if self.constants_neutral:
            if bi_formula.inspect.expression.is_constant_expression(node, env=inspect_env):
...
class NestedLevelTaggedBoundary(LevelBoundary):
    ...
    def get_node_tag(
        ...
        if (
            bi_formula.inspect.node.is_aggregate_function(node)
            and isinstance(parent, fork_nodes.QueryFork)
            and bi_formula.inspect.node.qfork_is_aggregation(parent)  # only LODs, avoid lookups here
        ):
        ...
        if (
            ...
            and bi_formula.inspect.function.supports_bfb(node.name, is_window=True)

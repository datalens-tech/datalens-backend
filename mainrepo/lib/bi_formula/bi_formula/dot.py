...
class DotTranslator:
    ...
    def _translate_node_literal(self, node: nodes.BaseLiteral, dot: graphviz.Digraph) -> str:
        node_name = make_name()
        dot.node(node_name, label='{!r}'.format(node.value), **self.terminal_opts,
    ...
    def _translate_node_expression_list(self, node: nodes.ExpressionList, dot: graphviz.Digraph) -> str:
        node_name = make_name()
        dot.node(node_name, label='[0..{}]'.format(len(node.children)-1),
                 shape='hexagon', style='filled', fillcolor='#bba7fc')
    ...
    def _translate_node_func_call(
        ...
        dot.node(node_name, label='{}({})'.format(node.name.upper(), ', '.join(args_names)),
                 shape='oval', style='filled', fillcolor='#bde6b1')
    ...
    def _translate_node_window_grouping_total(
            self, node: nodes.WindowGroupingTotal, dot: graphviz.Digraph
    ) -> str:
        node_name = make_name()

...
class ParserNotFoundError(Exception):
    def __init__(self, *args, **kwargs):
        super().__init__()
        self.description = "Cannot import parser files," \
...
class FormulaError(Exception):
    ...
    def __init__(
            self, *errors: Union[str, FormulaErrorCtx],
        ...
        for error in errors:
            if not isinstance(error, FormulaErrorCtx):
                error = FormulaErrorCtx(
                    message=error, level=MessageLevel.ERROR,
                    position=position or Position(), token=token,

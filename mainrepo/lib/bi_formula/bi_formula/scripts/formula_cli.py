...
import bi_formula.dot
import bi_formula.translation.translator
...
from bi_formula.core.dialect import (
    get_all_basic_dialects, get_dialect_from_str,
    DialectCombo, StandardDialect as D,
)
from bi_formula.definitions.base import NodeTranslation, MultiVariantTranslation
...
from bi_formula.slicing.schema import (
    SliceSchema, AggregateFunctionLevelBoundary, WindowFunctionLevelBoundary,
    TopLevelBoundary, NonFieldsBoundary,
)
from bi_formula.slicing.slicer import FormulaSlicer
from bi_formula.loader import load_bi_formula

from .common import csv_type, make_graphviz_graph
...
dialect_parser.add_argument('--dialect', required=True, type=dialect_type,
...
trans_parser = subparsers.add_parser('translate', parents=[optional_text_parser, dialect_parser, supperr_parser],
...
parse_parser = subparsers.add_parser('parse', parents=[optional_text_parser, supperr_parser],
                                     help='Parse formula and print its internal representation')
...
split_parser = subparsers.add_parser('split', parents=[text_parser, diff_parser],
                                     help='Split formula into nodes and print them by nesting level')

graph_parser = subparsers.add_parser('graph', parents=[text_parser],
                                     help='Generate GraphViz DOT graph')
...
slice_parser = subparsers.add_parser('slice', parents=[text_parser, diff_parser],
                                     help='Parse formula slice it into translation levels')
...
class FormulaCliTool:
    ...
    def graph(text: str, render_to: str, view: bool):
        formula = formula_parser.parse(text)
        dot = bi_formula.dot.translate(formula)
        make_graphviz_graph(dot, render_to=render_to, view=view)

    @classmethod
    def translate(cls, text: str, tablename: Optional[str], dialect: DialectCombo,
                  unknown_funcs: bool, suppress_errors: bool):
        if not text:
            # bulk mode, read from STDIN
            for line in cls._stdin_lines():
                cls.translate(
                    text=line, tablename=tablename, dialect=dialect, unknown_funcs=unknown_funcs,
        ...
        try:
            ...
            field_list = sorted(set(f.name for f in bi_formula.inspect.expression.used_fields(formula)))
            ...
            raw_sql = bi_formula.translation.translator.translate_and_compile(
                formula, dialect=dialect, restrict_funcs=not unknown_funcs,
                field_types=field_types, field_names=field_names,  # type: ignore  # TODO: fix
            )
            print(raw_sql)
    ...
    @staticmethod
    def list_dialects():
        print('\n'.join([d.common_name_and_version for d in sorted(
            get_all_basic_dialects(), key=lambda el: el.single_bit.orderable
        )]))
    ...
    def slice(cls, text: str, levels: List[str], diff: bool = False) -> None:
        ...
        formula = formula_parser.parse(text)
        slicer = FormulaSlicer(slice_schema=SliceSchema(levels=[
            boundaries[level_name] for level_name in levels
        ]))
    ...
    def print_registry(cls) -> None:
        ...
        for key, item in OPERATION_REGISTRY.items():
    ...
    def run(cls, args):
        ...
        if args.command == 'parse':
            tool.parse(
                text=args.optional_text, pretty=args.pretty,
                suppress_errors=args.suppress_errors, pos=args.pos,
            ...
            tool.translate(
                text=args.optional_text, tablename=args.table, dialect=args.dialect,
                unknown_funcs=args.unknown_funcs, suppress_errors=args.suppress_errors,
            )

...
from bi_formula.slicing.schema import (
    SliceSchema,
    WindowFunctionLevelBoundary, AggregateFunctionLevelBoundary,
    TopLevelBoundary, NonFieldsBoundary, NestedLevelTaggedBoundary,
)
from bi_formula.slicing.slicer import FormulaSlicer, SlicedFormulaInfo, LevelSlice
...
def test_aggregation_function_slicer():
    ...
    agg_slicer = FormulaSlicer(
        slice_schema=SliceSchema(levels=[
    ...
    expected = SlicedFormulaInfo(slices=[
        LevelSlice(
            name='agg',
            required_fields=set(),
            aliased_nodes={
                'agg_1': n.formula(n.func.MY_FUNC(n.field('My Field')))
            },
        ),
        LevelSlice(
            name='top',
            ...
            aliased_nodes={
                'top_1': n.formula(n.func.SUM(n.field('agg_1')))
            },
        ),
    ])
...
def test_window_function_slicer():
    ...
    sliced_fla_info = win_slicer.slice_formula(
        formula=n.formula(n.func('*')(
            n.func('/')(
                n.wfunc.RSUM(n.func.SUM(n.func.MY_FUNC(n.field('Sales'))), grouping=n.among(n.field('Dim')),
...
def test_tagged_slicer():
    ...
    expected = SlicedFormulaInfo(slices=[
        LevelSlice(
            ...
            required_fields=set(),
            aliased_nodes={
                'tagged_1': n.formula(n.wfunc.RSUM(n.field('My Field')))
            },
        ),
        LevelSlice(
            name='top',

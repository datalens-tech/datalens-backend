...
def test_functions():
    assert n.func('+')(
        n.func.MY_FUNC(123, True, nodes.LiteralString.make('456')),
        None
    ) == (
        nodes.FuncCall.make(
            name='+',
...
def test_window_functions():
    ...
    assert n.func.MY_FUNC(n.field('f0'), among=[n.field('f1')]) == (
        nodes.WindowFuncCall.make(
            name='my_func',
            args=[nodes.Field.make(name='f0')],
            grouping=nodes.WindowGroupingAmong.make(dim_list=[
                nodes.Field.make(name='f1')
            ])
        )
    )
...
def test_window_functions_before_filter_by():
    assert n.func.MY_FUNC(n.field('f0'), total=True, before_filter_by=[n.field('f1')]) == (
        nodes.WindowFuncCall.make(
            name='my_func',
            args=[nodes.Field.make(name='f0')],
            before_filter_by=nodes.BeforeFilterBy.make(field_names=frozenset([
                'f1',
            ]))
        )
    )

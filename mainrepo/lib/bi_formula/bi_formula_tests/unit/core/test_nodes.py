...
def test_copy():
    ...
    _check_node_copy(nodes.FuncCall.make('func', args=[nodes.LiteralInteger.make(123), nodes.LiteralString.make('qwerty')]))
    ...
    _check_node_copy(
        nodes.Ternary.make(
            ...
            nodes.LiteralInteger.make(34)
        )
    )
    _check_node_copy(nodes.WindowFuncCall.make(
        'wfunc',
    ...
    _check_node_copy(nodes.WindowFuncCall.make(
        ...
        ordering=nodes.Ordering.make(expr_list=[
            nodes.Field.make('dim'),
        ]),
        before_filter_by=nodes.BeforeFilterBy.make(field_names=[
            'field_1', 'field_3', 'field_2',
        ])
    ))
    _check_node_copy(nodes.IfBlock.make([
        nodes.IfPart.make(nodes.LiteralBoolean.make(True), nodes.LiteralString.make('qwerty')),
        nodes.IfPart.make(nodes.LiteralBoolean.make(True), nodes.LiteralString.make('qwerty'))],
        nodes.LiteralString.make('abc')
    ))
    _check_node_copy(nodes.CaseBlock.make(
    ...
    _check_node_copy(nodes.Formula.make(
...
def test_light_copy():
    def _check_node_copy(
        ...
        children = [
            to_replace_map[idx] if idx in to_replace_map else child
            for idx, child in enumerate(src.children)
        ]
    ...
    _check_node_copy(
        nodes.WindowFuncCall.make(
            'wfunc',
            args=[lit_123],
            grouping=win_grouping_within_my_field
        ),

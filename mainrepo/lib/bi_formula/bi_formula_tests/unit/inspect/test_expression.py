...
def test_used_func_calls():
    assert used_func_calls(n.formula(
        nodes.FuncCall.make(name='func', args=[nodes.LiteralString.make('qwe')])
    )) == [
        nodes.FuncCall.make(name='func', args=[nodes.LiteralString.make('qwe')]),
    ]
    assert used_func_calls(n.formula(nodes.FuncCall.make(name='func', args=[
        nodes.Field.make('f1'),
        nodes.FuncCall.make(name='other_func', args=[
            nodes.Field.make('f2'),
...
def test_used_fields():
    ...
    assert used_fields(n.formula(nodes.FuncCall.make(name='func', args=[
        nodes.Field.make('f1'),
        nodes.FuncCall.make(name='other_func', args=[
            nodes.Field.make('f2'),
            n.p(nodes.Field.make('f3')),
...
def test_is_constant_expression():
    ...
    assert is_constant_expression(
        n.p(nodes.LiteralString.make('qwe')), env=env
    )
    assert not is_constant_expression(
        nodes.FuncCall.make(name='func', args=[nodes.LiteralString.make('qwe')]), env=env
    )
    assert not is_constant_expression(
        n.p(nodes.FuncCall.make(name='func', args=[nodes.LiteralString.make('qwe')])),
        env=env
    )
...
def test_is_is_bound_only_to():
    ...
    dim_2 = nodes.FuncCall.make(name='+', args=[
    ...
    assert is_bound_only_to(
        n.p(nodes.LiteralString.make('qwe')), allow_nodes=allow_nodes
    )
    assert is_bound_only_to(
        n.p(dim_1), allow_nodes=allow_nodes
    )
    assert is_bound_only_to(
        n.p(dim_2), allow_nodes=allow_nodes
    )
    assert is_bound_only_to(
    ...
    assert not is_bound_only_to(
        n.p(nodes.Field.make(name='my field')), allow_nodes=allow_nodes
    )
    assert not is_bound_only_to(
        nodes.FuncCall.make(name='func', args=[
            nodes.LiteralString.make('qwe'),
...
def test_is_aggregate_expression():
    ...
    assert is_aggregate_expression(
        nodes.FuncCall.make(name='avg', args=[nodes.LiteralString.make('qwe')]), env=env
    )
    assert is_aggregate_expression(
        nodes.FuncCall.make(name='avg', args=[nodes.Field.make('my field')]), env=env
    )
    assert is_aggregate_expression(
        n.p(nodes.FuncCall.make(name='avg', args=[nodes.LiteralString.make('qwe')])),
        env=env
    )
    ...
    assert not is_aggregate_expression(
        nodes.WindowFuncCall.make(
            name='sum', args=[nodes.Field.make('my field')],
            grouping=nodes.WindowGroupingTotal()), env=env,
    )
...
def test_is_window_expression():
    ...
    assert is_window_expression(
        nodes.WindowFuncCall.make(
            name='rank', args=[nodes.LiteralString.make('qwe')],
    ...
    assert is_window_expression(
        nodes.FuncCall.make(
            name='sum', args=[
                nodes.WindowFuncCall.make(
                    name='rank', args=[nodes.Field.make('my field')],
                    grouping=nodes.WindowGroupingTotal(),
                )
            ],
        ),
    ...
    assert is_window_expression(
        n.p(nodes.WindowFuncCall.make(name='rank', args=[nodes.LiteralString.make('qwe')])),
        env=env
    )
    ...
    assert not is_window_expression(
        nodes.FuncCall.make(name='func', args=[nodes.Field.make('my field')]), env=env
    )
    assert not is_window_expression(
        n.p(nodes.FuncCall.make(name='avg', args=[nodes.LiteralString.make('qwe')])),
        env=env
...
def test_get_query_fork_nesting_level():
    env = InspectionEnvironment()
    assert get_query_fork_nesting_level(
        n.formula(n.func.SOMEFUNC(n.fork(
...
def test_infer_data_type():
    ...
    assert infer_data_type(
        nodes.Field.make('Other Field'),
        field_types=field_types, env=env,
    ) == DataType.INTEGER
    assert infer_data_type(
        n.case(
    ...
    assert infer_data_type(
        n.wfunc.RANK(n.field('Other Field')),
        field_types=field_types, env=env
    ) == DataType.INTEGER
    assert infer_data_type(
        n.binary('==', n.field('One Field'), n.field('Other Field')),
        field_types=field_types, env=env
    ) == DataType.BOOLEAN

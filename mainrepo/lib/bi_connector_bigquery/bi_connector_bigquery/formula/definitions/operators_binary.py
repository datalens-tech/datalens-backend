...
DEFINITIONS_BINARY = [
    ...
    # %
    base.BinaryModInteger(variants=[
    ...
    base.BinaryPlusDatetimeNumber(variants=[
        V(
            D.BIGQUERY,
            lambda date, days: date + _bq_sec_interval(sa.cast(days*DAY_SEC, sa.INTEGER))
        ),
    ]),
    ...
    base.BinaryMinusDateFloat(variants=[
        V(
            ...
            lambda date, days: sa.cast(
                sa.cast(date, sa.DATETIME)
                - _bq_sec_interval(sa.cast(days*DAY_SEC, sa.INTEGER)),
                sa.DATE
            )
        ),
    ]),
    base.BinaryMinusDatetimeNumber(variants=[
        V(D.BIGQUERY, lambda date, days: sa.cast(date, sa.DATETIME) - _bq_sec_interval(sa.cast(days*DAY_SEC, sa.INTEGER))),
    ]),

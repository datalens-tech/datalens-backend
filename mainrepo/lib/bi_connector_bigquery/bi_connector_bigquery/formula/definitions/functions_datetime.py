...
DEFINITIONS_DATETIME = [
    ...
    base.FuncDateadd3DateNonConstNum(variants=[
        V(
            D.BIGQUERY,
            lambda date, what, num: sa.func.DATE_ADD(
                date, _bq_dynamic_interval(un_literal(what), num)
            )
        ),
    ]),
    ...
    base.FuncDatetrunc2Date(variants=[
        V(D.BIGQUERY, lambda date, unit: (
            sa.func.DATE_TRUNC(
                date,
                raw_sql(base.norm_datetrunc_unit(unit).replace('week', 'week(monday)'))
            )
    ...
    base.FuncDayofweek2(variants=[
        V(
            D.BIGQUERY,
            lambda date, firstday: sa.func.MOD(
                sa.func.EXTRACT('DAYOFWEEK', date) - 1 - base.norm_fd(firstday),
                7
            ) + 1
        ),
    ]),

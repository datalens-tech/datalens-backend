...
from bi_formula.definitions.common_datetime import (
    ch_date_with_tz, datetime_interval, make_ch_tz_args,
)
...
def _datetrunc2_ch_impl_19_3_3(
        date_ctx: TranslationCtx, unit_ctx: TranslationCtx,
) -> ClauseElement:
    ...
    if unit in {'year', 'quarter', 'month', 'week'}:
        return sa.func.toDateTime(
            sa.func.toStartOfInterval(
                date_expr,
                datetime_interval(unit, num, caps=False),
                *tz_args),
            *output_tz_args)
    if unit in {'day', 'hour', 'minute', 'second'}:
        return sa.func.toStartOfInterval(
            date_expr,
            datetime_interval(unit, num, caps=False),
            *tz_args)
...
DEFINITIONS_DATETIME = [
    ...
    base.FuncDateadd3DateNonConstNum(variants=[
        V(D.CLICKHOUSE, lambda date, what, num: sa.cast(
            date + datetime_interval(what.value, sa.func.ifNull(num, 0), ch_func=True),
            ch_types.Nullable(ch_types.Date)
        )),
    ]),
    base.FuncDateadd3DatetimeNonConstNum(variants=[
        V(D.CLICKHOUSE, lambda dt, what, num: (
            sa.func.toDateTime(
                dt + datetime_interval(what.value, sa.func.ifNull(num, 0), ch_func=True),
                'UTC'
            )
        )),
    ]),
    base.FuncDateadd3GenericDatetimeNonConstNum(variants=[
        V(D.CLICKHOUSE, lambda dt, what, num: (
            dt + datetime_interval(what.value, sa.func.ifNull(num, 0), ch_func=True)
        )),
    ]),
    base.FuncDateadd3DatetimeTZNonConstNum(variants=[
        V(D.CLICKHOUSE, lambda dt, what, num: (
            dt + datetime_interval(what.value, sa.func.ifNull(num, 0), ch_func=True)
        )),
    ]),
    ...
    base.FuncDatetrunc2Date(variants=[
        V(D.CLICKHOUSE, lambda date, unit: (
    ...
    base.FuncDatetrunc2DatetimeTZ(variants=[
        VW(D.CLICKHOUSE, _datetrunc2_ch_impl_19_3_3),
    ]),
    base.FuncDatetrunc3Date(variants=[
        V(D.CLICKHOUSE, lambda date, unit, num: (
            sa.func.toStartOfInterval(sa.func.toDate(date), datetime_interval(unit.value, num.value, caps=False))
            if base.norm_datetrunc_unit(unit) in {'year', 'quarter', 'month', 'week'} else
            sa.func.toDate(
                sa.func.toStartOfInterval(
                    sa.func.toDate(date),
                    datetime_interval(unit.value, num.value, caps=False)
                )
            )
            if base.norm_datetrunc_unit(unit) == 'day' else
            sa.func.toDate(date)
        )),
    ]),
    base.FuncDatetrunc3Datetime(variants=[
    ...
    base.FuncDayDatetimeTZ(variants=[
        VW(D.CLICKHOUSE, lambda date_ctx: sa.func.toDayOfMonth(*ch_date_with_tz(date_ctx))),
    ]),
    ...
    base.FuncDayofweek2(variants=[
        VW(D.CLICKHOUSE, lambda date_ctx, firstday_ctx: base.dow_firstday_shift(
            sa.func.toDayOfWeek(*ch_date_with_tz(date_ctx)),
            firstday_ctx.expression)),
    ]),
    base.FuncDayofweek2TZ(variants=[
        VW(D.CLICKHOUSE, lambda date_ctx, firstday_ctx: base.dow_firstday_shift(
            sa.func.toDayOfWeek(*ch_date_with_tz(date_ctx)),
            firstday_ctx.expression)),
    ]),
    ...
    base.FuncHourDatetimeTZ(variants=[
        VW(D.CLICKHOUSE, lambda date_ctx: sa.func.toHour(*ch_date_with_tz(date_ctx))),
    ]),
    ...
    base.FuncMinuteDatetimeTZ(variants=[
        VW(D.CLICKHOUSE, lambda date_ctx: sa.func.toMinute(*ch_date_with_tz(date_ctx))),
    ]),
    ...
    base.FuncMonthDatetimeTZ(variants=[
        VW(D.CLICKHOUSE, lambda date_ctx: sa.func.toMonth(*ch_date_with_tz(date_ctx))),
    ]),
    ...
    base.FuncQuarterDatetimeTZ(variants=[
        VW(D.CLICKHOUSE, lambda date_ctx: sa.func.toQuarter(*ch_date_with_tz(date_ctx))),
    ]),
    ...
    base.FuncSecondDatetimeTZ(variants=[
        VW(D.CLICKHOUSE, lambda date_ctx: sa.func.toSecond(*ch_date_with_tz(date_ctx))),
    ]),
    ...
    base.FuncYearDatetimeTZ(variants=[
        VW(D.CLICKHOUSE, lambda date_ctx: sa.func.toYear(*ch_date_with_tz(date_ctx))),
    ]),
]

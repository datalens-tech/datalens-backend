...
from bi_formula.definitions.common_datetime import ch_date_with_tz, ensure_naive_first_arg
...
class FuncDbCastClickHouseBase(base.FuncDbCastBase):
    ...
    def generate_cast_type(
            cls, dialect: DialectCombo, wr_name: TranslationCtx, value: TranslationCtx,
            type_args: Sequence[TranslationCtx]
    ) -> TypeEngine:
...
DEFINITIONS_TYPE = [
    # bool
    base.FuncBoolFromNull(variants=[
    ...
    # date
    base.FuncDate1FromNull(variants=[
        V(D.CLICKHOUSE, lambda _: sa.func.toDate(sa.null())),
    ]),
    base.FuncDate1FromDatetime(variants=[
    ...
    base.FuncDate1FromString(variants=[
    ...
    base.FuncFloatFromDate(variants=[
        V(D.CLICKHOUSE, lambda value: sa.func.toFloat64(
            sa.func.toUnixTimestamp(sa.func.toDateTime(value, 'UTC'), 'UTC')
        )),
    ]),
    ...
    # genericdatetime
    base.FuncGenericDatetime1FromNull(variants=[
        V(D.CLICKHOUSE, lambda _: sa.func.toDateTime(sa.null())),
    ]),
    base.FuncGenericDatetime1FromDatetime.for_dialect(D.CLICKHOUSE),
    base.FuncGenericDatetime1FromDate(variants=[
        V(D.CLICKHOUSE, lambda expr: sa.func.toDateTime(expr)),
    ]),
    base.FuncGenericDatetime1FromNumber(variants=[
        V(D.CLICKHOUSE, lambda expr: sa.func.toDateTime(expr)),
    ]),
    base.FuncGenericDatetime1FromString(variants=[
        V(D.CLICKHOUSE, lambda expr: sa.func.toDateTimeOrNull(expr)),
    ]),
    FuncGenericDatetime2CH(),

    # genericdatetime_parse
    base.FuncGenericDatetimeParse(variants=[
        V(D.CLICKHOUSE, lambda value: sa.func.parseDateTimeBestEffortOrNull(value)),
    ]),

...
import logging
from typing import AsyncGenerator, ClassVar, Callable, Awaitable, Optional
...
from bi_file_uploader_lib.data_sink.raw_bytes import RawBytesAsyncDataStream, S3RawFileAsyncDataSink
...
class LinksView(FileUploaderBaseView):
    ...
    FILE_TYPE_TO_DATA_FILE_PREPARER_MAP: dict[
        FileType,
        Callable[[str, RedisModelManager, Optional[str]], Awaitable[DataFile]]
    ] = {
    ...
    async def post(self) -> web.StreamResponse:
        ...
        task_processor = self.dl_request.get_task_processor()
        await task_processor.schedule(DownloadGSheetTask(
            file_id=df.id,
...
class UpdateConnectionDataView(FileUploaderBaseView):
    ...
    async def post(self) -> web.StreamResponse:
        ...
        return web.json_response(files_schemas.UpdateConnectionDataResultSchema().dump(dict(
            files=dfile_by_spreadsheet.values()
        )))

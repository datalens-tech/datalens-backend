...
table2 = table("myothertable", column("otherid", Integer), column("othername", String))

table3 = table("thirdtable", column("userid", Integer), column("otherstuff", String))
...
query1 = (
    t1a.join(t2a, t1a.c.myid == t2a.c.otherid, full=True)
    .outerjoin(t2b, t1a.c.myid == t2b.c.otherid, full=True)
    .select()
    # Note: currently this doesn't get wrapped.
    .where(t1a.c.myid > -1)
    .where(t1a.c.myid < 9000)
    # ... and this does.
    .where(sa.and_((t1a.c.name != letter * 8) for letter in "abcde"))
...
query2 = (
    sq1.join(t4a, sq1.c.myid == t4a.c.rem_id)
    ...
    .order_by(
        ...
        t4a.c.datatype_id,
        sq1.c.name,
        desc(sq1.c.description),
        desc(sq1.c.otherid),
        desc(sq1.c.othername),
        t4a.c.datatype_id,
        t4a.c.value,
        desc(t5a.c.rem_id),
        desc(t5a.c.datatype_id),
        desc(t5a.c.value),
        desc(t5b.c.rem_id),
        desc(t5b.c.datatype_id),
        desc(t5b.c.value),
    )
    .limit(int("12345" * 10))
    .offset(int("54321" * 10))
...
def get_dialects():
    try:
        from dl_sqlalchemy_chyt.base import BICHYTDialect
    ...
    try:
        ...
        pass
    try:
        from dl_sqlalchemy_mssql.base import BIMSSQLDialect

        yield BIMSSQLDialect()
    except ImportError:
        pass
    try:
        from dl_sqlalchemy_oracle.base import BIOracleDialect

        yield BIOracleDialect()
    except ImportError:
        pass
    try:
        from dl_sqlalchemy_postgres import BIPGDialect

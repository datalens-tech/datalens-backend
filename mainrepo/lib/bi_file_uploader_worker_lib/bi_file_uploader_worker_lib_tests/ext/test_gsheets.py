...
TEST_SHEET_TITLES_INDICES = {title: idx for idx, title in enumerate((
    'image',
    'experiment',
    'corners',
    'elaborate',
...
EXPECTED_HEADERS = {  # { <sheet_title>: { <has_header>: [ <title1>, <title2>, ... ] } }
    'elaborate': {
        True: ['int field', 'date field', 'float field', 'string field', 'bool field',
               'datetime field', 'time field', 'time period field'],
...
_MOSCOWSHOPS_V8_TEST_PARAMS = [
    ...
    [
        ['OrderID', 'ClientID', 'ProductID', 'ShopID', 'ClientAdress', 'ShopAdress', 'ShopName', 'ProductBrend',
         'ProductCategory', 'DayOfBirth', 'OrderDatetime', 'DeliveryDatetime', 'Discount', 'ProductCount',
         'DeliveryAdressCoord', 'ShopAdressCoord', 'ProductName', 'ClientName', 'DeliveryType', 'PaymentType',
         'ProductPrice', 'FinelAmount', 'Gender', 'ProductSubcategory', 'Amount', 'ClientStatus'],
        ['OrderID', 'ClientID', 'ProductID', 'ShopID', 'OrderDatetime', 'DeliveryDatetime', 'Discount',
         'ProductCount', 'DeliveryAdressCoord', 'DeliveryType', 'PaymentType', 'Price', 'FinelAmount', 'Amount'],
        ...
        [idx_to_alphabet_notation(i) for i in range(5)],  # ['ProductID', 'ProductCategory', 'ProductSubcategory', 'ProductBrend', 'ProductName'],
    ],
    [
...
async def downloaded_gsheet_file_id(
    ...
    task = await task_processor_client.schedule(DownloadGSheetTask(file_id=df.id, authorized=False, schedule_parsing=False))
...
async def test_parse_gsheet(
    ...
    assert preview.preview_data == [  # No number formats specified => everything is string => no header
        ['int field', 'date normal', 'date gsheet', 'float field', 'string field', 'bool field', 'dt normal', 'dt gsheet', 'time field', 'time period field'],
        ['1', '2001-07-31', '31.07.2001', '12,50', 'asdf', 'True', '2022-08-12 12:30:32', '12.08.2022 12:30:32', '12:00:12', '13:00:12'],
        [None, None, None, None, None, None, None, None, None, None],
        ['2', '1995-04-12', '12.04.1995', '12,40', 'zxcv', 'True', '2022-08-14 12:30:34', '14.08.2022 12:30:34', '14:00:12', '15:00:12'],
        ['8', '2008-02-29', '29.02.2008', None, '12,00', 'True', '2022-08-15 12:30:35', '15.08.2022 12:30:35', '15:00:12', '16:00:12'],
        ['3', '2012-03-12', '12.03.2012', '12,30', 'dfgh', 'True', '2022-08-16 12:30:36', '16.08.2022 12:30:36', '16:00:12', '17:00:12'],
...
async def test_parse_gsheet_with_file_settings(
    ...
    task = await task_processor_client.schedule(ParseFileTask(
        file_id=file_id,
...
async def create_gsheets_v2_connection(us_manager: AsyncUSManager, dfile: DataFile):
    ...
    for src in dfile.sources:
        ...
        conn_sources.append(GSheetsFileS3Connection.FileDataSource(
            id=src.id,
            file_id=dfile.id,
            title=src.title,
            status=FileProcessingStatus.in_progress,
...
async def test_save_source_task(
    ...
    assert all(conn.get_file_source_by_id(source.id).status == FileProcessingStatus.in_progress for source in df.sources[1:])
...
async def test_download_and_parse_big_gsheets(
    ...
    for src, has_header, header_values, user_types in zip(df.sources, expected_has_header, expected_headers, expected_user_types):
...
async def test_gsheets_full_pipeline(
    ...
    conn = await create_gsheets_v2_connection(usm, df)
    assert all(conn.get_file_source_by_id(source.id).status == FileProcessingStatus.in_progress for source in df.sources)
    ...
    for src in df.sources:
        ...
        for conn_src, has_header, header_values, user_types in zip(updated_conn.data.sources, expected_has_header, expected_headers, expected_user_types):
...
async def test_too_large_sheet(
    ...
    df = DataFile(
        ...
        status=FileProcessingStatus.in_progress,
        file_type=FileType.gsheets,
        user_source_properties=GSheetsUserSourceProperties(spreadsheet_id='1dfUajkRxGTxGXrwjXtCRWhEjHlZX9tMVEvla5fio93w'),
    )
    await df.save()
...
async def test_url_params_encoding(
    ...
    df = DataFile(
        ...
        status=FileProcessingStatus.in_progress,
        file_type=FileType.gsheets,
        user_source_properties=GSheetsUserSourceProperties(spreadsheet_id='1dE5-owygpjK-F65qlCwUa1YHGzh4dzBhyajURSLMvYU'),
    )
    await df.save()

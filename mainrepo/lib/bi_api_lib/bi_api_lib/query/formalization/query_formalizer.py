...
def need_permission_on_entry(us_entry, permission) -> None:  # type: ignore
    ...
    if not us_entry.permissions[permission]:
        raise bi_core.exc.USPermissionRequired(us_entry.uuid, permission)
...
class DataQuerySpecFormalizer(SimpleQuerySpecFormalizer):  # noqa
    ...
    def _ensure_not_unsupported_type(self, field: BIField) -> None:
        if field.cast == BIType.unsupported:
            raise bi_query_processing.exc.LogicError(f"Cannot select fields of unsupported type: {field.title}")

...
from typing import (
    Any, AsyncIterable, Dict, ClassVar, Collection, List,
    Optional, TYPE_CHECKING, Type,
)
...
from bi_api_lib.app.data_api.resources.base import BaseView, requires
from bi_api_lib.request_model.data import DataRequestModel, FieldAction, Action
from bi_api_lib.api_common.dataset_loader import DatasetApiLoader, DatasetUpdateInfo
from bi_api_lib.api_common.data_serialization import DataRequestResponseSerializer, get_fields_data_serializable
from bi_query_processing.postprocessing.primitives import (
    PostprocessedQuery, PostprocessedQueryBlock, PostprocessedQueryUnion,
    PostprocessedRow,
)
from bi_query_processing.postprocessing.postprocessor import DataPostprocessor
from bi_api_lib.query.formalization.legend_formalizer import (
    LegendFormalizer, ResultLegendFormalizer, PivotLegendFormalizer,
    DistinctLegendFormalizer, RangeLegendFormalizer, PreviewLegendFormalizer,
)
...
from bi_core.us_manager.mutation_cache.engine_factory import CacheInitializationError
from bi_core.us_manager.mutation_cache.mutation_key_base import MutationKey
from bi_core.us_manager.mutation_cache.usentry_mutation_cache import RedisCacheEngine, USEntryMutationCache
...
class DatasetDataBaseView(BaseView):
    ...
    async def default_query_execution_cm_stack(
        ...
        """
        See also:
        `bi_api_lib.app.data_api.resources.dashsql.DashSQLView.enrich_logging_context`
        """
                ...
                try:
                    ...
                    try:
                        if isinstance(target_conn, ExecutorBasedMixin):
                            ce_cls_str = sr.get_conn_executor_factory().get_async_conn_executor_cls(
                                target_conn
                            ).__qualname__
                        else:
                            ce_cls_str = "N/D"
    ...
    def try_get_cache(self) -> Optional[USEntryMutationCache]:
        try:
            ...
            mutation_cache = mc_factory.get_mutation_cache(
                usm=self.dl_request.us_manager, engine=cache_engine,
            )
    ...
    async def check_for_notifications(
            self,
            services_registry: BiApiServiceRegistry,
    ...
    async def execute_all_queries(
        ...
        legend = legend_formalizer.make_legend(
    ...
    def _make_response_v1(
            ...
            totals_query: Optional[str] = None, totals: Optional[PostprocessedRow] = None,
    ) -> Dict[str, Any]:
    ...
    def check_dataset_revision_id(self, req_model: DataRequestModel) -> None:
        ...
        if dataset_revision_id is not None and dataset_revision_id != self.dataset.revision_id:
            LOGGER.warning(f'Dataset revision id mismatch: got {dataset_revision_id} from the request, '

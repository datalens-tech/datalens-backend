...
from bi_query_processing.legend.field_legend import LegendItem
...
class DataRequestResponseSerializer(DataRequestResponseSerializerV2Mixin):
                ...
                [
                    [
                        ...
                        [
                            ...
                            [
                                'DataType',
                                bi_to_yql(enum_not_none(item.data_type))
                            ]
                        ]
                    ]
    ...
    def make_data_response_v1(
        ...
        if (
                merged_stream.meta.blocks[0].debug_query
                and query_type in cls._SHOW_QUERY_FOR_QUERY_TYPES
        ):
    ...
    def make_data_response_v2(
        ...
        data_rows: List[dict] = [
            {'data': row.data, 'legend': row.legend_item_ids}
            for row in merged_stream.rows
        ]
        data['result_data'] = [
            {'rows': data_rows}
        ]
...
class PivotDataRequestResponseSerializer(DataRequestResponseSerializerV2Mixin):
    ...
    def make_pivot_response(
        ...
        pivot_spec_data = {
            'structure': [
                PivotItemSchema().dump(item)
                for item in pivot_table.pivot_legend.items
            ],
        }
        ...
        table_data = {
            'columns': [
                [
                    _serialize_cell_vector(value_vector)
                    for value_vector in col.values
                ]
            ...
            'row_dimension_headers': [
                _serialize_cell_vector(flat_header)
                for flat_header in pivot_table.get_row_dim_headers()
            ],
            'rows': [
                {
                    'header': [
                        _serialize_cell_vector(value_vector)
                        for value_vector in row.header.values
                    ],
                    'header_with_info': {
                        'cells': [
                            _serialize_cell_vector(value_vector)
                    ...
                    'values': [
                        _serialize_cell_vector(value_vector)
                        for value_vector in row.values
                    ],
                }

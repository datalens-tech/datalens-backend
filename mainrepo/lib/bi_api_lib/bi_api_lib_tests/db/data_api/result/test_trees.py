...
from bi_api_lib_tests.utils import data_source_settings_from_table
...
def get_tree(
        ...
        dataset: Dataset, data_api: SyncHttpDataApiV2,
        order_by: Sequence[Union[ResultField, OrderedField]] = (),
    ...
    branch_items = make_request_legend_items_for_tree_branches(
        branches=branches, dataset=dataset, tree_title='dept_tree',
    )
    ...
    result = OrderedDict(
        (
            ...
            tuple([cast_func(row[idx+1]) for idx, (name, cast_func) in enumerate(measures)])
        )
...
def test_simple_tree(api_v1, data_api_v2, db_with_array_support, connection_with_array_support):
    data_api = data_api_v2
    connection_id = connection_with_array_support.uuid
    ds = make_tree_dataset(db=db_with_array_support, connection_id=connection_id, api_v1=api_v1)

    tree = get_tree([
        (1, []),
    ], dataset=ds, data_api=data_api)
    assert tree == {
        ('Company',): (10,),
    }
...
def test_simple_tree_with_order_by(api_v1, data_api_v2, db_with_array_support, connection_with_array_support):
    data_api = data_api_v2
    connection_id = connection_with_array_support.uuid
    ds = make_tree_dataset(db=db_with_array_support, connection_id=connection_id, api_v1=api_v1)
...
def test_simple_tree_with_order_by_and_rsum(api_v1, data_api_v2, db_with_array_support, connection_with_array_support):
    data_api = data_api_v2
    connection_id = connection_with_array_support.uuid
    ds = make_tree_dataset(db=db_with_array_support, connection_id=connection_id, api_v1=api_v1)
...
def test_tree_and_totals_incompatibility_error(api_v1, data_api_v2, db_with_array_support, connection_with_array_support):
    data_api = data_api_v2
    connection_id = connection_with_array_support.uuid
    ds = make_tree_dataset(db=db_with_array_support, connection_id=connection_id, api_v1=api_v1)
...
def test_multiple_trees_error(api_v1, data_api_v2, db_with_array_support, connection_with_array_support):
    data_api = data_api_v2
    connection_id = connection_with_array_support.uuid
    ds = make_tree_dataset(db=db_with_array_support, connection_id=connection_id, api_v1=api_v1)
...
def test_invalid_data_type_for_tree_error(api_v1, data_api_v2, db_with_array_support, connection_with_array_support):
    data_api = data_api_v2
    connection_id = connection_with_array_support.uuid
    ds = make_tree_dataset(db=db_with_array_support, connection_id=connection_id, api_v1=api_v1)
...
def test_invalid_legend_item_reference_error(api_v1, data_api_v2, db_with_array_support, connection_with_array_support):
    data_api = data_api_v2
    connection_id = connection_with_array_support.uuid
    ds = make_tree_dataset(db=db_with_array_support, connection_id=connection_id, api_v1=api_v1)

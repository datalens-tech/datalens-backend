    ...
    result_resp = data_api_v2.get_result(
        ...
        filters=[
            ds.find_field(title='Category').filter(
                op=WhereClauseOperation.EQ, values=['Technology']).for_block(block_id=0),
            ds.find_field(title='Category').filter(
                op=WhereClauseOperation.NE, values=['Technology']).for_block(block_id=1),
        ],
        blocks=[
            BlockSpec(block_id=0, placement=RootBlockPlacement()),
            BlockSpec(block_id=1, parent_block_id=0, placement=AfterBlockPlacement()),
...
def test_multiblock_limit_offset(api_v1, data_api_v2, dataset_id):
    ...
    def get_result(block_cnt: int, limit: Optional[int] = None, offset: Optional[int] = None) -> List[list]:
        result_resp = data_api_v2.get_result(
            ...
            fields=[
                *[
                    ds.find_field(title='City').as_req_legend_item(block_id=block_id)
            ...
            blocks=[
                BlockSpec(block_id=0, placement=RootBlockPlacement()),
                *[
                    BlockSpec(block_id=block_id, placement=AfterBlockPlacement())
                    for block_id in range(1, block_cnt)
                ],
            ],
            limit=limit,
            offset=offset,

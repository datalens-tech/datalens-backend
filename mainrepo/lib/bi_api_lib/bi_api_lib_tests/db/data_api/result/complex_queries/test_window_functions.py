...
from bi_api_lib_tests.db.data_api.result.complex_queries.utils import MultiQueryInterceptor, count_joins
...
def test_window_function_filter(api_v1, data_api_v2, dataset_id):
    ...
    result_resp = get_result(filters=[
...
def test_order_dependent_window_function(api_v1, data_api_v2, dataset_id):
    ...
    assert all(pytest.approx(rsum_asc_values_1[i]) == rsum_asc_values_1[i-1] + sum_values_1[i]
               for i in range(1, len(data_1)))
    assert all(pytest.approx(rsum_desc_values_1[i]) == rsum_desc_values_1[i-1] - sum_values_1[i-1]
               for i in range(1, len(data_1)))
    ...
    assert all(pytest.approx(rsum_asc_values_2[i]) == rsum_asc_values_2[i-1] + sum_values_2[i]
               for i in range(1, len(data_2)))
    assert all(pytest.approx(rsum_desc_values_2[i]) == rsum_desc_values_2[i-1] - sum_values_2[i-1]
               for i in range(1, len(data_2)))
...
def test_subquery_column_name_conflict(api_v1, data_api_v2, dataset_id):
    ...
    ds = add_formulas_to_dataset(api_v1=api_v1, dataset_id=dataset_id, formulas={
        'Group Sales': 'SUM([Sales])',
        'Sales RSUM BFB': (
            'RSUM(SUM([Sales]) TOTAL BEFORE FILTER BY [Order Date])'
        ),
    ...
    result_resp = data_api.get_result(
        ...
        filters=[
            ds.find_field(title='Order Date').filter(
                op=WhereClauseOperation.EQ, values=['2014-05-01'],
            ),
            ds.find_field(title='Sales RSUM BFB').filter(
                op=WhereClauseOperation.LT, values=['2000000'],
            ),
        ],
        fail_ok=True,
    )
...
def test_bfb_with_only_winfunc_measure(api_v1, data_api_v2, dataset_id):
    ...
    def get_data(measures: tuple[str, ...]) -> list[list[str]]:
        result_resp = data_api.get_result(
            ...
            fields=[
                ds.find_field(title='Order Date'),
                *[ds.find_field(title=measure) for measure in measures]
            ],
            filters=[

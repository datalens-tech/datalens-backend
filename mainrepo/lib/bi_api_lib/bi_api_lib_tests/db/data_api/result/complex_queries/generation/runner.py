...
from bi_api_lib_tests.db.data_api.result.complex_queries.generation.generator import TestSettings
...
class PreGeneratedLODTestRunner:
    ...
    def get_measure_data(
            self, ds: Dataset, test_settings: TestSettings, measures: list[str],
    ) -> dict[str, list[float]]:
        result_resp = self.data_api.get_result(
            ...
            filters=[
                ds.find_field(title=name).filter(op, values)
                for name, (op, values) in test_settings.filters.items()
            ],
            order_by=[
                *[ds.find_field(title=name) for name in test_settings.base_dimensions],
            ],
    ...
    def run_test(self, test_settings: TestSettings, ignore_400_error: bool = False) -> None:
        ...
        try:
            all_measures = self.get_measure_data(
                ds=ds, test_settings=test_settings,
                measures=[name for name, _ in measure_fields]
            )
            for measure_name, measure_formula in measure_fields:

...
def _make_redis_persistent_settings(cfg: Any, db: int) -> Optional[RedisSettings]:
    ...
    return RedisSettings(  # type: ignore
        ...
        DB=db,
    ) if is_setting_applicable(cfg, 'REDIS_PERSISTENT_HOSTS') else None
...
class FileUploaderBaseSettings:
    REDIS_APP: RedisSettings = s_attrib(  # type: ignore
        "REDIS_APP",
        fallback_factory=(
            lambda cfg: _make_redis_persistent_settings(cfg=cfg, db=cfg.REDIS_FILE_UPLOADER_DATA_DB)
        ),
    )
    ...
    REDIS_ARQ: RedisSettings = s_attrib(  # type: ignore
        "REDIS_ARQ",
        fallback_factory=(
            lambda cfg: _make_redis_persistent_settings(cfg=cfg, db=cfg.REDIS_FILE_UPLOADER_TASKS_DB)
        ),
    )
    ...
    S3: S3Settings = s_attrib(  # type: ignore
        ...
        fallback_factory=(
            ...
            ) if is_setting_applicable(cfg, 'S3_ENDPOINT_URL') else None
        ),
    )

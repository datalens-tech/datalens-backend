...
class GroupByNormalizer:
    def normalize_flat_query_grouping(
            self, compiled_flat_query: CompiledQuery,
    ) -> CompiledQuery:
        ...
        for formula in compiled_flat_query.select:
            ...
            if (
                    not is_aggregate_expression(expr, env=inspect_env)
                    and used_fields(expr)
            ):
        ...
        if add_group_by:
            compiled_flat_query = compiled_flat_query.clone(
                group_by=compiled_flat_query.group_by + add_group_by
            )

        return compiled_flat_query

    def _gen_patches_for_multi_query(
            self, compiled_multi_query: CompiledMultiLevelQuery,
            ...
            if updated_query is not compiled_flat_query:
                patches.append(
                    CompiledMultiLevelQueryReplacementPatch(
                        subquery_idx=subquery_idx, new_subquery=updated_query,
                    )
                )
    ...
    def normalize_multi_query_grouping(
        ...
        compiled_multi_query = apply_multi_query_replacement_patches(
            compiled_multi_query=compiled_multi_query, patches=replacement_patches,
        )

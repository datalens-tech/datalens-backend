...
class RecursiveQuerySlicer:
    ...
    def _slice_levels_using_planner(
        ...
        original_used_avatar_ids = {
            from_id
            for query in compiled_levels[0].queries
        ...
        for compiled_level in compiled_levels:
            ...
            level_types = {
                lvl.level_type
                for sub_multi_query in sub_multi_queries
                for lvl in sub_multi_query.levels
            }
        ...
        new_used_avatar_ids = {
            from_id
            for q in compiled_levels[0].queries
            for from_id in q.joined_from.iter_ids()
        }
        assert new_used_avatar_ids == original_used_avatar_ids, (
            f'Avatar ID mismatch. Original set: ({original_used_avatar_ids});'
            f'new set ({new_used_avatar_ids})'
        )
        ...
        assert new_top_level_query_ids == original_top_level_query_ids, (
            f'Query ID mismatch. Original set: ({original_top_level_query_ids});'
            f'new set ({new_top_level_query_ids})'
        )
    ...
    def _slice_levels_recursively_for_level_type(
        ...
        for query_planner in self._secondary_planners_by_level_type.get(level_type, ()):
            self._log_info(
                f'Sending {len(compiled_levels)} {level_type.name} levels '
                f'to planner {type(query_planner).__name__}'
            )
        ...
        self._log_info(
            f'Got {len(compiled_levels)} levels after slicing '
            f'for level type {level_type.name}'
        )
    ...
    def slice_query_recursively(self, compiled_query: CompiledQuery) -> CompiledMultiLevelQuery:
        ...
        for level_type in level_type_order:
            old_compiled_levels_for_type = [
                lvl for lvl in compiled_multi_query.levels
        ...
        for level_idx, compiled_level in enumerate(all_new_compiled_levels):
            ...
            level_used_avatar_ids = {
                from_id
                for q in compiled_level.queries
                for from_id in q.joined_from.iter_ids()
            }

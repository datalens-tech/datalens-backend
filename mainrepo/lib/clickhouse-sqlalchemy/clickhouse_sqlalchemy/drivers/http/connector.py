...
from ...parsers import FORMATS, Parts
...
class Connection(object):
    ...
    def __init__(self, *args, **kwargs):
        format_name = kwargs.pop('format', 'default')
        self.format_info = FORMATS[format_name]()
        kwargs.update(format_info=self.format_info)
...
class Cursor(object):
    ...
    def execute(self, operation, parameters=None, context=None):
        ...
        execute_ext = getattr(transport, 'execute_ext', None)
        if execute_ext is not None:
            response_gen = execute_ext(raw_sql, params=params)
            self._process_response_ext(response_gen)
        else:
    ...
    def _process_response_ext(self, response):
        try:
            part, item = next(response)
        except StopIteration:
            raise Exception("Not a well-behaved extended response: empty")
        assert part == Parts.META
        self._schema = item
        self._columns = [col['name'] for col in item]
        self._types = [col['type'] for col in item]

        self._response = self._wrap_response_ext(response)
        if self._prefetch:
            self._rows = list(self._response)

    def _wrap_response_ext(self, response):
        for part, item in response:
            if part == Parts.DATACHUNK:
                for row in item:
                    yield row
                ...
                self._stats = item
                self._totals = self._totals or item.get('totals')
            elif part == Parts.META:
                raise Exception((
                    "Not a well-behaved extended response: "
                    "unexpected meta piece"))

...
class SelectTestCase(BaseTestCase):
    ...
    def test_group_by_with_totals(self):
        ...
        query = select.create_legacy_select([table.c.x]).group_by(table.c.x)
        self.assertEqual(
            self.compile(query),
            'SELECT x FROM t1 GROUP BY x'
        )

        query = select.create_legacy_select([table.c.x]).group_by(table.c.x).with_totals()
        self.assertEqual(
            self.compile(query),
    ...
    def test_sample(self):
        ...
        query = select.create_legacy_select([table.c.x]).sample(0.1).group_by(table.c.x)
        self.assertEqual(
            self.compile(query),
    ...
    def test_final(self):
        ...
        query = select.create_legacy_select([table.c.x]).final().group_by(table.c.x)
        self.assertEqual(
            self.compile(query),
    ...
    def test_nested_type(self):
        ...
        query = select.create_legacy_select(
    ...
    def test_nested_array_join(self):
        ...
        self.assertEqual(
            self.compile(query),
            'SELECT p.child1, p.child2, parent.child1 AS child1_1 '
            'FROM t1 '
            'ARRAY JOIN parent AS p'
        )
    ...
    def test_join(self):
        ...
        def make_statement(type=None,
            ...
            return select.create_legacy_select([table_1.c.x]).select_from(join)

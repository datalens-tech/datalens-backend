...
from bi_core.data_processing.prepared_components.default_manager import (
    DefaultPreparedComponentManager,
)
...
if TYPE_CHECKING:
    ...
    from bi_core.data_processing.prepared_components.manager_base import (
        PreparedComponentManagerBase,
    )
...
class SourceDbExecAdapter(ProcessorDbExecAdapterBase):  # noqa
    ...
    def _make_query_res_info(
            self, query: Union[str, Select], user_types: Sequence[BIType],
    ) -> QueryAndResultInfo:
    ...
    async def _execute_and_fetch(
            self, *,
            query: Union[str, Select], user_types: Sequence[BIType],

...
from dl_formula.inspect.expression import is_constant_expression
...
from dl_query_processing.legacy_pipeline.subqueries.query_tools import (
    get_formula_list_used_avatar_ids,
...
class MultiQuerySanitizer:
    ...
    def _remove_unused_avatars_from_join_on(
        self,
        non_join_formulas: Sequence[CompiledFormulaInfo],
        ...
        while True:
            ...
            join_on_list = [
                join_on_formula for join_on_formula in join_on_list if join_on_formula.right_id not in remove_avatar_ids
            ]
    ...
    def sanitize_flat_query(
        ...
        used_aliases: Optional[AbstractSet[str]],
        sanitize_select: bool = True,
        sanitize_join_on: bool = True,
        ...
        if sanitize_select:
            ...
            new_select = [formula for formula in compiled_query.select if formula.alias in used_aliases]
        ...
        only_constants = all(
            is_constant_expression(formula.formula_obj.expr, env=inspect_env) for formula in new_select
        )
        ...
        if sanitize_join_on and compiled_query.join_on:
            non_join_formulas: Sequence[CompiledFormulaInfo] = tuple(
                itertools.chain(new_select, new_group_by, new_filters, new_order_by)
            )
        ...
        new_all_formulas = (
            new_select
            + new_group_by
            + new_order_by  # type: ignore
            + new_filters
            + new_join_on  # type: ignore
        )
        ...
        if not used_avatar_ids:
            ...
            root_from_id = next(
                iter(from_obj.id for from_obj in compiled_query.joined_from.froms if from_obj.id in used_avatar_ids)
            )
    ...
    def sanitize_compiled_level(
        ...
        for compiled_query in compiled_level.queries:
            updated_query = self.sanitize_flat_query(
                ...
                is_bottom_level=is_bottom_level,
                used_aliases=used_aliases,
                sanitize_select=sanitize_select,
                sanitize_join_on=True,
            )

...
def joined_from_from_avatar_ids(
    from_ids: Sequence[str], base_avatars: dict[str, FromObject], cols_by_query: dict[str, Sequence[str]]
) -> JoinedFromObject:
    ...
    for from_id in from_ids:
        ...
        if from_id in base_avatars:
            ...
            from_obj = SubqueryFromObject(
                ...
                query_id=from_id,
                alias=from_id,
                columns=tuple(FromColumn(id=col_name, name=col_name) for col_name in cols_by_query[from_id]),
            )

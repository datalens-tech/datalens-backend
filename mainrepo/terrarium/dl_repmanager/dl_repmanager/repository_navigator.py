...
from typing import ClassVar, Collection, Generator, Optional

import attr

from dl_repmanager.primitives import PackageInfo, EntityReferenceType, EntityReference, ImportSpec
...
class RepositoryNavigator:
    ...
    def _filter_imports(
            self, import_specs: list[ImportSpec], mask: Optional[re.Pattern] = None, exclude_standard: bool = True,
    ) -> list[ImportSpec]:
    ...
    def _collect_imports_from_module(
    ...
    def collect_import_bases_from_module(
        ...
        return {
            get_base_package_name(import_spec.import_module_name)
            for import_spec in import_specs
        }
    ...
    def resolve_entity_to_type(
            self, entity_ref: EntityReference,
        ...
        match entity_ref.ref_type:
            case EntityReferenceType.package_type:
                ...
                if to_ref_type == EntityReferenceType.package_type:
                    ...
                    for package_entity_ref in package_entity_refs:
                        result.update(self.resolve_entity_to_type(
                            entity_ref=package_entity_ref, to_ref_type=to_ref_type,
                        ))
                    return result
            ...
            case EntityReferenceType.path:
                ...
                if to_ref_type == EntityReferenceType.path:
                    return {
                        EntityReference(ref_type=EntityReferenceType.path, name=str(path))
                        for path in paths
                    }
                ...
                if to_ref_type == EntityReferenceType.package_reg:
                    return {
                        package_info.reg_entity_ref
                        for package_info in path_to_package_info.values()
                    }
        ...
        raise ValueError(
            f'Unsupported conversion from: {entity_ref.ref_type.name} '
            f'to {to_ref_type.name}'
        )

    def resolve_entities_to_type(
            self, entity_list: Collection[EntityReference],
            to_ref_type: EntityReferenceType,
    ) -> set[EntityReference]:
    ...
    def _collect_imports_from_entities(self, entity_list: Collection[EntityReference]) -> list[ImportSpec]:
        path_refs = self.resolve_entities_to_type(
            entity_list=entity_list, to_ref_type=EntityReferenceType.path)

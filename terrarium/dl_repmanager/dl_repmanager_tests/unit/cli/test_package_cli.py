...
from dl_repmanager_tests.unit.tool_runner import PackageCliRunner


class TestPackageCli(RepmanagerCliTestBase):
    @pytest.fixture(scope="class")
    def package_cli(self, package) -> PackageCliRunner:
        return self.get_package_cli(package_path=package.abs_path)

    def test_list_i18n_domains(self, repo, package, package_cli) -> None:
        with self.toml_writer(repo=repo, toml_path=package.toml_path) as toml_writer:
            ...
            toml_writer.set_array_value(section_name=section_name, key="first_domain", value=[{"path": "first_path"}])
